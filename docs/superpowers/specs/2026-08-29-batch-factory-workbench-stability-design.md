...
- 左列：260px。
- 中列：min 440px，默认占剩余空间约 52%。
- 右列：min 380px，默认占剩余空间约 38%。
- 右上 VIDEO：右列高度约 58%。
- 右下合并/发布：右列高度约 42%。
...
- 900px–1179px：两列，小说队列固定较窄；中间与右侧作为主工作区上下排列，禁用不合理的纵向三列拖动。
- < 900px：单列堆叠，每个模块正常滚动，不提供拖动分割线。

具体阈值可以在实现时通过现有侧栏占用宽度验证，但必须由容器宽度决定，而不是只看 `window.innerWidth`。
