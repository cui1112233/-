...
原来“解析输入/解析格式”的设置区域直接改造成“版本对应配置档”区域。其主要作用是同步 121 网站配置档，并把配置档绑定到原文、AI1～AI5；不再显示解析格式、列顺序等旧控件。书籍清单输入仍保留为任务入口所必需的内容，不承担解析配置职责。AI 生成数量由 `selected_versions` 中的 AI 版本决定，不再使用独立的默认 AI 文案数量作为处理依据。

## Architecture

`/novel-fetch` 继续使用主线 React 外壳，但其工作区由 `/batch-rewrite/index.html` 提供。处理页原解析设置区改为版本对应配置档同步区；版本选择、AI 槽位处理方案和 121 配置档绑定统一保存到任务元数据，并通过任务请求传递到后端。网站提交直接复用同一份选中版本，避免处理和提交各自维护一套选择。
...
1. 原“解析输入/解析格式”区域不再显示解析控件，改为“版本对应配置档”区域。
2. 处理页显示原文、AI1～AI5 六个版本选择框。
3. AI1～AI5 每个版本拥有独立的处理方案选择框。
4. 任务创建时保存 `selected_versions`、`target_versions` 和 `ai_slot_methods_snapshot`。
5. 重新打开任务时恢复原先的版本选择和 AI 槽位方案，不被当前配置覆盖。
6. AI 生成只生成已选择的 AI 版本；未选择的版本不生成。
7. 网站提交使用处理页确认的版本集合，并支持 original、ai1～ai5 的独立配置档绑定。
8. “同步批量后台配置”更新可用 121 配置档。
9. “同步批量风格类型”更新可用风格目录。
10. 失败响应必须说明是版本选择、配置同步、登录会话还是上传接口错误。
11. 兼容没有显式版本选择的历史任务，按旧 `aiCount` 推导目标版本。
