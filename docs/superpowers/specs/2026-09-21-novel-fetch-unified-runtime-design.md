...
本轮收口不得以改版或功能削减换取实现简化。`/novel-fetch` 路径、当前深色视觉布局、处理/任务/配置/知识库/处理规则/记录六个栏目、任务表格、真实日期筛选、批量操作、121 登录与提交、知识库和规则入口均为冻结的用户可见契约。

实施顺序是先在不改变页面外观和交互的前提下替换模型解析、任务执行和审计后端；随后再逐块替换旧页面内部实现。每一块替换前建立当前公网的截图、关键 DOM 文案、按钮可用性和 API 行为清单，替换后逐项对照。只有同一功能清单、已登录验收和数据保留检查均通过，相关旧资产或路由才可移除；不得先删除旧链路后补功能。
...
`/novel-fetch` 最终直接渲染 React 小说获取工作台，不再使用 iframe，也不加载 `batch-rewrite/v2`、`v78-*`、`121-login-hotfix` 或任务可见性等补丁资产。迁移期间保持冻结的视觉和交互契约。页面包括处理、任务、配置、知识库、规则和记录六个视图，但它们共享同一 React 状态、同一 API client 和同一任务审计模型。
...
- 视觉与功能回归测试逐项覆盖冻结清单：六个栏目、任务表格、日期筛选、批量操作、121 提交、知识库和规则入口的可见性、文案和已登录行为必须与切换前一致。
