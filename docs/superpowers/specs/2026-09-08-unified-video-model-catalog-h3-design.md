...
3. 用户只选择“MiniMax H3”，不再选择文生/图生模式；没有上传图片时自动使用 H3 文生视频工作流，上传图片时自动使用 H3 多图参考生视频工作流。界面显示支持的时长上限为 15 秒和相应分辨率选项。
...
- AutoDL 同时提供 H3 文生视频工作流 `minimax_h3_lightx2v_no_pic` 和 H3 多图参考生视频工作流 `minimax_h3_lightx2v_v5_15s`；二者应由同一个“MiniMax H3”模型记录按输入自动选择。
...
- 把统一的最终 prompt、时长、分辨率和参考图转换为 H3 工作流请求体；有参考图时使用 `minimax_h3_lightx2v_v5_15s`，没有参考图时使用 `minimax_h3_lightx2v_no_pic`。
...
- 选中 H3 时，剧本页和批量工厂都把可用的参考图、prompt、duration、resolution 传给服务端；服务端根据参考图是否存在自动选择工作流，页面不要求用户选择模式。
...
- H3 无图 workflow ID，默认使用 `minimax_h3_lightx2v_no_pic`。
- H3 有图 workflow ID，默认使用 `minimax_h3_lightx2v_v5_15s`。
...
- 模型目录合同：H3 为 `kind: video`，名称、两种 workflow ID、15 秒能力和自动模式规则正确；前端不得维护第二份 H3 列表。
...
- 剧本页合同：H3 出现在同一目录下；无图和有图分别触发正确工作流，不出现额外模式选择框。
...
4. 配置 Token 后，不上传图片提交一条测试视频，确认自动走 H3 文生视频；再上传图片提交一条测试视频，确认自动走 H3 多图参考生视频，并看到“已提交/排队中/生成中/成功”状态变化。
...
- 本次不增加文生/图生模式选择框；模式由是否上传参考图自动决定。
