...
`功能/修复分支 -> 测试 -> 合入 v88 -> 记录 commit SHA -> 再发布/同步公网`
...
1. 当前 Git `v88` 最新 SHA。
2. 本任务改动是否已经写进 Git，而不是只存在公网。
3. 是否会用旧文件/旧热修复覆盖当前 V88。
4. 小改是否可以走增量部署，而不是完整 Docker 构建。
5. 发布完成后是否记录运行 SHA 并做健康检查。
...
**Git `v88` 是唯一正式源码；公网只是运行副本；所有修复先入 Git，再发布；日常小改优先 Git -> ECS 增量直部署，不再靠反复修改公网维持版本。**
