...
test('H3 script route resolves an account-owned local reference image before submission', async t => {
  let submitted;
  const h3App = await startApp({
    h3ApiKeyReader: () => 'server-only-h3-token',
    referenceImageResolver: ({ imageURL }) => imageURL === '/api/novel-panel/reference-assets/file/character/hero/main'
      ? 'https://public.example.test/api/reference-assets/h3/character/hero/main?u=alice&e=9999999999&s=signed'
      : '',
    h3Submit: async input => {
      submitted = input;
      return { statusCode: 200, text: JSON.stringify({ data: { task_id: 'task-local-ref' } }) };
    }
  });
  t.after(() => h3App.server.close());
  ...
  const response = await fetch(`${h3App.baseURL}/api/script-video`, {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      modelKey: H3_MODEL_KEY,
      prompt: '人物抬头',
      imageUrls: ['/api/novel-panel/reference-assets/file/character/hero/main']
    })
  });

  assert.equal(response.status, 202);
  assert.equal(submitted.payload.ref_image_0, 'https://public.example.test/api/reference-assets/h3/character/hero/main?u=alice&e=9999999999&s=signed');
});
