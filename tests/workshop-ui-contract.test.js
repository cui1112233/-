...
test('上传路由通过 MySQL 工作台网关读取版本且上传面板放开 workshop 门槛', () => {
  const app = read('app.js');
  const uploadRouter = read('routes/novel-fetch-upload.js');
  assert.match(app, /createNovelFetchUploadRouter\(\{ store: resolvedNovelFetchStore, workshopGateway: shuihuoGateway \}\)/);
  assert.match(uploadRouter, /createMySQLWorkshopStore/);
  assert.match(uploadRouter, /await workshopTasks\.readVersionText/);
  assert.doesNotMatch(uploadRouter, /workshopTasks\.pathForAiVersion/);
  const nf = read('frontend/src/user/pages/NovelFetchPage.jsx');
