...
const { processSensitiveTextV2 } = require('../lib/novel-fetch-workshop/sensitive-v2');
...
test('replace mode never calls AI', async () => {
  let calls = 0;
  const result = await processSensitiveTextV2({ text: '甲坏词。乙坏词。', settings: settings('replace'), ai: { chatCompletion: async () => { calls += 1; } } });
...
test('ai_each repairs each hit independently', async () => {
  let calls = 0;
  const result = await processSensitiveTextV2({
