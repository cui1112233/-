...
const loginHotfix = fs.readFileSync(path.join(__dirname, '..', 'frontend/public/batch-rewrite/121-login-hotfix.js'), 'utf8');
...
test('121 登录守卫必须有超时并在结束时恢复按钮', () => {
  ...
  assert.match(loginHotfix, /REQUEST_TIMEOUT_MS = 65_000/);
  assert.match(loginHotfix, /if \(submit\) submit\.disabled = false/);
  assert.match(loginHotfix, /api\('\/api\/web-submit\/test-visible'/);
});
...
test('登录弹窗在配置尚未加载时也能收口，不会因空配置永久停在验证中', () => {
  assert.match(loginHotfix, /async function ensureServerConfig\(\)/);
  assert.match(loginHotfix, /\.\.\.\(state\.config\?\.web_submit \|\| \{\}\)/);
  assert.match(loginHotfix, /if \(submit\) submit\.disabled = false/);
  assert.match(loginHotfix, /dialog\.addEventListener\('submit'/);
});
