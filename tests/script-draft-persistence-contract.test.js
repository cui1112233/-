...
test('script workbench restores and synchronously persists tab-isolated drafts', () => {
  ...
  assert.match(page, /const \[draftTabReady, setDraftTabReady\] = useState\(false\)/);
  assert.match(page, /setTabId: nextTabId => \{\s*setScriptDraftTabId\(window\.sessionStorage, nextTabId\);\s*draftTabIdRef\.current = nextTabId;/);
  assert.match(page, /coordinator\.ready\.then\(\(\{ tabId \}\) => \{/);
  assert.match(page, /draftTabIdRef\.current = tabId;\s*const restoredDraft = loadScriptDraft\(window\.localStorage, draftUsernameRef\.current, draftTabIdRef\.current\)/);
  ...
  assert.match(page, /if \(!draftReadyRef\.current \|\| !draftTabReady\) return;/);
  assert.match(page, /coordinator\.cleanup\(\)/);
  assert.match(page, /const restoredDraft = loadScriptDraft/);
  assert.match(page, /readyTimer = window\.setTimeout/);
