(() => {
  ...
  function refreshLegacyTasks() { byId('taskRefreshBtn')?.click(); }
  ...
  async function createSchedule() {
    try {
      ...
      setText('v78ScheduleStatus', '定时任务已保存'); await loadSchedules();
    } catch (error) { setText('v78ScheduleStatus', error.message); }
  }
  async function loadSchedules() {
    try {
      const data = await v2Api('/schedules');
      const box = byId('v78ScheduleList'); if (!box) return;
      box.innerHTML = '';
      for (const item of (data.schedules || [])) {
        const row = document.createElement('div'); row.className = 'v78-v2-row';
        const text = document.createElement('span'); text.textContent = `${new Date(item.runAt).toLocaleString()} · ${item.status || 'scheduled'}`;
        ...
        row.append(text, actions); box.appendChild(row);
      }
  ...
  function mountAdvancedTaskPanel() {
    ...
    panel.innerHTML = `
      ...
      <div id="v78AdvancedTaskResults" class="v78-v2-list"></div>`;
    host.insertBefore(panel, listDetails);
    byId('v78TaskSearch').onclick = loadAdvancedTasks;
    byId('v78TaskReset').onclick = () => { byId('v78TaskDate').value = ''; byId('v78BookIdSearch').value = ''; byId('v78TaskStatus').value = ''; loadAdvancedTasks(); };
  ...
  function shiftTaskDate(delta) {
    ...
    date.setDate(date.getDate() + delta); byId('v78TaskDate').value = localDateKey(date); loadAdvancedTasks();
  }
  async function loadAdvancedTasks() {
    try {
      const book = String(value('v78BookIdSearch')).trim(); const status = String(value('v78TaskStatus')).trim(); const date = String(value('v78TaskDate')).trim();
      const parts = [];
      if (book) parts.push(`bookId=${encodeURIComponent(book)}`);
      if (status) parts.push(`status=${encodeURIComponent(status)}`);
      if (date) parts.push(`date=${encodeURIComponent(date)}`);
      const data = await v2Api(`/tasks${parts.length ? `?${parts.join('&')}` : ''}`);
      const box = byId('v78AdvancedTaskResults'); if (!box) return; box.innerHTML = '';
      for (const task of (data.tasks || [])) {
        const row = document.createElement('div'); row.className = 'v78-v2-row';
        const id = task.book_id || task.bookId || task.id || ''; const name = task.book_name || task.bookName || '';
        const state = task.status || task.original_status || task.ai_status || task.classify_status || '待处理';
        row.textContent = `${id} · ${name || '未命名'} · ${state}`; box.appendChild(row);
      }
      if (!box.childNodes.length) box.textContent = '没有符合条件的任务';
      setText('v78TaskOpsStatus', `查询完成：${(data.tasks || []).length} 条`);
    } catch (error) { setText('v78TaskOpsStatus', error.message); }
  }
  async function applyAiCount() {
    try {
      const ids = selectedTaskIds(); if (!ids.length) throw new Error('请先在原任务列表勾选任务');
      ...
      setText('v78TaskOpsStatus', `已更新 ${result.updated || 0} 个任务`); refreshLegacyTasks(); await loadAdvancedTasks();
    } catch (error) { setText('v78TaskOpsStatus', error.message); }
  }
  async function permanentDeleteSelected() {
    try {
      ...
      setText('v78TaskOpsStatus', `已永久删除 ${result.deleted || ids.length} 个任务`); refreshLegacyTasks(); await loadAdvancedTasks();
    } catch (error) { setText('v78TaskOpsStatus', error.message); }
  }
  async function restoreTombstone() {
    try {
      const id = String(value('v78RestoreBookId')).trim(); if (!id) throw new Error('请输入要恢复的书籍ID');
  ...
  function boot() {
    injectStyles(); enforceFixedPlatformUi(); mountAutomationPanel(); mountAdvancedTaskPanel();
    let attempts = 0;
    const mountTimer = window.setInterval(() => {
      enforceFixedPlatformUi(); mountAutomationPanel(); mountAdvancedTaskPanel(); attempts += 1;
      if (attempts >= 20) window.clearInterval(mountTimer);
    }, 250);
    loadRealtimeStatus(); loadSchedules(); loadAdvancedTasks();
    if (!realtimeTimer) realtimeTimer = window.setInterval(loadRealtimeStatus, 2000);
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot, { once: true }); else boot();
  window.addEventListener('load', () => { enforceFixedPlatformUi(); mountAutomationPanel(); mountAdvancedTaskPanel(); }, { once: true });
})();
