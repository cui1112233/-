...
func FromEnv() (Config, error) {
	...
	if cfg.Slice >= 6 {
		if cfg.External121Enabled && (cfg.External121Endpoint == "" || cfg.External121APIKey == "") { return Config{}, fmt.Errorf("Slice 6 121 requires explicit endpoint and API key") }
		if cfg.ExternalYadiEnabled && (cfg.ExternalYadiEndpoint == "" || cfg.ExternalYadiAPIKey == "") { return Config{}, fmt.Errorf("Slice 6 Yadi requires explicit endpoint and API key") }
		if cfg.External121Enabled || cfg.ExternalYadiEnabled {
			...
			if decodeErr != nil { return Config{}, decodeErr }
...
func decodeCredentialKey(raw string) ([]byte, error) {
	if raw == "" { return nil, fmt.Errorf("Slice 6 external publish requires QIANTIE_BATCH_FACTORY_V11_CREDENTIALS_KEY") }
	if decoded, err := base64.StdEncoding.DecodeString(raw); err == nil && len(decoded) == 32 { return decoded, nil }
	if len(raw) == 32 { return []byte(raw), nil }
