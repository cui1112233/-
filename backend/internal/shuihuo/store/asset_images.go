...
// AssetImages owns generated image references for an asset. These records are
// deliberately separate from storyboard media.
...
func (s *AssetImages) CreateGenerated(ctx context.Context, image domain.AssetImage) (domain.AssetImage, error) {
	...
	image.AssetNameSnapshot = asset.Name
	image.AssetCategorySnapshot = asset.Category
	image.AssetPromptSnapshot = asset.Prompt

	result, err := tx.ExecContext(ctx, `
INSERT INTO shuihuo_asset_images(project_id, asset_id, task_id, object_key, is_primary, asset_name_snapshot, asset_category_snapshot, asset_prompt_snapshot)
VALUES(?, ?, ?, ?, ?, ?, ?, ?)
`, image.ProjectID, image.AssetID, image.TaskID, image.ObjectKey, image.IsPrimary, image.AssetNameSnapshot, image.AssetCategorySnapshot, image.AssetPromptSnapshot)
	if err != nil {
		return domain.AssetImage{}, err
	}
...
`, imageID, ownerID).Scan(&image.ID, &image.ProjectID, &image.AssetID, &image.TaskID, &image.ObjectKey, &image.IsPrimary, &image.CreatedAt, &image.UpdatedAt)
	return image, err
}
...
WHERE i.id = ? AND p.user_id = ?
FOR UPDATE
`, imageID, ownerID).Scan(&assetID); err != nil {
		return domain.AssetImage{}, err
	}
...
DELETE i FROM shuihuo_asset_images i
JOIN shuihuo_projects p ON p.id = i.project_id
WHERE i.id = ? AND p.user_id = ?
`, imageID, ownerID)
	if err != nil {
