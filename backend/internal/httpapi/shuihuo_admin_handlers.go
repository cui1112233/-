...
func (api *API) handleCreateAdminModel(w http.ResponseWriter, r *http.Request) {
	...
	definition := models.Definition{
		ModelID: strings.TrimSpace(req.ModelID), Name: strings.TrimSpace(req.Name), Kind: req.Kind, AdapterKind: strings.TrimSpace(req.AdapterKind),
		Enabled: req.Enabled, Hidden: req.Hidden, SortOrder: req.SortOrder, AdminNote: strings.TrimSpace(req.AdminNote), AllowedRoles: req.AllowedRoles,
	...
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "模型配置无效或 modelId/名称重复"})
		return
	}
