...
func TestSliceTwoBatchDirectorRunsOnlySelectedBooks(t *testing.T) {
	now := time.Unix(1700000000, 0)
	store := batchfactoryv11.NewMemoryStore()
	batch, _ := store.CreateBatch(context.Background(), "alice", batchfactoryv11.CreateBatchInput{Title: "b", Books: []batchfactoryv11.CreateBookInput{
		{Title: "k1", SourceText: "第一本原文。"},
		{Title: "k2", SourceText: "第二本原文。"},
	}})
	provider := &directorHTTPProvider{output: directorHTTPJSON}
	service := &batchfactoryv11.DirectorService{Store: store, Provider: provider}
	api := NewRouter(RouterOptions{BridgeSecret: "secret", Now: func() time.Time { return now }, Slice: 2, Store: store, Director: service})
	path := "/api/batch-factory/v11/batches/" + batch.ID + "/director"
	rec := signedJSONRequest(t, api, now, "alice", http.MethodPost, path, map[string]any{"bookIds": []string{batch.Books[1].ID}})
	...
	if loaded.Books[0].DirectorRevision != nil || loaded.Books[1].DirectorRevision == nil {
		t.Fatalf("unexpected revisions: book1=%+v book2=%+v", loaded.Books[0].DirectorRevision, loaded.Books[1].DirectorRevision)
	}
}
