...
func hydrateMemoryBatchSettingsState(b Batch, patches map[string]SettingsPatch, hooks map[string][]HookRevision, directors map[string][]DirectorRevision) Batch {
	...
	for i := range b.Books {
		...
		if revisions := directors[bookKey]; len(revisions) > 0 {
			...
			book.DownstreamStale = latest.SourceDigest != sourceDigest(book.SourceText)
		}
...
func (s *MemoryStore) UpdateBookSource(_ context.Context, owner, batchID, bookID string, input SourceUpdate) (Book, error) {
	...
	for index := range batch.Books {
		...
		book.SourceText = text
		book.Revision++
		batch.UpdatedAt = time.Now().UTC()
		s.batches[batchID] = memoryOwned[Batch]{Owner: owner, Value: batch}
		updated := hydrateMemoryBatchSettingsState(batch, s.patches, s.hooks, s.directors).Books[index]
		return updated, nil
	}
	return Book{}, ErrNotFound
}
