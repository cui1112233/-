...
func (s *ProductionService) SubmitBatchProductionWithProvider(ctx context.Context, owner, batchID, requestID, provider string) (BatchStatus, error) {
	return s.SubmitBatchProductionForBooks(ctx, owner, batchID, requestID, provider, nil)
}

// SubmitBatchProductionForBooks submits the same single-book production unit
// for the requested books. A nil or empty selection preserves the legacy
// all-books behavior; a non-empty selection is validated before any provider
// task is created so a typo cannot produce a partial, surprising run.
func (s *ProductionService) SubmitBatchProductionForBooks(ctx context.Context, owner, batchID, requestID, provider string, bookIDs []string) (BatchStatus, error) {
	if s == nil || !s.Enabled {
	...
	if len(selected) > 0 {
		for bookID := range selected {
			if _, err := bookFromBatch(batch, bookID); err != nil {
				return BatchStatus{}, err
			}
		}
	}
