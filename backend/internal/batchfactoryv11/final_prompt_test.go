...
func TestFinalPromptUsesServerPromptCatalogAndContentWindow(t *testing.T) {
	store, batch, book, video := seedCompiledVideo(t)
	prompt, err := (&PromptCompilerService{Store: store, Catalog: NewPromptCatalog(store)}).Compile(context.Background(), "alice", batch.ID, book.ID, video.ID)
	if err != nil {
		t.Fatal(err)
	}
	for _, expected := range []string{"你是短剧编剧。根据内容窗口", "人物、场景、道具必须分别输出", "每个视频片段必须给出可拍的动作", "原文内容窗口：她被当众羞辱后沉默离开。"} {
		if !strings.Contains(prompt.CompiledPrompt, expected) {
			t.Fatalf("missing server-owned prompt component %q in\n%s", expected, prompt.CompiledPrompt)
		}
	}
	if prompt.EffectiveSettings.SourceByField["scriptPromptPresetId"] != "system" || prompt.EffectiveSettings.SourceByField["videoPromptPresetId"] != "system" {
		t.Fatalf("prompt sources=%+v", prompt.EffectiveSettings.SourceByField)
	}
}
