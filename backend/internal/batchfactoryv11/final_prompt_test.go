...
func seedCompiledVideo(t *testing.T) (*MemoryStore, Batch, Book, Video) {
	...
	if _, err := (&DirectorService{Store:store, Provider:provider}).RunDirector(context.Background(), "alice", batch.ID, book.ID); err != nil { t.Fatal(err) }
	batch, _ = store.GetBatch(context.Background(), "alice", batch.ID)
	book = batch.Books[0]
	video := book.Videos[0]
...
func TestEffectiveSettingsResolveSystemBatchBookVideoWithSources(t *testing.T) {
	...
	if got := rawString(value.Values, "aspectRatio", ""); got != "16:9" || value.SourceByField["aspectRatio"] != "book" { t.Fatalf("aspect=%q sources=%+v", got, value.SourceByField) }
	if got := rawInt(value.Values, "duration", 0); got != 8 || value.SourceByField["duration"] != "video" { t.Fatalf("duration=%d sources=%+v", got, value.SourceByField) }
	if value.SourceByField["productionMode"] != "batch" || value.SourceByField["subtitlePolicy"] != "system" { t.Fatalf("sources=%+v", value.SourceByField) }
	if value.DirectorRevisionID == "" || len(value.SnapshotHash) != 64 { t.Fatalf("effective=%+v", value) }
}
...
func TestFinalPromptRejectsOrphanedVideoIdentity(t *testing.T) {
	...
	provider := &queuedDirectorProvider{values:[]string{validDirectorJSON()}}
	if _, err := (&DirectorService{Store:store, Provider:provider}).RunDirector(context.Background(), "alice", batch.ID, book.ID); err != nil { t.Fatal(err) }
	if _, err := (&PromptCompilerService{Store:store}).Compile(context.Background(), "alice", batch.ID, book.ID, oldVideo.ID); err != ErrNotFound { t.Fatalf("expected ErrNotFound for orphaned VIDEO, got %v", err) }
}
