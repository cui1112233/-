...
func loadProductionJob(ctx context.Context, q productionQueryer, owner, id string) (ProductionJob, error) {
	...
	rows, err := q.QueryContext(ctx, `SELECT id,video_id,status,attempt,final_prompt_hash,compiled_prompt,COALESCE(provider_task_id,''),COALESCE(media_url,''),COALESCE(error_message,''),created_at,updated_at FROM batch_factory_v11_production_tasks WHERE job_id=? AND owner_username=? ORDER BY created_at,id`, id, owner)
	if err != nil {
		return ProductionJob{}, err
	}
	defer rows.Close()
	for rows.Next() {
		...
		if err := rows.Scan(&task.ID, &task.VideoID, &taskState, &task.Attempt, &task.FinalPromptHash, &task.CompiledPrompt, &task.ProviderTaskID, &task.MediaURL, &task.ErrorMessage, &task.CreatedAt, &task.UpdatedAt); err != nil {
			return ProductionJob{}, err
		}
...
func (s *MySQLStore) CreateProductionJob(ctx context.Context, value ProductionJob) (ProductionJob, error) {
	...
	for index := range value.Tasks {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO batch_factory_v11_production_tasks(id,job_id,owner_username,video_id,attempt,final_prompt_hash,compiled_prompt,provider_task_id,media_url,status,error_message,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`, task.ID, value.ID, value.Owner, task.VideoID, task.Attempt, task.FinalPromptHash, task.CompiledPrompt, nullableString(task.ProviderTaskID), nullableString(task.MediaURL), task.Status, nullableString(task.ErrorMessage), now, now); err != nil {
			return ProductionJob{}, err
		}
