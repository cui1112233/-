...
func (s *DirectorService) RunHook(ctx context.Context, owner, batchID, bookID string) (HookRevision, error) {
	...
	return s.Store.CreateHookRevision(ctx, owner, batchID, bookID, strings.TrimSpace(text), sourceDigest(sourceForPrompt(book)))
}
...
func (s *DirectorService) RunDirector(ctx context.Context, owner, batchID, bookID string) (DirectorRevision, error) {
	...
	return s.Store.PersistDirectorRevision(ctx, owner, book, snapshot, sourceDigest(sourceForPrompt(book)), hook.ID, result)
}
