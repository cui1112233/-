...
func TestHTTPVideoAdapterSubmitsFrozenPromptAndReadsTaskID(t *testing.T) {
	...
	prompt := FinalPrompt{CompiledPrompt: "人物：林晚\n画幅 9:16；时长 8 秒", EffectiveSettings: EffectiveSettings{Values: SettingsPatch{
		"duration": json.RawMessage(`8`), "imageAspectRatio": json.RawMessage(`"1:1"`), "videoAspectRatio": json.RawMessage(`"16:9"`), "videoResolution": json.RawMessage(`"1080p"`),
	}}}
	ref, err := adapter.Submit(context.Background(), FrozenVideoModel{ID: "video-v1", MaxDuration: 15}, prompt)
	if err != nil {
		t.Fatal(err)
	}
	if ref.ProviderTaskID != "task-1" || ref.State != ProductionQueued {
		t.Fatalf("ref=%+v", ref)
	}
	if received["model"] != "video-v1" || received["prompt"] != prompt.CompiledPrompt || received["duration"] != float64(8) || received["aspectRatio"] != "16:9" || received["resolution"] != "1080p" {
...
func TestHTTPVideoAdapterPollsCompletedMedia(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet || !strings.HasSuffix(r.URL.Path, "/task-1") {
	...
	if ref.State != ProductionSucceeded || ref.MediaURL != "https://media.example/video.mp4" {
		t.Fatalf("ref=%+v", ref)
	}
}

func TestHTTPVideoAdapterRequiresHTTPSAndCredentials(t *testing.T) {
	if err := (&HTTPVideoAdapter{Endpoint: "http://example.com", APIKey: "secret", Model: "video"}).Validate(); err == nil {
		t.Fatal("expected HTTPS validation")
	}
	if err := (&HTTPVideoAdapter{Endpoint: "https://example.com", Model: "video"}).Validate(); err == nil {
		t.Fatal("expected API key validation")
	}
	if err := (&HTTPVideoAdapter{Endpoint: "https://example.com", APIKey: "secret"}).Validate(); err == nil {
		t.Fatal("expected model validation")
	}
}
