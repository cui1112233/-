...
func applyLayer(values SettingsPatch, sources map[string]string, layer SettingsPatch, source string) {
	...
	for key := range layer { keys = append(keys, key) }
	sort.Strings(keys)
...
func configPatch(raw json.RawMessage) SettingsPatch {
	...
	if json.Unmarshal(raw, &direct) != nil { return SettingsPatch{} }
	if nested, ok := direct["settings"]; ok {
		var settings SettingsPatch
		if json.Unmarshal(nested, &settings) == nil { return settings }
	}
...
func rawStrings(patch SettingsPatch, key string) []string {
	var values []string
	if raw, ok := patch[key]; ok { _ = json.Unmarshal(raw, &values) }
	return values
}

func videoFromBook(book Book, videoID string) (Video, int, error) {
	for index, video := range book.Videos {
		if video.ID == videoID && video.CompatibilityState == "active" { return video, index, nil }
	}
...
func (s *PromptCompilerService) ResolveEffective(ctx context.Context, owner, batchID, bookID, videoID string) (EffectiveSettings, Book, Video, int, error) {
	if s == nil || s.Store == nil { return EffectiveSettings{}, Book{}, Video{}, -1, ErrUnavailable }
	batch, err := s.Store.GetBatch(ctx, owner, batchID)
	if err != nil { return EffectiveSettings{}, Book{}, Video{}, -1, err }
	...
	if versionID != "" {
		...
		if listErr != nil { return EffectiveSettings{}, Book{}, Video{}, -1, listErr }
		for _, version := range versions {
			if version.ID == versionID { applyLayer(values, sources, configPatch(version.Config), "config-version:"+version.ID); break }
		}
	}
	...
	directorID := ""
	if book.DirectorRevision != nil { directorID = book.DirectorRevision.ID }
...
func namedPromptMap(values []NamedPrompt) map[string]string {
	...
	for _, value := range values { out[value.Name] = value.Prompt }
	return out
}
...
func selectPrompts(names []string, library map[string]string) string {
	...
	for _, name := range names {
		if prompt := strings.TrimSpace(library[name]); prompt != "" { parts = append(parts, name+"："+prompt) }
	}
...
func addComponent(out *[]PromptComponent, key, label, content string) {
	...
	if content != "" { *out = append(*out, PromptComponent{Key:key, Label:label, Content:content}) }
}
...
func (s *PromptCompilerService) Compile(ctx context.Context, owner, batchID, bookID, videoID string) (FinalPrompt, error) {
	...
	for _, notice := range effective.Compatibility {
		if notice.State == "incompatible" { return FinalPrompt{}, fmt.Errorf("%w: incompatible effective settings", ErrConflict) }
	}
	draft := book.DirectorRevision.Output.Storyboard[ordinal]
	values := effective.Values
	characterRefs := rawStrings(values, "characterRefs"); if len(characterRefs) == 0 { characterRefs = draft.Characters }
	...
	if rawBool(values, "qualityEnabled", false) { addComponent(&components, "quality", "画质约束", rawString(values, "quality", "")) }
	if rawBool(values, "restrictionEnabled", false) { addComponent(&components, "restriction", "画面限制", rawString(values, "restriction", "")) }
	if rawBool(values, "negativeEnabled", false) { addComponent(&components, "negative", "负面提示词", rawString(values, "negative", "")) }
