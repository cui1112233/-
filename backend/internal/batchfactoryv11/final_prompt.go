...
func (s *PromptCompilerService) namedPromptMapWithDrafts(ctx context.Context, owner, batchID, bookID, category string, values []NamedPrompt) (map[string]string, error) {
	...
	for _, value := range values {
		...
		for _, scope := range []string{batchID + ":" + bookID, batchID} {
			draft, err := s.Store.GetDraft(ctx, owner, key, "asset-prompt", scope)
			if err != nil {
...
func (s *PromptCompilerService) Compile(ctx context.Context, owner, batchID, bookID, videoID string) (FinalPrompt, error) {
	...
	scenePrompts, err := s.namedPromptMapWithDrafts(ctx, owner, batchID, book.ID, "scene", book.Assets.Scenes)
	if err != nil {
		return FinalPrompt{}, err
	}
	propPrompts, err := s.namedPromptMapWithDrafts(ctx, owner, batchID, book.ID, "prop", book.Assets.Props)
	if err != nil {
		return FinalPrompt{}, err
	}
