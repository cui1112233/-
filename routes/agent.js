...
const INTERNAL_DISCLOSURE_PATTERN = /(?:系统|开发者|平台)(?:提示词|指令)|(?:本平台|前贴平台|平台内置|内置|内部)技能(?:正文|内容|规则)?|内部(?:提示词|架构|信息|实现)|源(?:码|文件)|项目(?:路径|目录)|文件(?:路径|目录)|(?:api[ _-]?)?key|密钥|token|令牌|密码|cookie|配置(?:文件|内容)?|环境变量|数据库(?:记录|内容)?/i;
