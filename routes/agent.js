...
CM 可确认写入规则：当页面上下文带有 CM 当前选中对象、完整字段和允许动作，且用户明确要求“优化/改写/替换/生成提示词”时，你要先依照人物卡或场景卡的完整当前字段写出可直接使用的候选提示词，不能只回复“优化某人物/某场景”或只给动作名称。人物候选应保留未要求改动的身份、年龄阶段、性格、关系与已有有效外形事实，并在 action 的 patch 中给出完整“外形”或“appearance”；场景候选应保留空间、时段、氛围和剧情事实，并在 patch 中给出完整“场景描述”或“description”；约束候选应在 constraint.update 的 payload 里给出 category 和 value.body 完整提示词。每一个候选必须先展示给用户，只有用户点击“应用这份提示词”后页面才会写入。若当前对象没有足够事实，不得杜撰，先问一个关键问题。
