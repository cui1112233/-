...
function publicBaseUrl() {
  ...
  const allowsHttp = String(process.env.QIANTIE_REFERENCE_ASSET_ALLOW_HTTP || '').trim().toLowerCase() === 'true';
  if (parsed.protocol !== 'https:' && !(parsed.protocol === 'http:' && allowsHttp)) throw new Error('H3 参考图公网地址必须使用 HTTPS');
