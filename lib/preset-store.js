...
// Preset ids may include localized labels (for example the Q版 script format)
// while retaining the same no-whitespace and bounded-length constraints.
const ID_PATTERN = /^[\p{L}\p{N}][\p{L}\p{N}._-]{0,63}$/u;
...
function publicPreset(preset) {
  return {
    ...
    slot: isIdentifier(preset.protocolLock?.slot) ? preset.protocolLock.slot : null,
...
function createPresetStore({ systemDir, lockTimeoutMs, lockRetryMs, validatePresetSlot } = {}) {
  ...
  const hasSlotValidator = typeof validatePresetSlot === 'function';
  ...
  function createDraft(actor, input = {}) {
    ...
    if (!isIdentifier(id) || !isIdentifier(module) || !isText(name, 160) || !KINDS.has(kind)
      ...
      || (hasSlotValidator && !validatePresetSlot({ id, module, kind, protocolLock }))) {
  ...
  function publish(actor, id, version) {
    ...
    return withStoreLock(() => {
      ...
      if (hasSlotValidator && !validatePresetSlot(preset)) {
        throw storeError('Preset must have a valid ownership slot before publishing', 'CONFLICT');
      }
  ...
  function rollback(actor, id, version) {
    ...
    return withStoreLock(() => {
      ...
      if (hasSlotValidator && !validatePresetSlot(preset)) {
        throw storeError('Preset must have a valid ownership slot before rollback', 'CONFLICT');
      }
      const current = publishedPreset(state, id);
  ...
  function resolveSelection({ module, presetIds } = {}) {
    ...
    return withStoreLock(() => {
      ...
      const baseSlot = bases[0].protocolLock?.slot;
      if (addons.some(addon => addon.protocolLock?.slot && addon.protocolLock.slot !== baseSlot)) {
        throw storeError('Addon must use the same ownership slot as selected base');
      }
