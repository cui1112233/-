...
const PUBLIC_ARTIFACT_PATH = /^\/api\/local-executor\/v1\/artifacts\/[^/]+$/;

function buildForwardRequest(req, targetBaseUrl) {
  const originalUrl = req.originalUrl || req.url || '';
  const parsed = new URL(originalUrl, 'http://qiantie.local');
  const isPublicArtifact = req.method === 'GET' && PUBLIC_ARTIFACT_PATH.test(parsed.pathname);
  if ((!isPublicArtifact && req.method !== 'POST') || !parsed.pathname.startsWith(PREFIX)) {
  ...
  const body = isPublicArtifact || streamBody
    ? null
...
module.exports = { PREFIX, ARTIFACT_PATH, PUBLIC_ARTIFACT_PATH, buildForwardRequest, isMp4 };
