...
const API_SCOPES = new Set(['*', 'text', 'image', 'tts']);
...
function createMemberStore({ systemDir, accountStore } = {}) {
  ...
  function validateManagerBinding(state, boundTo, subjectUsername) {
    if (!boundTo) return null;
    if (boundTo === subjectUsername) throw memberError('成员不能绑定到自己');
    const manager = ensureActiveMember(state, boundTo, '绑定的管理账号不存在或已停用');
    if (manager.role !== 'manager') throw memberError('成员只能绑定到 manager');
    return manager;
  }
  ...
  function createManagedMember(actorUsername, {
    ...
    return withLock(() => {
      ...
      if (nextRole === 'member') validateManagerBinding(state, nextBoundTo, username);
      ...
      state.profiles.push({
        ...
        displayName: normalizeDisplayName(displayName, createdAccount.username),
        avatarUrl: null,
        role: nextRole,
        boundTo: nextBoundTo,
        monthlyTokenLimit: normalizeQuota(monthlyTokenLimit),
        createdAt: now,
        updatedAt: now
      });
      ...
      if (apiEnabled && nextRole === 'member') {
        if (!nextBoundTo) throw memberError('未绑定 manager 的成员不能启用 API');
        const grant = {
          id: crypto.randomUUID(),
          at: now,
  ...
  function updateManagedMember(actorUsername, targetUsername, patch = {}) {
    return withLock(() => {
      ...
      if (nextRole === 'member') validateManagerBinding(state, nextBoundTo, target.username);
      else nextBoundTo = null;
  ...
  function setApiAccess(actorUsername, targetUsername, enabled, scope = '*') {
    ...
    return withLock(() => {
      ...
      if (enabled) validateManagerBinding(state, target.boundTo, target.username);
  ...
  function visibleTeam(actorUsername) {
    return withLock(() => {
      ...
      if (actor.role === 'manager') return all.filter(item => item.username === actor.username || (item.role === 'member' && item.boundTo === actor.username));
      return [actor];
    });
  }
  ...
  function listAudit(actorUsername, limit = 100) {
    return withLock(() => {
      ...
      if (actor.role === 'manager') {
        const subjects = new Set(visibleTeam(actor.username).map(item => item.username));
