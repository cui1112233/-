...
function createNovelFetchStore({ usersDir } = {}) {
  ...
  function setSession(username, cookie, metadata = {}) {
    assertValidUsername(username);
    const previous = readSessionRecord(username) || {};
    const sessionKey = String(cookie || '').trim();
    const targetUsername = String(metadata.targetUsername || previous.targetUsername || '').trim();
    const baseUrl = String(metadata.baseUrl || previous.baseUrl || '').trim();
    writeJsonAtomic(sessionPath(username), {
      ...previous,
      mode: 'http_cookie',
      cookie: sessionKey,
      sessionKey,
      ...
      ...(baseUrl ? { baseUrl } : {}),
      status: String(metadata.status || previous.status || 'ready'),
  ...
  function getBrowserSession(username) {
    const value = readSessionRecord(username);
    if (!value || !['browser_worker', 'http_cookie'].includes(value.mode) || typeof value.sessionKey !== 'string' || !value.sessionKey) return null;
    return value;
  }
  ...
  function setBrowserSession(username, session = {}) {
    ...
    const validatedAt = String(session.validatedAt || previous?.validatedAt || '').trim();
