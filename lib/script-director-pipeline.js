...
function validateDirectorExecution(raw, plan, { matchAudio = false, audioTotalSeconds = null } = {}) {
  ...
  const cleanShots = shots.map((shot, position) => {
    ...
    if (!Number.isInteger(sourceIndex) || sourceIndex < 1 || sourceIndex > units.length || seen.has(sourceIndex)) throw new Error('执行导演 source_index 缺失、越界或重复。');
    seen.add(sourceIndex);
    const unit = units[sourceIndex - 1];
  ...
  if (seen.size !== units.length) throw new Error('执行导演遗漏 Global Plan source_unit。');
