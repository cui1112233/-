...
const TARGET_API_LOGIN_PATH = '/tttadmin/api/login.php';
...
function buildLoginRequest(username, password) {
  return {
    method: 'POST',
    url: `http://${TARGET_HOST}${TARGET_API_LOGIN_PATH}`,
...
function requestHttp({ method = 'GET', url, headers = {}, body, timeoutMs = 30000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    // 上传与 JSON 登录请求都需显式指定 Content-Length，避免使用 chunked 传输编码（目标 PHP 接口可能不识别）
...
module.exports = {
  TARGET_HOST, TARGET_LOGIN_PATH, TARGET_API_LOGIN_PATH, TARGET_UPLOAD_PATH, TARGET_CHECK_PATH,
  PLATFORM_ID, GENDER_ID, STYLE_ID, VALID_PLATFORM_IDS, VALID_STYLE_NAMES, DEFAULT_ADVANCED,
  normalizeAdvanced, buildUploadFields, buildMultipart, buildLoginRequest, isLoginPage, isDashboard, requestHttp
};
