...
const TARGET_ORGANIZATION_PATH = '/tttadmin/api/organization.php';
...
function normalizeAdvanced(input = {}) {
  ...
  return {
    ...
    // 121's form has a fourth selection, \"自定义 AI 头部\".  It must survive
    // a settings round trip so a book's MP4 can be submitted as its own AI head.
...
function buildUploadFields({ platformId, gender, style, advanced, styleMap }) {
...
function multipartFiles(fileOrFiles) {
  ...
  if (!values.length || values.some(value => !value || !String(value.filename || '').trim())) {
    throw new Error('121 上传至少需要一个命名文件');
  }
...
function requestHttp({ method = 'GET', url, headers = {}, body, timeoutMs = 15000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      // A pre-signed 121 TOS URL is HTTPS.  Falling back to port 80 makes
      // Node start a TLS handshake against an HTTP listener (EPROTO).
...
module.exports = {
  TARGET_HOST, TARGET_LOGIN_PATH, TARGET_LOGIN_API_PATH, TARGET_UPLOAD_PATH, TARGET_CHECK_PATH, TARGET_BOOK_LIST_PATH, TARGET_CONFIG_LIST_PATH, TARGET_ORGANIZATION_PATH, TARGET_ASSET_PRESIGN_PATH,
  TARGET_CHECK_TIMEOUT_MS, PLATFORM_ID, GENDER_ID, STYLE_ID, VALID_PLATFORM_IDS, VALID_STYLE_NAMES, PER_BOOK_MATERIAL_LIMIT, DEFAULT_ADVANCED,
