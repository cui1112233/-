...
function distributeBookMaterials(advanced, versionCount) {
  ...
  return jieya.map((jieyaNum, index) => ({ ...normalized, jieyaNum, gunpingNum: gunping[index] || 0 }));
}
...
function buildTargetUploadFilename(bookId) {
  ...
  if (!TARGET_UPLOAD_FILENAME_PATTERN.test(normalized)) throw new Error('121 上传文件名只支持纯数字书号或 UUID 格式');
...
function buildLoginPageUrl() { return `http://${TARGET_HOST}${TARGET_LOGIN_PATH}`; }
...
function isLoginPage(body) { return String(body || '').includes('管理员登录'); }
function isDashboard(body) { const b = String(body || ''); return b.includes('自定义文案') && !b.includes('管理员登录'); }

function requestHttp({ method = 'GET', url, headers = {}, body, timeoutMs = 30000 } = {}) {
  return new Promise((resolve, reject) => {
    const parsed = new URL(url);
    const options = { hostname: parsed.hostname, port: parsed.port || 80, path: parsed.pathname + parsed.search, method, headers: { ...headers } };
...
module.exports = {
  ...
  normalizeAdvanced, normalizeBookAdvanced, distributeBookMaterials, buildUploadFields, buildMultipart, buildTargetUploadFilename, buildTargetBookListUrl, buildLoginPageUrl, buildLoginRequest, cookieHeaderFromSetCookie, mergeCookieHeaders, isLoginPage, isDashboard, requestHttp
};
