...
const TARGET_BOOK_LIST_PATH = '/tttadmin/api/zbooklist_get.php';
const TARGET_CONFIG_LIST_PATH = '/tttadmin/api/zdy_config.php?action=list';
const TARGET_ORGANIZATION_LIST_PATH = '/tttadmin/api/organization.php';
const TARGET_ASSET_PRESIGN_PATH = '/tttadmin/api/music_put_url.php';
const TARGET_CHECK_TIMEOUT_MS = 30_000;
const TARGET_UPLOAD_FILENAME_PATTERN = /^(?:\d+|[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i;
...
function normalizeAdvanced(input = {}) {
  ...
  return {
    ...
    jieyaAiHead: [0, 1, 2, 3].includes(Number(input.jieyaAiHead)) ? Number(input.jieyaAiHead) : 0,
...
function normalizeBookAdvanced(input = {}) {
  const normalized = normalizeAdvanced(input);
  const jieyaNum = Math.round(clamp(input.jieyaNum, 0, PER_BOOK_MATERIAL_LIMIT, normalized.jieyaNum));
  return { ...normalized, jieyaNum, gunpingNum: PER_BOOK_MATERIAL_LIMIT - jieyaNum };
}
...
function distributeBookMaterials(advanced, versionCount) {
  const normalized = normalizeBookAdvanced(advanced);
...
function buildUploadFields({ platformId, gender, style, organization = DEFAULT_ORGANIZATION_ID, advanced, styleMap }) {
  ...
  const styleId = Number(styleMap?.[style] ?? STYLE_ID[style]);
  if (!Number.isFinite(styleId)) throw new Error('无效的风格');
  const organizationId = String(organization || DEFAULT_ORGANIZATION_ID).trim();
  if (!/^\d+$/.test(organizationId) || Number(organizationId) <= 0) throw new Error('无效的组织归属');
  return {
...
function multipartFiles(fileOrFiles) {
  const values = Array.isArray(fileOrFiles) ? fileOrFiles : [fileOrFiles];
  if (!values.length || values.some(value => !value || !String(value.filename || '').trim())) throw new Error('121 上传至少需要一个命名文件');
  return values.map(value => ({
    field: String(value.field || 'files[]').trim(),
    filename: String(value.filename).trim(),
    content: Buffer.isBuffer(value.content) ? value.content : Buffer.from(String(value.content || ''), 'utf8'),
    contentType: String(value.contentType || (String(value.filename).toLowerCase().endsWith('.mp4') ? 'video/mp4' : 'text/plain; charset=utf-8')).trim()
  }));
}

function buildMultipart(fields, fileOrFiles) {
  const boundary = `----qiantie${Date.now().toString(36)}${Math.random().toString(36).slice(2, 10)}`;
  const chunks = [];
  for (const [key, value] of Object.entries(fields)) {
    chunks.push(Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="${key}"\r\n\r\n${value}\r\n`));
  }
  for (const file of multipartFiles(fileOrFiles)) {
...
function md5Buffer(value) {
  return crypto.createHash('md5').update(Buffer.isBuffer(value) ? value : Buffer.from(value || [])).digest('hex');
}

// 121 的自定义文案上传接口会从文件名识别书号；不能把 AI 版本等后缀混进文件名。
function buildTargetUploadFilename(bookId) {
  const normalized = String(bookId || '').trim();
  if (!TARGET_UPLOAD_FILENAME_PATTERN.test(normalized)) {
    throw new Error('121 上传文件名只支持纯数字书号或 UUID 格式');
  }
  return `${normalized}.txt`;
}

function buildTargetBookListUrl(bookId) {
  const filename = buildTargetUploadFilename(bookId);
  const params = new URLSearchParams({ bookid: filename.replace(/\.txt$/i, ''), page: '1', pageSize: '10' });
  return `http://${TARGET_HOST}${TARGET_BOOK_LIST_PATH}?${params.toString()}`;
}
...
function requestHttp({ method = 'GET', url, headers = {}, body, timeoutMs = 15000 } = {}) {
  return new Promise((resolve, reject) => {
    const parsed = new URL(url);
    const requestedTimeoutMs = Number(timeoutMs);
    const fallbackTimeoutMs = Number.isFinite(requestedTimeoutMs) && requestedTimeoutMs > 0 ? requestedTimeoutMs : 15000;
    const effectiveTimeoutMs = parsed.pathname === TARGET_CHECK_PATH
      ? Math.max(TARGET_CHECK_TIMEOUT_MS, fallbackTimeoutMs)
      : fallbackTimeoutMs;
    const options = {
    ...
    if (!transport) { reject(new Error('unsupported request protocol')); return; }
    ...
    req.on('error', reject);
    req.setTimeout(effectiveTimeoutMs, () => req.destroy(new Error('请求超时')));
...
module.exports = {
  TARGET_HOST, TARGET_LOGIN_PATH, TARGET_LOGIN_API_PATH, TARGET_UPLOAD_PATH, TARGET_CHECK_PATH, TARGET_BOOK_LIST_PATH, TARGET_CONFIG_LIST_PATH, TARGET_ORGANIZATION_LIST_PATH, TARGET_ASSET_PRESIGN_PATH,
  TARGET_CHECK_TIMEOUT_MS, PLATFORM_ID, GENDER_ID, STYLE_ID, VALID_PLATFORM_IDS, VALID_STYLE_NAMES, PER_BOOK_MATERIAL_LIMIT, DEFAULT_ADVANCED, DEFAULT_ORGANIZATION_ID,
  normalizeAdvanced, normalizeBookAdvanced, distributeBookMaterials, buildUploadFields, multipartFiles, buildMultipart, md5Buffer, buildTargetUploadFilename, buildTargetBookListUrl, buildLoginUrl, buildLoginPageUrl, buildLoginRequest, cookieHeaderFromSetCookie, mergeCookieHeaders, isLoginPage, isDashboard, requestHttp
};
