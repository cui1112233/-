...
function buildUploadFields({ platformId, gender, style, advanced, styleMap, organization }) {
  ...
  if (organization !== undefined) {
    const fieldName = String(organization?.field_name || organization?.fieldName || '').trim();
    const id = String(organization?.id ?? organization?.value ?? '').trim();
    if (!/^[a-zA-Z][a-zA-Z0-9_\[\]-]*$/.test(fieldName) || !id) throw new Error('无效的组织归属');
    fields[fieldName] = id;
  }
  return fields;
}
...
module.exports = {
  TARGET_HOST, TARGET_LOGIN_PATH, TARGET_LOGIN_API_PATH, TARGET_UPLOAD_PATH, TARGET_CHECK_PATH, TARGET_ORGANIZATION_PATH, TARGET_BOOK_LIST_PATH, TARGET_CONFIG_LIST_PATH,
