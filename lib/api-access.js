...
function resolveTeamAuthorization({ accountStore, memberStore, usageStore, username, scope = 'text' } = {}) {
  if (!memberStore || !username) throw accessError('无法解析 API 权限', 'API_ACCESS_INVALID', 500);
  const account = accountStore?.getInternalAccount?.(username);
  const owner = account?.isOwner === true;
  const member = memberStore.getMember(username) || (owner ? { username, active: true, role: 'manager' } : null);
  if (!member || !member.active) throw accessError('账号不可用', 'ACCOUNT_INACTIVE', 403);

  let billedTo = username;
  let teamOwner = owner || ['dev', 'manager'].includes(member.role) ? username : null;
...
function resolveApiAccess({ accountStore, memberStore, usageStore, username, scope = 'text', configReader = readConfig } = {}) {
  ...
  const authorization = resolveTeamAuthorization({ accountStore, memberStore, usageStore, username, scope });
  const { member, billedTo } = authorization;
  const owner = accountStore.getInternalAccount?.(username)?.isOwner === true;
  ...
  try {
    if (scope === 'image') {
      ...
      if (!String(config?.video?.apiKey || '').trim()) throw new Error('视频生成 API Key 未配置');
    } else {
...
function resolveModelAccess({ accountStore, memberStore, username, kind, modelId, configReader = readConfig } = {}) {
  return resolveRuntimeModel({ username, kind, modelId, memberStore, accountStore, configReader });
}
...
module.exports = { resolveApiAccess, resolveTeamAuthorization, resolveModelAccess, recordUsageFromPayload };
