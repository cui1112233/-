...
function createAgentStore({
  ...
  function readLegacyMessages(username) {
    ...
    try {
      ...
      return Array.isArray(messages) ? messages.filter(isValidMessage).slice(-maxEntries) : [];
    } catch (error) {
      return [];
    }
  }
  ...
  function readDocument(username) {
    ...
    try {
      ...
      if (
        ...
        return {
          version: TASK_DOCUMENT_VERSION,
          tasks: document.tasks.slice(0, maxTasks).map(task => ({
            ...cloneTask(task),
            messages: task.messages.slice(-maxEntries).map(cloneMessage)
          }))
        };
      }
    } catch (error) {
  ...
  function writeDocument(username, document) {
    ...
    const suffixSource = document.tasks.at(-1)?.id || 'document';
    const suffix = Buffer.from(String(suffixSource)).toString('hex');
    const temporary = path.join(directory, `.agent-tasks.${suffix}.tmp`);
  ...
  function createTask(username) {
    const document = readDocument(username);
    if (document.tasks.length >= maxTasks) throw new Error('任务数量已达上限');
    const timestamp = now();
    const task = {
      id: id(),
  ...
  function append(username, taskId, message) {
    ...
    task.messages = [...task.messages, entry].slice(-maxEntries);
