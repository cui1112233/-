...
function readReleaseInfo({ env = process.env, rootDir = path.resolve(__dirname, '..') } = {}) {
  ...
  // A frontend-only release may advance Node without recreating the paired Go
  // service. Prefer the Node-specific immutable identity when it is present.
  let gitSha = clean(env.QIANTIE_NODE_RELEASE_SHA)
    || clean(env.QIANTIE_RELEASE_SHA)
    || clean(releaseEnv.QIANTIE_NODE_RELEASE_SHA)
    || clean(releaseEnv.QIANTIE_RELEASE_SHA);
  if (!gitSha) {
