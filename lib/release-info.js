...
function loadReleaseInfo(env = process.env, filePath = DEFAULT_RELEASE_INFO_PATH) {
  ...
  if (suppliedChannel === 'development') {
    ...
    const candidate = { ...development, ...merged, release_channel: 'development' };
    if (candidate.compatibility_components === undefined) candidate.compatibility_components = DEFAULT_COMPATIBILITY_COMPONENTS;
    if (candidate.app_version !== development.app_version || candidate.build_id !== development.build_id) {
      return validateIdentity({ ...candidate, git_revision: candidate.git_revision || '0'.repeat(40), image_digest: candidate.image_digest || `sha256:${'0'.repeat(64)}` });
    }
    return developmentIdentity(candidate.compatibility_components);
  }
