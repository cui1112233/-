...
test('smart unified style accepts the H3 style.system result and renders genre, style, and era first', () => {
  const result = normalizeSmartUnifiedVisualStyle({
    final_genre: '现代都市反转短剧',
    genre: '现代都市反转短剧',
    trailer_style: '高级数字电影质感，真实肤色、柔和高光与克制冷暖对比。',
    story_era: '当代都市职场世界',
    negative_prompt: '避免人物畸形与身份漂移。',
    picture_limit_prompt: '对白只属于声音层，不生成字幕。',
    quality_constraint_prompt: '真实皮肤、稳定五官、清晰光影。'
  });
  assert.match(result.prompt, /^现代都市反转短剧；高级数字电影质感，真实肤色、柔和高光与克制冷暖对比。；当代都市职场世界。/);
  assert.equal(result.fields.final_genre, '现代都市反转短剧');
  assert.equal(result.fields.story_era, '当代都市职场世界');
});

test('H3 style.system preserves a detailed full-film trailer style beyond the legacy field limit', () => {
  const trailerStyle = `电影级古装短剧摄影基线，${'保留自然肤色、丝绸纹理、烛火层次与可读暗部。'.repeat(38)}`;
  const result = normalizeSmartUnifiedVisualStyle({
    final_genre: '古装宅斗短剧', genre: '古装宅斗短剧', trailer_style: trailerStyle, story_era: '架空古代王府',
    negative_prompt: '无畸形', picture_limit_prompt: '无字幕', quality_constraint_prompt: '真实材质'
  });
  assert.equal(result.fields.trailer_style, trailerStyle);
});

test('H3 style.system accepts full-film optics and acquisition specifications in trailer_style', () => {
  const trailerStyle = [
    '全片数字电影摄影基线，Super 35 传感器与 Log 色彩流程，24fps、180° 快门、EI 800。',
    '球面电影镜头组，主焦段以 35mm、50mm、85mm 为全片叙事基线，T2.0–T2.8，柔和高光滚降与自然焦外。',
    '该字段仅定义全片摄影规格，不定义任何单镜动作、场景、人物站位或具体构图。'
  ].join('');

  const result = normalizeSmartUnifiedVisualStyle({
    final_genre: '现代都市短剧', genre: '现代都市短剧', trailer_style: trailerStyle, story_era: '当代都市',
    negative_prompt: '无畸形', picture_limit_prompt: '无字幕', quality_constraint_prompt: '真实材质'
  });
...
test('H3 trailer_style still rejects a concrete single-shot instruction', () => {
  assert.throws(
    () => normalizeSmartUnifiedVisualStyle({
      final_genre: '现代都市短剧', genre: '现代都市短剧',
      trailer_style: '镜头 1：50mm 特写推进女主角表情。',
      story_era: '当代都市', negative_prompt: '无畸形', picture_limit_prompt: '无字幕', quality_constraint_prompt: '真实材质'
    }),
    /单镜|焦段|时间码|统一视觉/
  );
});
