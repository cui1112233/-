...
function normalizeQuota(value) {
  ...
  if (!Number.isSafeInteger(number) || number < 0 || number > 10_000_000_000) {
    throw collaborationError('成员月度 Token 额度不合法');
  }
  return number;
}
...
function createTeamCollaborationStore({ systemDir } = {}) {
  ...
  function emptyState() {
    return { teams: [], invites: [], notifications: [], archives: [] };
  }
  ...
  function getTeamByManager(managerUsername) {
    return withJsonLock(lockPath, () => {
      const team = readUnsafe().teams.find(item => item.managerUsername === managerUsername);
  ...
  function listTeams() {
    return withJsonLock(lockPath, () => readUnsafe().teams.map(item => ({ ...item })));
  }
  ...
  function inviteView(state, invite) {
    ...
    return {
      ...
      managerUsername: invite.managerUsername,
      createdAt: invite.createdAt,
      expiresAt: invite.expiresAt,
      monthlyTokenLimit: invite.monthlyTokenLimit,
      apiScopes: [...(invite.apiScopes || [])],
      usedAt: invite.usedAt,
      usedBy: invite.usedBy,
      expired: Date.parse(invite.expiresAt) <= Date.now(),
      available: !invite.usedAt && Date.parse(invite.expiresAt) > Date.now()
    };
  }
  ...
  function listInvitesForManager(managerUsername) {
    return withJsonLock(lockPath, () => {
      ...
      return state.invites
        .filter(item => item.managerUsername === managerUsername)
        .map(item => inviteView(state, item))
  ...
  function listNotifications(username, limit = 50) {
    const count = Math.min(Math.max(Number.parseInt(limit, 10) || 50, 1), 100);
    return withJsonLock(lockPath, () => readUnsafe().notifications
