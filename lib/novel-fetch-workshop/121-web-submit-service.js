...
function normalizeProfileBindings(value) {
  ...
  return Object.fromEntries(['original', 'ai1', 'ai2', 'ai3'].map(version => [version, text(source[version])]));
}
...
function targetReceipt(data) {
  ...
  const successCount = Math.max(0, Number(success.count) || (Array.isArray(success.files) ? success.files.length : 0));
  ...
  if (Object.keys(result).length && successCount < 1) throw new Error('121 未确认接收到上传文件');
  return { verified: successCount > 0 || Object.keys(result).length === 0, upload_success_count: successCount, upload_failed_count: failedCount, message: text(data?.message || data?.msg) };
}
...
function create121WebSubmitService({
  ...
  async function workerAction(owner, action, payload = {}) {
    ...
    const response = await browserClient.action({ ...ready.request, action, payload });
    return response;
  }
  ...
  async function saveConfig(owner, incoming = {}) {
    ...
    return { ok: true, settings: publicWebSubmit(web) };
  }
  ...
  async function submit(owner, body = {}) {
    ...
    for (const group of plan.groups) {
      ...
      for (const item of group.items) {
        ...
        try {
          ...
          let remoteRecord = { status: '待确认', found: false, detail: '121 列表尚未核验' };
          try {
            const lookup = parseJsonBody(await workerAction(owner, 'book_list', { bookId: item.id, page: 1, pageSize: 10 }), '121 列表核验');
            const record = Array.isArray(lookup.data) ? lookup.data.find(row => text(object(row).bookid) === text(item.id)) : null;
            remoteRecord = record ? { status: '完成', found: true, remote_id: text(record.id), remote_time: text(record.addtime), detail: '121 后台已找到对应书号记录' } : remoteRecord;
          } catch (error) { remoteRecord = { status: '待确认', found: false, detail: error?.message || '121 后台记录核验失败' }; }
          ...
          if (verified) {
            ...
            pending = true;
            await plan.store.updateTaskMeta(owner, item.id, { siteSubmitStatus: 'accepted_pending', siteSubmitAcceptedVersions: [...new Set([...(task.meta.siteSubmitAcceptedVersions || []), item.version])] });
            item.status = 'accepted_pending';
          }
          item.remote_receipt = { ...receipt, remote_record: remoteRecord };
          if (typeof plan.store.appendSiteSubmitLog === 'function') await plan.store.appendSiteSubmitLog(owner, item.id, { status: item.status, version: item.version, group_id: group.group_id, material_allocation: item.advanced, retries: Math.max(0, attempts - 1), remote_receipt: item.remote_receipt, transport: '121_browser_worker', time: now() });
        } catch (error) {
          groupFailed = true;
          item.status = 'failed';
          item.error = error?.message || '上传失败';
          await plan.store.updateTaskMeta(owner, item.id, { siteSubmitStatus: 'failed', siteSubmitFailedVersions: [...new Set([...(task?.meta?.siteSubmitFailedVersions || []), item.version])] });
          if (typeof plan.store.appendSiteSubmitLog === 'function') await plan.store.appendSiteSubmitLog(owner, item.id, { status: 'failed', version: item.version, group_id: group.group_id, material_allocation: item.advanced, error: item.error, transport: '121_browser_worker', time: now() });
        }
      }
    ...
    const existingGroups = Array.isArray(plan.config.site_submit_groups) ? plan.config.site_submit_groups : [];
    await plan.store.saveConfig({ ...plan.config, site_submit_groups: [...existingGroups, ...plan.groups].slice(-200) });
    return { material_limit: plan.material_limit, groups: plan.groups, skipped: plan.skipped, success_groups: successGroups, accepted_groups: acceptedGroups, failed_groups: failedGroups, tasks: await plan.store.listTasks(owner) };
  }
...
module.exports = {
  normalizeUploadProfiles, normalizeProfileBindings, normalizeStyleCatalog, publicWebSubmit,
  parseTargetStyleCatalog, importedAdvancedFromTarget, selectVersionUploadProfile,
