...
function create121WebSubmitService({
  ...
  function saveDirectSession(owner, result) {
    const cookie = text(result?.cookie);
    if (!cookie) throw new Error('121 登录未返回会话 Cookie');
    sessionStore.setSession(owner, cookie);
    return cookie;
  }
  ...
  async function ensureSession(owner) {
    ...
    return { username, cookie: saveDirectSession(owner, result), refreshed: true };
  }
  ...
  async function saveConfig(owner, incoming = {}) {
    ...
    if (password) {
      ...
      saveDirectSession(owner, result);
