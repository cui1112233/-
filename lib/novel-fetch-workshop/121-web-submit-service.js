    ...
    for (const group of plan.groups) {
      ...
      for (const item of group.items) {
        ...
        try {
          ...
          item.remote_record = remoteRecord;
          await plan.store.appendSiteSubmitLog(owner, item.id, { version: item.version, status: item.status, receipt, remote_record: remoteRecord, time: now() });
        } catch (error) {
          groupFailed = true;
          failedGroups += 1;
          item.status = 'failed';
      ...
      if (!groupFailed && pending) acceptedGroups += 1;
      else if (!groupFailed) successGroups += 1;
      group.completed_at = now();
    }
    return { ok: failedGroups === 0, groups: plan.groups, skipped: plan.skipped, summary: { submitted: successGroups, accepted_pending: acceptedGroups, failed: failedGroups } };
  }
