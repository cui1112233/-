...
function normalizeSavedWebSubmit(existingValue, incomingValue) {
  ...
  return {
    enabled: incoming.enabled === undefined ? existing.enabled === true : incoming.enabled === true,
    ...
    skip_submitted: incoming.skip_submitted === undefined ? existing.skip_submitted !== false : incoming.skip_submitted !== false,
...
function create121WebSubmitService({
  ...
  if (!directClient || typeof directClient.login !== 'function' || typeof directClient.verify !== 'function' || typeof directClient.action !== 'function') throw new Error('121 direct client is required');
  if (!sessionStore || typeof sessionStore.getSession !== 'function' || typeof sessionStore.setSession !== 'function') throw new Error('121 session store is required');
  if (!credentialStore || typeof credentialStore.get !== 'function' || typeof credentialStore.set !== 'function') throw new Error('121 credential store is required');
  ...
  function saveDirectSession(owner, result, metadata = {}) {
    const cookie = text(result?.cookie);
    if (!cookie) throw new Error('121 登录未返回会话 Cookie');
    sessionStore.setSession(owner, cookie, metadata);
    return cookie;
  }
  ...
  async function current(owner) {
    ...
    const session = sessionStore.getSession(owner);
    const credentials = credentialStore.get(owner);
    const web = normalizeSavedWebSubmit(config.web_submit, config.web_submit);
    // 水货生产与小说获取共用同一份账号级 121 会话。旧会话可能只保存了
    // Cookie；新会话携带目标站账号后，小说获取可直接恢复其登录展示，无需用户重复登录。
    if (!web.username) {
      web.username = text(session?.targetUsername || credentials?.targetUsername);
    }
    return { store, config, web };
  }

  async function ensureSession(owner) {
    const { web } = await current(owner);
    const credentials = credentialStore.get(owner);
    const username = text(web.username || credentials?.targetUsername);
    const session = sessionStore.getSession(owner);
    const cookie = text(session?.cookie);
    if (!username) {
      const error = new Error('请先保存 121 账号并验证登录会话');
      error.code = 'DIRECT_SESSION_MISSING';
      error.status = 409;
      throw error;
    }
    if (cookie) {
      try {
        await directClient.verify({ cookie });
        return { username, cookie, refreshed: false };
      } catch (error) {
        const expired = error?.status === 401 || error?.code === 'SESSION_EXPIRED' || error?.code === 'session_expired';
        if (!expired) throw error;
      }
    }
    if (!credentials?.password || text(credentials.targetUsername) !== username) {
      const missing = new Error('121 登录会话已失效，请重新保存账号密码');
      missing.code = 'DIRECT_CREDENTIALS_MISSING';
      missing.status = 401;
      throw missing;
    }
    const result = await directClient.login({ username, password: credentials.password });
    return { username, cookie: saveDirectSession(owner, result, { targetUsername: username, baseUrl: normalizedBaseUrl }), refreshed: true };
  }

  function directPathForAction(action, payload = {}) {
    if (action === 'config_list') return target.TARGET_CONFIG_LIST_PATH;
    if (action === 'dashboard') return target.TARGET_CHECK_PATH;
    if (action === 'book_list') {
      const url = new URL(target.buildTargetBookListUrl(payload.bookId));
      return `${url.pathname}${url.search}`;
    }
    if (action === 'upload') return target.TARGET_UPLOAD_PATH;
    throw new Error(`未知的 121 直接接口动作：${action}`);
  }

  async function directAction(owner, action, payload = {}) {
    const ready = await ensureSession(owner);
    const isUpload = action === 'upload';
    const body = isUpload && payload.bodyBase64 ? Buffer.from(String(payload.bodyBase64), 'base64') : undefined;
    return directClient.action({
      cookie: ready.cookie,
      method: isUpload ? 'POST' : 'GET',
      path: directPathForAction(action, payload),
      headers: isUpload ? { 'Content-Type': text(payload.contentType) } : {},
      body
    });
  }
  ...
  async function saveConfig(owner, incoming = {}) {
    ...
    if (password) {
      ...
      const result = await directClient.login({ username: web.username, password });
      saveDirectSession(owner, result, { targetUsername: web.username, baseUrl: normalizedBaseUrl });
  ...
  async function environment(owner) {
    const targetUrl = `${normalizedBaseUrl}/index.php`;
    try {
      const ready = await ensureSession(owner);
      return { ok: true, targetUrl, checks: [{ name: '121 后台登录会话', ok: true, detail: `已登录：${targetUrl}` }, { name: '121 直接接口', ok: true, detail: ready.refreshed ? '会话已刷新并验证通过' : '会话验证通过' }] };
    } catch (error) {
      return { ok: false, targetUrl, checks: [{ name: '121 后台登录会话', ok: false, detail: error?.message || `验证失败：${targetUrl}` }, { name: '121 直接接口', ok: false, detail: '未建立可用会话' }] };
    }
  }

  async function testVisible(owner) {
    const ready = await ensureSession(owner);
    return { ok: true, output: [], result: { checks: { login_session: true, direct_http: true }, session_status: ready.refreshed ? 'refreshed' : 'ready' } };
  }

  async function syncConfigs(owner, { persist = true } = {}) {
    const { store, config } = await current(owner);
    const response = await directAction(owner, 'config_list');
    ...
    if (persist) await store.saveConfig({ ...config, web_submit: nextWeb });
    return { ok: true, settings: publicWebSubmit(nextWeb), groups: profiles, output: [`已从 121 同步 ${profiles.length} 个配置档`] };
  }

  async function syncStyles(owner, { persist = true } = {}) {
    const { store, config } = await current(owner);
    const response = await directAction(owner, 'dashboard');
    ...
    if (persist) await store.saveConfig({ ...config, styles, web_submit: nextWeb });
        ...
        try {
          ...
          if (item.confirmation_only) {
            ...
            for (let attempt = 0; attempt <= plan.web.retry_times; attempt += 1) {
              attempts += 1;
              try {
                uploaded = await directAction(owner, 'upload', { contentType: `multipart/form-data; boundary=${multipart.boundary}`, bodyBase64: multipart.body.toString('base64') });
                lastError = null;
                break;
          ...
          try {
            const lookup = parseJsonBody(await directAction(owner, 'book_list', { bookId: item.id, page: 1, pageSize: 10 }), '121 列表核验');
  ...
  return { getConfig, saveConfig, environment, testVisible, syncConfigs, syncStyles, preview, submit, ensureSession };
}
