...
function targetReceipt(data) {
  ...
  const accepted = successCount > 0 || Boolean(remoteId) || ['queued', 'queueing', 'running', 'executing', 'created'].includes(remoteStatus);
  return {
    verified: accepted,
    accepted,
    confirmed: false,
...
function summarizeRemoteBookRecord(data, bookId, fields, advanced) {
  ...
  if (!record) return { status: '待确认', found: false, confirmed: false, detail: '121 上传接口已接收文件，但列表暂未返回该书号记录', mismatches: [] };
        ...
        try {
          ...
          if (receipt.accepted) {
            try {
              ...
              receipt.remote_record = { status: '待确认', found: false, confirmed: false, detail: error?.message || '121 后台记录核验失败', mismatches: [] };
            }
          } else {
            receipt.remote_record = { status: '待确认', found: false, confirmed: false, detail: '121 上传响应没有可验证的接收回执', mismatches: [] };
          }
          if (receipt.remote_record?.confirmed === true) {
