...
function create121WebSubmitService({
  ...
  async function ensureSession(owner, { headed = false } = {}) {
    ...
    try {
      ...
      const expired = error?.status === 401 || error?.code === 'session_expired' || error?.code === 'TARGET_SESSION_EXPIRED' || error?.workerResponse?.status === 'expired';
      if (expired) {
        const missing = new Error('121 登录会话已失效，请重新输入账号密码');
        missing.code = 'TARGET_SESSION_EXPIRED';
        missing.status = 401;
        throw missing;
      }
      throw error;
  ...
  async function saveConfig(owner, incoming = {}) {
    ...
    if (password) {
      ...
      try {
        ...
        const expired = error?.status === 401 || error?.code === 'session_expired'
