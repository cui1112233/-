    ...
    for (const group of plan.groups) {
      ...
      for (const item of group.items) {
        ...
        try {
          ...
          let remoteRecord = { status: '待确认', found: false, detail: '121 列表尚未核验', mismatches: [] };
          try {
            const lookup = parseJsonBody(await workerAction(owner, 'book_list', { bookId: item.id, page: 1, pageSize: 10 }), '121 列表核验');
            remoteRecord = summarizeRemoteBookRecord(lookup, item.id, fields, item.advanced);
          } catch (error) {
            remoteRecord = { status: '待确认', found: false, detail: error?.message || '121 后台记录核验失败', mismatches: [] };
          }
          if (receipt.verified) {
...
module.exports = {
  ...
  targetReceipt, summarizeRemoteBookRecord, create121WebSubmitService
};
