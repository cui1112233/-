    ...
    const req = https.get(url, res => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve({ text: parsed && parsed.text || '', bookinfo: parsed && parsed.bookinfo });
        } catch (_) {
...
function createWorkshopTasks({ usersDir, fetchUpstream } = {}) {
  ...
  async function saveTasks(username, tasks) {
    ...
    for (const task of tasks) {
      ...
      const bookId = String(task.bookId == null ? '' : task.bookId).trim();
      if (!bookId) continue;
  ...
  function listTasks(username) {
    ...
    return readIndex(username)
      .filter(entry => isPlainObject(entry) && entry.bookId)
      .map(entry => ({
        ...entry,
        hasOriginal: fs.existsSync(originalPath(username, entry.bookId)),
        hasOriginalRaw: fs.existsSync(originalRawPath(username, entry.bookId)),
        hasAi: hasAnyAiVersion(username, entry.bookId)
      }));
  }
