...
function createWorkshopTasks({ usersDir, fetchUpstream, fetchConfig = {}, rules, getLayout } = {}) {
  ...
  function upsertIndexEntry(username, meta) {
    ...
    books.push({
      ...
      batchCreatedAt: meta.batchCreatedAt || '',
      batchOrder: Number(meta.batchOrder) || 0,
  ...
  function listTasks(username) {
    ...
    for (const entry of readIndex(username)) {
      ...
      result.push({
        ...entry,
        batchId: meta.batchId || entry.batchId || '',
        batchCreatedAt: meta.batchCreatedAt || entry.batchCreatedAt || '',
        batchOrder: Number(meta.batchOrder ?? entry.batchOrder) || 0,
