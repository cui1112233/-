...
function defaultFetchUpstream(bookId, platformId, maxTxt) {
    ...
    const req = https.get(url, res => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve({ text, bookinfo: parsed && parsed.bookinfo });
        } catch (_) {
...
function createWorkshopTasks({ usersDir, fetchUpstream } = {}) {
  ...
  // 列出任务的全部 AI 版本：ai/ai{n}/{bookId}.txt 存在时返回 [{ name, exists, chars, text }]（按 ai 序号排序）
  function listAiVersions(username, bookId) {
    const bid = normalizeBookId(bookId);
    ...
    try {
      ...
      if (error && error.code === 'ENOENT') return versions;
      throw error;
    }
    for (const entry of entries) {
      if (!entry.isDirectory() || !/^ai\d+$/.test(entry.name)) continue;
      const filePath = path.join(aiRoot, entry.name, `${bid}.txt`);
      if (!fs.existsSync(filePath)) continue;
      const text = fs.readFileSync(filePath, 'utf8');
      versions.push({ name: entry.name, exists: true, chars: text.length, text });
    }
    versions.sort((a, b) => (Number(a.name.replace(/\D/g, '')) || 0) - (Number(b.name.replace(/\D/g, '')) || 0));
    return versions;
  }
  ...
  // ===== 任务列表：从 index 读，补 hasOriginal/hasAi 等存在性标记，合并敏感词/提交状态 =====
  function listTasks(username) {
    ...
    for (const entry of readIndex(username)) {
      ...
      result.push({
        ...
        // 敏感词状态（meta 有则附加，无则空兜底）
        sensitiveStatus: meta.sensitiveStatus || '',
        sensitiveHitCount: meta.sensitiveHitCount || 0,
        sensitiveFixedCount: meta.sensitiveFixedCount || 0,
        sensitiveMode: meta.sensitiveMode || '',
        sensitiveModel: meta.sensitiveModel || '',
        // AI/分类状态
        ...
        error: meta.error || '',
        // 提交状态（无提交结果时为空兜底）
        ...siteSubmitSummary(username, bid)
      });
    }
    return result;
  }
  ...
  async function fetchOriginal(username, bookId, maxTxt) {
    ...
    try {
      const result = await fetchOne(bid, meta.platformId || '', maxTxt);
      ...
      writeJsonAtomic(metaPath(username, bid), meta);
      upsertIndexEntry(username, meta);
      appendLog(username, bid, 'original_fetch_failed', { bookId: bid, error: message });
  ...
  // ===== 敏感词记录（sensitive_hits / sensitive_fixed，缺省为空结构）=====
  function sensitiveHitsPath(username, bookId) { return path.join(dir(username), 'sensitive_hits', `${normalizeBookId(bookId)}.json`); }
  function sensitiveFixedPath(username, bookId) { return path.join(dir(username), 'sensitive_fixed', `${normalizeBookId(bookId)}.json`); }

  // 读取任务敏感词命中/修复记录：{ sensitive_hits, sensitive_fixed }（文件不存在时为空对象）
  function readSensitiveRecords(username, bookId) {
    const bid = normalizeBookId(bookId);
    const hits = readJsonOrMissing(sensitiveHitsPath(username, bid));
    const fixed = readJsonOrMissing(sensitiveFixedPath(username, bid));
    return {
      sensitive_hits: hits.found && isPlainObject(hits.value) ? hits.value : {},
      sensitive_fixed: fixed.found && isPlainObject(fixed.value) ? fixed.value : {}
    };
  }

  // ===== 网站提交日志与结果（site_submit，第二阶段 web-submit 写入，此处提供读写接口）=====
  function siteSubmitResultPath(username, bookId) { return path.join(dir(username), 'site_submit', `${normalizeBookId(bookId)}.json`); }
  function siteSubmitLogPath(username, bookId) { return path.join(dir(username), 'site_submit', `${normalizeBookId(bookId)}.jsonl`); }

  function readSiteSubmitResult(username, bookId) {
    const bid = normalizeBookId(bookId);
    const result = readJsonOrMissing(siteSubmitResultPath(username, bid));
    return result.found && isPlainObject(result.value) ? result.value : null;
  }

  function writeSiteSubmitResult(username, bookId, result) {
    const bid = normalizeBookId(bookId);
    const value = { ...(isPlainObject(result) ? result : {}), book_id: bid, updated_at: isoLocalTime(new Date()) };
    fs.mkdirSync(path.dirname(siteSubmitResultPath(username, bid)), { recursive: true });
    writeJsonAtomic(siteSubmitResultPath(username, bid), value);
    return value;
  }

  function appendSiteSubmitLog(username, bookId, event, data) {
    const bid = normalizeBookId(bookId);
    const logPath = siteSubmitLogPath(username, bid);
    fs.mkdirSync(path.dirname(logPath), { recursive: true });
    const line = JSON.stringify({ time: isoLocalTime(new Date()), event, data: isPlainObject(data) ? data : {} });
    fs.appendFileSync(logPath, `${line}\n`, 'utf8');
  }
  ...
  // 提交状态汇总（对齐 Python web_submit_summary）：无结果文件时返回空兜底结构
  function siteSubmitSummary(username, bookId) {
    const result = readSiteSubmitResult(username, bookId);
    if (!result) {
      return {
        site_submit_status: '',
        site_submit_done_versions: [],
        site_submit_failed_versions: [],
        site_submit_group_id: '',
        site_submit_error: '',
        site_submit_updated_at: ''
      };
    }
    const versions = isPlainObject(result.versions) ? result.versions : {};
    const done = Object.keys(versions).filter(key => isPlainObject(versions[key]) && versions[key].status === 'submitted');
    const failed = Object.keys(versions).filter(key => isPlainObject(versions[key]) && versions[key].status === 'failed');
    let latestError = '';
    for (const key of failed) {
      if (isPlainObject(versions[key]) && versions[key].error) {
        latestError = String(versions[key].error);
        break;
      }
    }
    return {
      site_submit_status: result.status || (done.length && !failed.length ? 'submitted' : failed.length ? 'failed' : ''),
      site_submit_done_versions: done,
      site_submit_failed_versions: failed,
      site_submit_group_id: result.last_group_id || '',
      site_submit_error: latestError || result.error || '',
      site_submit_updated_at: result.updated_at || ''
    };
  }

  // ===== 批量重试：按「分类→抓取→AI 生成」逐级补跑缺失/失败环节 =====
  // deps 可注入：{ retryClassify(username, meta), fetchOriginal(username, bookId, maxTxt), generateAi(username, bookId, count) }
  // 默认：分类/AI 跳过，抓取走内部 fetchOriginal。返回 { requested, retried, failed, results }。
  async function batchRetry(username, bookIds, deps = {}) {
    assertUsername(username);
    if (!Array.isArray(bookIds)) throw new Error('bookIds must be an array');
    const retryClassify = typeof deps.retryClassify === 'function' ? deps.retryClassify : async (u, meta) => meta;
    const fetchOriginalStep = typeof deps.fetchOriginal === 'function'
      ? deps.fetchOriginal
      : async (u, bookId, maxTxt) => fetchOriginal(u, bookId, maxTxt);
    const generateAiStep = typeof deps.generateAi === 'function' ? deps.generateAi : async () => ({ status: 'skipped' });

    const ids = bookIds.filter(rawId => String(rawId == null ? '' : rawId).trim() !== '');
    const results = [];
    let retried = 0;
    let failed = 0;
    for (const rawId of ids) {
      ...
      try {
        let meta = readMeta(username, bookId);
        if (!meta) throw new Error('任务不存在');
        appendLog(username, bookId, 'task_retry_started', { bookId });
        meta = { ...meta, status: 'retrying', error: '', updatedAt: new Date().toISOString() };
        writeJsonAtomic(metaPath(username, bookId), meta);
        upsertIndexEntry(username, meta);

        let didWork = false;
        // 1. 分类补跑：缺 style/gender，或分类失败/等待 AI 配置
        if (!meta.style || !meta.gender || meta.classifyStatus === 'failed' || meta.classifyStatus === 'waiting_ai_config') {
          meta = (await retryClassify(username, meta)) || meta;
          didWork = true;
        }
        // 2. 抓取补跑：原文未就绪或文件缺失
        meta = readMeta(username, bookId) || meta;
        if (meta.originalStatus !== 'done' || !fs.existsSync(originalPath(username, bookId))) {
          await fetchOriginalStep(username, bookId, meta.maxTxt);
          meta = readMeta(username, bookId) || meta;
          didWork = true;
        }
        // 3. AI 补跑：AI 失败/部分/等待状态且原文就绪
        if (['failed', 'partial', 'waiting_original', 'waiting_ai_config'].includes(meta.aiStatus)
          && fs.existsSync(originalPath(username, bookId))) {
          await generateAiStep(username, bookId, Number(meta.aiCount) || 1);
          meta = readMeta(username, bookId) || meta;
          didWork = true;
        }

        if (!didWork) {
          appendLog(username, bookId, 'task_retry_skipped', { reason: 'no_failed_or_waiting_step' });
        }
        // 收尾：重试结束且任务整体无失败 → 统一标记 retry_done。
        // 补跑过程中 fetch 可能把 status 覆盖为 original_done/original_failed，
        // 因此只要本次做过补跑（或仍停在 retrying）且无失败态，就落回 retry_done。
        if (!taskIsFailed(meta) && (meta.status === 'retrying' || (didWork && meta.status !== 'retry_done'))) {
          meta = { ...meta, status: 'retry_done', updatedAt: new Date().toISOString() };
          writeJsonAtomic(metaPath(username, bookId), meta);
          upsertIndexEntry(username, meta);
        }
        appendLog(username, bookId, 'task_retry_finished', {
          status: meta.status, originalStatus: meta.originalStatus, aiStatus: meta.aiStatus
        });
        const ok = !taskIsFailed(meta);
        if (ok) retried++; else failed++;
        results.push({
          id: bookId,
          ok,
          status: meta.status || '',
          error: meta.error || meta.classifyError || meta.aiError || ''
        });
      } catch (error) {
        const message = error && error.message ? String(error.message) : String(error);
        failed++;
        results.push({ id: bookId, ok: false, status: '', error: message });
      }
    }
    return { requested: ids.length, retried, failed, results };
  }
  ...
  return {
    ...
    deleteTasks,
    restoreOriginal,
    readSensitiveRecords,
    readSiteSubmitResult,
    writeSiteSubmitResult,
    appendSiteSubmitLog,
    readSiteSubmitLog,
    siteSubmitSummary,
    batchRetry
...
// ===== 重试判定辅助（模块级纯函数，供 batchRetry 使用）=====

// 拼接任务各状态字段，用于 failed/waiting/partial 判定
function statusText(meta) {
  return [
    meta && meta.status,
    meta && meta.originalStatus,
    meta && meta.aiStatus,
    meta && meta.classifyStatus,
    meta && meta.error,
    meta && meta.classifyError,
    meta && meta.aiError
  ].map(value => String(value == null ? '' : value).toLowerCase()).join(' ');
}

// 是否失败：任一状态字段含 failed / 失败
function taskIsFailed(meta) {
  return /failed|失败/.test(statusText(meta));
}

// 是否需要重试：failed / waiting / partial 任一命中（对齐 Python task_needs_retry）
function taskNeedsRetry(meta) {
  return /failed|失败|waiting|等待|partial/.test(statusText(meta));
}

module.exports = { createWorkshopTasks, normalizeNewlines, dropEmptyLines, statusText, taskIsFailed, taskNeedsRetry };
