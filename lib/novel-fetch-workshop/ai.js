...
function resolveAiSettings(configStore, purpose) {
  ...
  const { ai, ai_presets = [], ai_assignments = {} } = aiConfig;
  ...
  base = base || {};
  const settings = {
    // 兼容工作台旧版/前端测试接口曾保存的 camelCase 配置。
    baseUrl: base.base_url || base.baseUrl || '',
    apiKey: base.api_key || base.apiKey || '',
    model: base.model || ''
  };
  // 中央模型选择为空时兼容已有的旧版账户配置。模型目录迁移后，旧配置仍可能只保留
  // 顶层 provider/baseUrl/model/apiKey；不能因为没有 text_model_id 就把它误判为无凭证。
  if (!selectedModelId) {
    settings.baseUrl = settings.baseUrl || configured.baseUrl || '';
    settings.apiKey = settings.apiKey || configured.apiKey || '';
    settings.model = settings.model || configured.model || '';
  }
  // 新版统一模型目录只在执行时解析凭据，避免把 API Key 回填到旧工作台页面。
  // 选定中央模型后，它优先于按用途的旧预设；旧配置仍保留作为兼容回退。
  if (selectedModelId && typeof configStore.resolveRuntimeModel === 'function') {
    try {
