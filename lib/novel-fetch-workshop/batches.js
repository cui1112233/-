...
function createNovelFetchBatches({ usersDir, clock = () => new Date() } = {}) {
  ...
  function complete(owner, id, result = {}) {
    return mutate(owner, state => {
      ...
      record.status = Number(result.cancelled) > 0 ? 'cancelled' : ((Number(result.fetch_failed) || Number(result.rewrite_failed)) ? 'partial' : 'done');
  ...
  return { create, current, list, get, complete, prepareRerun };
}
