...
function createNovelFetchBatches({ usersDir, clock = () => new Date() } = {}) {
  ...
  function create(owner, input = {}) {
    return mutate(owner, state => {
      ...
      const taskIds = [...new Set((Array.isArray(input.taskIds) ? input.taskIds : []).map(text).filter(Boolean))];
      const taskOrder = [...new Set((Array.isArray(input.taskOrder) ? input.taskOrder : taskIds).map(text).filter(Boolean))]
  ...
  function complete(owner, id, result = {}) {
    return mutate(owner, state => {
      ...
      const originalIds = [...new Set((Array.isArray(record.taskIds) ? record.taskIds : []).map(text).filter(Boolean))];
      const reportedIds = [...new Set((Array.isArray(result.current_batch_ids) ? result.current_batch_ids : []).map(text).filter(Boolean))];
      const scopedIds = originalIds.length ? originalIds : reportedIds;
      const membership = new Set(scopedIds);
      const taskStates = tasks.map(snapshotTaskState)
        .filter(item => item.bookId && (!membership.size || membership.has(item.bookId)));
      if (taskStates.length) record.taskStates = taskStates;
      if (!originalIds.length && scopedIds.length) record.taskIds = scopedIds;
      else if (!originalIds.length && taskStates.length) record.taskIds = [...new Set(taskStates.map(item => item.bookId))];
      if (!Array.isArray(record.taskOrder) || !record.taskOrder.length) record.taskOrder = (record.taskIds || []).slice();
