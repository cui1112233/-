...
function createV78NovelFetchBatchExecutor(options = {}) {
  ...
  const createLifecycleClient = options.createLifecycleClient || require('./lifecycle-client').createNovelFetchLifecycleClient;
  ...
  return async function executeBatch(owner, payload, executionContext = {}) {
    ...
    try {
      ...
      const cleanup = await lifecycle.cleanupBodies('expired', 100);
      report({ type: 'storage_cleanup', status: 'done', message: `正文自动清理完成：删除 ${cleanup.deleted || 0} 份正文，历史记录保留。`, cleanup });
      return { ...result, cleanup };
    } catch (error) {
      const message = error?.message || String(error);
      report({ type: 'storage_cleanup', status: 'warning', message: `正文自动清理失败：${message}` });
