...
function createV78NovelFetchBatchExecutor(options = {}) {
  ...
  return async function executeBatch(owner, payload, executionContext = {}) {
    ...
    const fallbackFetchOriginal = typeof tasks.fetchOriginal === 'function' ? tasks.fetchOriginal.bind(tasks) : null;
    const sourceFetchOriginal = typeof options.sourceFetchOriginal === 'function'
      ? request => options.sourceFetchOriginal(owner, { ...object(request), tasks, fallbackFetchOriginal })
      : undefined;
    // The shared runner still calls tasks.fetchOriginal. Route that single call through the
    // source injection when configured, while retaining the exact previous fetch function as
    // an explicit fallback. This keeps the current verified fetch path intact until a real
    // source-site submit/poll/result/download adapter contract is available.
    if (sourceFetchOriginal && fallbackFetchOriginal) {
      tasks.fetchOriginal = (_username, bookId, maxTxt) => sourceFetchOriginal({ bookId, maxTxt });
    }
