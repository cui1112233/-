...
const DEFAULT_CONFIG = {
  ...
  fetch: { default_max_txt: 4000, timeout_seconds: 30, concurrency: 4, retries: 1 },
...
function createMySQLWorkshopStore({ targetBaseUrl, bridgeSecret, account, fetchUpstream } = {}) {
  ...
  async function fetchOriginal(username, bookId, maxTxt) {
    const document = await readDocument(assertBookID(bookId));
    try {
      const response = fetchUpstream
        ? await fetchUpstream(document.bookId, document.meta.platformId || '', maxTxt)
        : await fetch(`https://txt.121w.com/api.php?bookid=${encodeURIComponent(document.bookId)}&platform=${encodeURIComponent(document.meta.platformId || '')}&max_txt=${encodeURIComponent(maxTxt)}`).then(async res => { if (!res.ok) throw new Error(`上游抓取失败（${res.status}）`); return res.json(); });
      const raw = String(response?.text || '');
      if (!raw) throw new Error('未获取到原文内容');
      document.originalRaw = raw;
      document.original = cleanText(raw);
      document.meta = { ...document.meta, originalStatus: 'done', originalRawChars: raw.length, originalChars: document.original.length, originalFetchedAt: now(), originalProcessedAt: now(), status: 'original_done', bookinfo: response?.bookinfo || document.meta.bookinfo, updatedAt: now() };
      if (!document.meta.bookName && response?.bookinfo?.book_name) document.meta.bookName = String(response.bookinfo.book_name);
      document.logs.push({ time: now(), event: 'original_fetched', data: { bookId: document.bookId, originalChars: document.original.length } });
      await writeDocument(document);
      return { status: 'done' };
    } catch (error) {
  ...
  return {
    ...
    getStyles: () => STYLES,
    saveTasks, listTasks, getTask, updateTaskMeta, fetchOriginal, readOriginal, saveOriginalText, readLogs, readVersionText, saveVersionText, appendLog, appendSiteSubmitLog, readSiteSubmitLog, readSiteSubmitResult, deleteTasks
  };
}

module.exports = { createMySQLWorkshopStore, DEFAULT_CONFIG, normalizeNewlines, dropEmptyLines, cleanText };
