...
const META_DEFAULTS = {
  bookId: '', bookName: '', paidBookId: '', freeBookId: '', platformId: '', platformName: '', gender: '', genderSource: '', style: '', styleSource: '', tags: '', reason: '', rating: '', sourceLine: '', parseMode: '', parseColumns: [], maxTxt: 4000, aiCount: 1, classifyStatus: '', classifyError: '', classifyConfidence: null, classifyReason: '', classifierModel: '', status: 'created', originalStatus: '', originalChars: 0, originalRawChars: 0, originalFetchAttempts: 0, originalErrorCode: '', aiStatus: '', aiGeneratedCount: 0, aiError: '', error: '', createdAt: '', updatedAt: ''
};
...
function createMySQLWorkshopStore({ targetBaseUrl, bridgeSecret, account, fetchUpstream } = {}) {
  ...
  async function fetchDirectOriginal({ bookId, platformId, maxTxt } = {}) {
    const requestedBookId = assertBookID(bookId);
    const requestedPlatform = String(platformId || '').trim();
    if (!requestedPlatform) {
      const error = new Error('请选择抓取平台');
    ...
    return {
      bookId: requestedBookId,
      platformId: requestedPlatform,
      text: limitProcessedOriginal(result.text, effectiveMaxTxt),
      rawText: result.text,
      attempts: result.attempts,
      bookinfo: isPlainObject(result.bookinfo) ? result.bookinfo : {}
    };
  }
  async function fetchOriginal(username, bookId, maxTxt) {
    ...
    try {
      ...
      document.meta = { ...document.meta, platformAutoDetected: false, originalStatus: 'failed', status: 'original_failed', originalFetchAttempts: attempts, originalErrorCode: code, error: error.message || String(error), updatedAt: now() };
      document.logs.push({ time: now(), event: 'original_fetch_failed', data: { bookId: document.bookId, error: document.meta.error, code, attempts } });
      await writeDocument(document);
