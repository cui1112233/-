  ...
  async function fetchOriginal(username, bookId, maxTxt) {
    ...
    try {
      ...
      for (const platformId of candidates) {
        try {
          const candidate = fetchUpstream
            ? await fetchUpstream(document.bookId, platformId, maxTxt)
            : await fetch(buildFetchURL(endpoint, { bookId: document.bookId, platformId, maxTxt }))
              .then(async res => { if (!res.ok) throw new Error(`上游抓取失败（HTTP ${res.status}）`); return res.json(); });
      ...
      document.logs.push({ time: now(), event: 'original_fetched', data: { bookId: document.bookId, originalChars: document.original.length, requestedPlatform, resolvedPlatform, platformAutoDetected: resolvedPlatform !== requestedPlatform } });
      await writeDocument(document);
      return { status: 'done' };
    } catch (error) {
