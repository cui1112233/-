...
function bodyRefsFromRecord(record) {
  const refs = {};
  if (isPlainObject(record?.bodyRefs)) {
    for (const [versionId, ref] of Object.entries(record.bodyRefs)) {
      if (!VERSION_ID_PATTERN.test(versionId)) continue;
      refs[versionId] = isPlainObject(ref) ? { ...ref, versionId: ref.versionId || versionId } : { versionId, state: 'ready' };
    }
  }
  // 兼容已做过 S1 懒迁移、但尚未回写 bodyRefs 的旧任务；这里只保留版本可用性，不保留全文。
  if (isPlainObject(record?.versions)) {
    for (const [versionId, value] of Object.entries(record.versions)) {
      if (!VERSION_ID_PATTERN.test(versionId) || refs[versionId]) continue;
      refs[versionId] = isPlainObject(value) ? { ...value, versionId: value.versionId || versionId } : { versionId, state: 'ready' };
    }
  }
  if (!refs.original && (record?.meta?.originalStatus === 'done' || Number(record?.meta?.originalChars) > 0)) {
    refs.original = { versionId: 'original', state: 'ready' };
  }
  return refs;
}
...
function createMySQLWorkshopStore({ targetBaseUrl, bridgeSecret, account, fetchUpstream } = {}) {
  ...
  function bodyPathFor(bookId, versionId) {
    const base = `${pathFor(bookId)}/bodies`;
    return versionId === undefined ? base : `${base}/${encodeURIComponent(assertVersionID(versionId))}`;
  }
  async function readBody(bookId, versionId) {
    const record = await request('GET', bodyPathFor(bookId, versionId));
    return String(record.content || '');
  }
  async function writeBody(bookId, versionId, content, state = 'ready') {
    return request('PUT', bodyPathFor(bookId, versionId), { content: String(content || ''), state: String(state || 'ready') });
  }
  async function listBodyRefs(bookId) {
    const response = await request('GET', bodyPathFor(bookId));
    return Array.isArray(response.bodies) ? response.bodies : [];
  }
  async function deleteBody(bookId, versionId) {
    const response = await request('DELETE', bodyPathFor(bookId, versionId));
  ...
  async function readDocument(bookId) {
    ...
    return {
      ...
      bodyRefs: bodyRefsFromRecord(record),
      logs: Array.isArray(record.logs) ? record.logs : []
    };
  }
  async function writeDocument(document) {
    await request('PUT', pathFor(document.bookId), {
      meta: document.meta || {},
      bodyRefs: isPlainObject(document.bodyRefs) ? document.bodyRefs : {},
      logs: document.logs || []
    });
  }
  async function existingOrNew(bookId) {
    try {
      return await readDocument(bookId);
    } catch (error) {
      if (error.status === 404) return { bookId, meta: {}, bodyRefs: {}, logs: [] };
      throw error;
    }
  }
  ...
  async function getTask(username, bookId) {
    try {
      const document = await readDocument(assertBookID(bookId));
      return { meta: document.meta, hasOriginalRaw: false, document };
    } catch (error) { if (error.status === 404) return null; throw error; }
  }
  async function listTasks(username) {
    const response = await request('GET', '/api/novel-fetch-workshop/tasks');
    return (response.tasks || []).map(record => {
      const refs = bodyRefsFromRecord(record);
      const bodyVersions = Object.keys(refs);
      return {
        ...(isPlainObject(record.meta) ? record.meta : {}),
        bookId: record.bookId,
        hasOriginal: bodyVersions.includes('original'),
        hasOriginalRaw: false,
        hasAi: bodyVersions.some(key => /^ai\d+$/.test(key)),
        bodyVersions,
        bodyRefs: refs,
        updatedAt: record.updatedAt || record.meta?.updatedAt || ''
      };
    });
  }
  ...
  async function fetchOriginal(username, bookId, maxTxt) {
    ...
    try {
      ...
      const originalRef = await writeBody(document.bookId, 'original', original, 'ready');
      document.bodyRefs = { ...document.bodyRefs, original: originalRef };
      document.meta = {
        ...document.meta,
        platformAutoDetected: false,
        originalStatus: 'done', originalRawChars: raw.length, originalChars: original.length,
  ...
  async function readOriginal(username, bookId) {
    try { return await readBody(assertBookID(bookId), 'original'); }
  ...
  async function saveOriginalText(username, bookId, text) {
    ...
    const ref = await writeBody(id, 'original', content, 'ready');
    document.bodyRefs = { ...document.bodyRefs, original: ref };
    document.meta = { ...document.meta, originalChars: content.length, originalProcessedAt: now(), updatedAt: now() };
    document.logs.push({ time: now(), event: 'original_rules_applied', data: { bookId: document.bookId, originalChars: content.length } });
    await writeDocument(document);
  }
  async function restoreOriginal(username, bookId) {
    await readDocument(assertBookID(bookId));
    throw new Error('没有可恢复的原始备份');
  }
  async function readLogs(username, bookId) { return (await readDocument(assertBookID(bookId))).logs; }
  async function readVersionText(username, bookId, version) {
    const versionId = version === 'edited' ? 'original' : assertVersionID(version);
    try { return await readBody(assertBookID(bookId), versionId); }
    catch (error) { if (error.status === 404) return ''; throw error; }
  }
  async function saveVersionText(username, bookId, version, text) {
    const id = assertBookID(bookId);
    const versionId = version === 'edited' ? 'original' : assertVersionID(version);
    const document = await readDocument(id);
    const ref = await writeBody(id, versionId, String(text || ''), 'ready');
    document.bodyRefs = { ...document.bodyRefs, [versionId]: ref };
    await writeDocument(document);
    return ref;
  }
  ...
  return {
    ...
    getStyles: () => STYLES,
    saveTasks, listTasks, getTask, updateTaskMeta, fetchOriginal, readOriginal, saveOriginalText, restoreOriginal, readLogs, readVersionText, saveVersionText, readBody, writeBody, listBodyRefs, deleteBody, appendLog, appendSiteSubmitLog, readSiteSubmitLog, readSiteSubmitResult, deleteTasks
  };
}
