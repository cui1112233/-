...
const { taskSelectedVersions, generatedVersions, normalizeSelectedVersions, normalizeAiSlotMethods } = require('./version-selection');
...
function toV78Task(task = {}) {
  const selectedVersions = taskSelectedVersions(task);
  return {
    ...task,
    ...
    ai_slot_methods: normalizeAiSlotMethods(task.aiSlotMethods || task.ai_slot_methods),
    ai_files: Array.isArray(task.ai_files) ? task.ai_files : generatedVersions(task),
...
function createNovelFetchTaskOps({ accountResolver, createStore, tombstones, parseBooks, clock = () => new Date(), applySavedRules, createConfigSnapshot } = {}) {
  ...
  let applySavedRulesToOriginal = applySavedRules || null;
  let makeConfigSnapshot = createConfigSnapshot || null;
  ...
  async function setSelectedVersions(owner, ids, selectedVersions, aiSlotMethods = {}) {
    const versions = normalizeSelectedVersions(selectedVersions);
    ...
    for (const [id] of records) await store.updateTaskMeta(owner, id, { selectedVersions: versions, aiSlotMethods: methods });
    return { ok: true, missing: [], updated: records.length, selectedVersions: versions, aiSlotMethods: methods };
  }

  async function reprocessSensitive(owner, body = {}) {
    if (!applySavedRulesToOriginal || !makeConfigSnapshot) {
      const batchExecutor = require('./v2-batch-executor');
      applySavedRulesToOriginal ||= batchExecutor.createApplySavedRulesToOriginal();
      makeConfigSnapshot ||= batchExecutor.createConfigStoreSnapshot;
    }
    const store = storeFor(owner);
  ...
  return { processConflicts, permanentDelete, restoreTombstone, list, setSelectedVersions, reprocessSensitive };
}
