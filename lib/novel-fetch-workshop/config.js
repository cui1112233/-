...
const DEFAULT_WORKSHOP_CONFIG = {
  workflow: {
    ...
    auto_sync_site_styles: false,
    auto_reclassify_invalid_style: false,
  },
  fetch: {
    endpoint: 'https://txt.121w.com/api.php', default_max_txt: 4000, timeout_seconds: 30, concurrency: 2, retries: 1,
    auto_detect_platform: false, min_original_chars: 0, skip_short_original: false
  },
  ai: { base_url: '', api_key: '', model: '', timeout_seconds: 180, max_concurrency: 2, retry_times: 2,
  ...
  sensitive_ai: { enabled: true, mode: 'ai_each', context_chars: 12, max_hits_per_task: 80, concurrency: 4, retries: 1,
  ...
  web_submit: { batch_size: 20, flush_seconds: 0 },
...
function createWorkshopConfigStore({ systemDir } = {}) {
  ...
  // 方案 A：patch 中的 ai / ai_presets / ai_assignments 三个键剥离出来转交 ai-config.json 写入（复用
  // saveAiConfig 的写入路径），其余键合并写入 app.json。这样无论经 saveConfig 还是 saveAiConfig 修改 ai 配置，
  // getConfig() 读回（ai 相关以 ai-config.json 为最终覆盖源）都一致，避免 app.json 中 ai 值被静默丢弃。
  function saveConfig(patch = {}) {
    ...
    delete mainPatch.ai;
    delete mainPatch.ai_presets;
    delete mainPatch.ai_assignments;

    // 先写 ai-config.json（saveAiConfig 内部使用同一把 .config.lock），再合并主配置写 app.json
    if (Object.keys(aiPatch).length > 0) saveAiConfig(aiPatch);

    const merged = deepMerge(getConfig(), mainPatch);
