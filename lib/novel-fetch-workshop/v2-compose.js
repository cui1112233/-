...
function attachV78NovelFetchV2({
  ...
  const directClientFactory = createDirectClient || require('./121-direct-client').create121DirectClient;
  ...
  const directClient = directClientFactory();
  ...
  if (!sessionStore?.getSession || !sessionStore?.setSession) throw new Error('core novel fetch 121 session store is required');
  const createStore = ({ account }) => require('./mysql-store').createMySQLWorkshopStore({ account });
  const baseWebSubmit = webSubmitFactory({ accountResolver, createStore, directClient, sessionStore, credentialStore });
