...
function requireCredentialSecret(value) {
  const secret = String(value || '').trim();
  if (!secret) throw new Error('QIANTIE_121_CREDENTIAL_SECRET credential encryption secret is required');
  if (secret === FORBIDDEN_DEV_SECRET) throw new Error('QIANTIE_121_CREDENTIAL_SECRET must not use the development default');
  return secret;
}
...
function attachV78NovelFetchV2({
  ...
  createWebSubmit,
  credentialSecret = process.env.QIANTIE_121_CREDENTIAL_SECRET
} = {}) {
  ...
  if (!createBrowserClient && browserClient?.configured !== true) {
    throw new Error('QIANTIE_121_BROWSER_WORKER_URL and QIANTIE_121_WORKER_SECRET are required for Novel Fetch V2');
  }
  const encryptionSecret = requireCredentialSecret(credentialSecret);
  const credentialStore = credentialStoreFactory({ usersDir, secret: encryptionSecret });
...
module.exports = { requireCredentialSecret, attachV78NovelFetchV2 };
