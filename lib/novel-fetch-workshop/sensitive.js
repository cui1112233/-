...
// 未配置替换词时不做猜测性替换；未知词保留原文并进入失败重试。
// 保留导出名是为了兼容旧调用方，但不再把短词映射套进更长的普通词。
const SENSITIVE_NEUTRAL_REPLACEMENTS = Object.freeze({});
...
function isProviderSafetyRefusal(content) {
  const text = String(content == null ? '' : content).trim();
  return /the prompt could not be submitted/i.test(text)
    || /prohibited use polic(?:y|ies)/i.test(text)
    || /prompt contains sensitive words/i.test(text)
    || /内容安全策略|安全策略拦截|提示词.*被拒绝|请求.*被拒绝/.test(text);
}

// ===== 中性回落 =====
// 仅使用用户配置的替换词；未知词保留原词，
// 让命中以 failed 状态进入重试/人工处理，而不是静默写入“合规表达”破坏原句。
...
function neutralizeSensitiveSnippet(snippet, keywordText, rules) {
  ...
  for (const keyword of keywords) {
    ...
    if (!kw) continue;
    const replacement = configuredSensitiveReplacement(rules, kw) || kw;
...
async function repairSensitiveHit({ settings, hit, rules, ai }) {
  ...
  for (let attempt = 1; attempt <= retries + 1; attempt++) {
    try {
      ...
      if (isProviderSafetyRefusal(content)) throw new Error('上游内容政策拒绝，未修改原文');
...
async function processSensitiveText({ text, settings, ai } = {}) {
  ...
  if (!cfg.enabled) {
    ...
    return { text: replaced, hits, fixedItems: [], fixedCount: hits.length };
  }

  // AI 模式
  const hits = findSensitiveHits(source, rules, { maxHits, contextChars });
  if (!hits.length) return { text: source, hits, fixedItems: [], fixedCount: 0 };
  ...
  return { text: repaired, hits, fixedItems, fixedCount };
}
