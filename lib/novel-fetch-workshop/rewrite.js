...
async function writeModelAudit(tasks, { bookId, settings, status, attempts, errorMessage }) {
  if (typeof tasks?.writeRunAudit !== 'function' || !settings?.textModelId) return;
  try {
    await tasks.writeRunAudit({
      runId: crypto.randomUUID(), bookId, stage: 'rewrite', status, attempts,
      textModelId: settings.textModelId, modelId: settings.textModelId,
      modelDisplayName: settings.modelDisplayName || settings.model || '', errorMessage: errorMessage || '',
      finishedAt: new Date().toISOString()
    });
  } catch (_) { /* audit must not turn a completed generation into a failure */ }
}
...
async function generateAiVersion({ configStore, tasks, username, task, aiIndex, count, slotMethods, ai } = {}) {
  ...
  const strategy = methodForAiIndex(
    ...
    aiIndex,
    normalizeAiSlotMethods(slotMethods || task?.aiSlotMethods || task?.ai_slot_methods || rewriteConfig.ai_slot_methods)
  );
  ...
  if (lastRequestError) {
    ...
    await writeModelAudit(tasks, { bookId, settings, status: 'failed', attempts: retryTimes + 1, errorMessage: message });
  ...
  await writeModelAudit(tasks, { bookId, settings, status: 'succeeded', attempts: 1 });
