...
async function generateAiVersion({ configStore, tasks, username, task, aiIndex, count, slotMethods, ai } = {}) {
  ...
  if (lastRequestError) {
    ...
    await writeModelAudit(tasks, { bookId, stage: 'rewrite', settings, status: 'failed', attempts: retryTimes + 1, errorMessage: message });
  ...
  await writeModelAudit(tasks, { bookId, stage: 'rewrite', settings, status: 'succeeded', attempts: 1 });
