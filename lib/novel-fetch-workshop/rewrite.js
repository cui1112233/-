...
async function generateAiVersion({ configStore, tasks, username, task, aiIndex, count, ai } = {}) {
  ...
  try {
    ...
    await tasks.appendLog(username, bookId, 'ai_generate_failed', { bookId, aiIndex, strategy, error: message });
  ...
  await tasks.saveVersionText(username, bookId, `ai${aiIndex}`, merged);
