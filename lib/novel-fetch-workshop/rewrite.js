...
function buildRewriteExtraInstruction({ task, config, aiIndex, knowledge }) {
  const prompt = knowledge?.templateSystem || (config && config.rewrite && config.rewrite.prompt) || '';
  return [
    `本次生成第 ${aiIndex} 个AI文案。`,
...
function buildHighImitationMessages({ config, task, targetLines, anchorLines, fullText, aiIndex, knowledge }) {
  ...
  const system = { role: 'system', content: [HIGH_IMITATION_SYSTEM, highPrompt].filter(Boolean).join('\n\n') };
  const user = {
    role: 'user',
    content: JSON.stringify({
      book_id: (task && task.bookId) || '',
...
async function generateAiVersions({ configStore, tasks, username, task, aiIndexes, versions, count, slotMethods, ai } = {}) {
