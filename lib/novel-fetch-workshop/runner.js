...
async function existingOriginalReady(tasks, username, bookId) {
  if (typeof tasks?.getTask !== 'function') return false;
  const record = await tasks.getTask(username, bookId);
  if (!record?.meta || record.meta.originalStatus !== 'done') return false;
  return Boolean((await readSavedOriginal(tasks, username, bookId)).trim());
}
...
async function runNovelFetchBatch({
  ...
  if (workflow.auto_fetch_original !== false) {
    const fetchedResults = await runWithConcurrency(classified, config.fetch?.concurrency || 1, async task => {
      ...
      if (await existingOriginalReady(tasks, username, task.bookId)) {
  ...
  if (!shouldStop() && workflow.auto_submit_after_rewrite === true && workflow.auto_submit_confirmed === true && config.web_submit?.enabled === true && fetchedIds.length && typeof submit === 'function') {
    ...
    for (const ids of splitSubmitBatches(fetchedIds, policy.web_submit.batch_size)) {
      if (shouldStop()) break;
      const submitted = await submit({ mode: 'selected', ids, versions: autoSubmitVersions });
