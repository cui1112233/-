...
async function runNovelFetchBatch({
  ...
  if (workflow.auto_fetch_original !== false) {
    const fetchedResults = await runWithConcurrency(classified, config.fetch?.concurrency || 1, async task => {
      ...
      // Retry/recovery is artifact-first: a completed saved original is a checkpoint, not a reason to fetch again.
      ...
      // A stop requested while fetch was in flight lets fetch finish/save, then prevents the next stage.
      if (await taskStopRequested(tasks, username, task.bookId, shouldStop)) {
