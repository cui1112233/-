...
const { normalizeSelectedVersions, normalizeAiSlotMethods } = require('./version-selection');
...
async function runNovelFetchBatch({
  ...
  const selectedVersions = normalizeSelectedVersions(payload?.selected_versions);
  const aiSlotMethods = normalizeAiSlotMethods(payload?.ai_slot_methods);
  ...
  if (workflow.auto_rewrite_after_fetch) {
    ...
    const rewriteResults = await runWithConcurrency(fetchedIds, rewriteConcurrency, async bookId => {
      ...
      return generateAiVersions({ configStore, tasks, username, task: task?.meta, versions: task?.meta?.selectedVersions, slotMethods: task?.meta?.aiSlotMethods });
    });
