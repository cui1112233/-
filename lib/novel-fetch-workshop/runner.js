...
async function runNovelFetchBatch({
  ...
  const prepared = parsedTasks.map(item => ({
    ...
    aiCount,
    ...(payload?.batch_id ? { batchId: String(payload.batch_id) } : {})
  }));
  ...
  if (retryStage || workflow.auto_fetch_original !== false) {
    ...
    for (const [index, item] of fetchedResults.entries()) {
      ...
      if (item?.ok && item.value?.stopped) {
        cancelled += 1;
        report({ type: 'fetch', status: 'cancelled', book_id: task?.bookId || '', message: `${task?.bookId || '任务'} 已取消；当前已完成步骤已保留。` });
        continue;
      }
  ...
  if (shouldRewrite) {
    ...
    const rewriteResults = await runWithConcurrency(fetchedIds, rewriteConcurrency, async bookId => {
      ...
      return generateAiVersions({ configStore, tasks, username, task: task?.meta, count: task?.meta?.aiCount || 1, shouldStop });
    });
    for (const [index, item] of rewriteResults.entries()) {
      ...
      if (item.value?.status === 'cancelled') {
        cancelled += 1;
        report({ type: 'rewrite', status: 'cancelled', book_id: fetchedIds[index], message: `${fetchedIds[index]} 已取消；已生成文案已保留。` });
        continue;
      }
  ...
  if (!shouldSubmit && typeof tasks.updateTaskMeta === 'function') {
