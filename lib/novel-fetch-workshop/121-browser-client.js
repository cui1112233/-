...
function normalizeWorkerSecret(value) {
  const secret = String(value || '').trim();
  if (!secret || secret === FORBIDDEN_DEV_SECRET) return '';
  return secret;
}
...
function create121BrowserClient({
  ...
  const internalSecret = normalizeWorkerSecret(secret);
...
module.exports = { unavailableError, normalizeWorkerBaseUrl, normalizeWorkerSecret, create121BrowserClient };
