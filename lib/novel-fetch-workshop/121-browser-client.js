...
function create121BrowserClient({
  ...
  timeoutMs = Number(process.env.QIANTIE_121_CLIENT_TIMEOUT_MS) || 20000,
  loginTimeoutMs = Number(process.env.QIANTIE_121_CLIENT_LOGIN_TIMEOUT_MS) || 45000
} = {}) {
  ...
  async function request(path, { method = 'POST', body, requestTimeoutMs = timeoutMs } = {}) {
    ...
    const timeout = Math.max(1000, Math.min(Number(requestTimeoutMs) || 20000, 60000));
