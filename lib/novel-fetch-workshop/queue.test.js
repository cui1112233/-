...
test('删除运行中的书籍会登记安全停止请求，不停止其他书', () => {
  const state = {
    state: 'running',
    items: [
      { id: 'running', state: 'running', payload: { task_ids: ['2054404988995215768'] }, attempts: 1 },
      { id: 'other', state: 'queued', payload: { task_ids: ['2084992034306373466'] }, attempts: 0 }
    ], events: []
  };
  const store = {
    load: () => structuredClone(state), read: () => structuredClone(state),
  ...
  assert.deepEqual(result.stopRequestedBookIds, ['2054404988995215768']);
  assert.equal(queue.isBookStopRequested('alice', '2054404988995215768'), true);
  assert.equal(queue.isBookStopRequested('alice', '2084992034306373466'), false);
  assert.equal(queue.status('alice').items.find(item => item.id === 'other').state, 'queued');
});
