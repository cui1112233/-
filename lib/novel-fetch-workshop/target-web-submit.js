...
function createTargetAwareWebSubmit({ service, accountResolver, createStore, onTaskUpdated } = {}) {
  ...
  return {
    ...
    syncOrganizations: (...args) => service.syncOrganizations(...args),
    syncStyles: (...args) => service.syncStyles(...args),
    ensureSession: (...args) => service.ensureSession(...args),
    // Batch Factory reads organization/config catalogs through the same
    // Playwright-backed session as upload. Do not expose its opaque session
    // identifier as a direct HTTP Cookie.
    workerAction: (...args) => service.workerAction(...args),
