...
function createNovelFetchQueue({ store, execute, sleep = ms => new Promise(resolve => setTimeout(resolve, ms)) } = {}) {
  ...
  async function runLoop(owner) {
    for (;;) {
      ...
      try {
        const control = {
          shouldStop: () => isStopRequested(owner),
          report: eventValue => {
            ...
            if (active?.state === 'running') active.leaseExpiresAt = leaseExpiresAt();
  ...
  function cancelBooks(owner, bookIds = []) {
    ...
    if (cancelledItemIds.length) {
      event(state, 'books_cancelled', { bookIds: [...wanted], itemIds: cancelledItemIds });
      save(owner, state);
    }
    return { ...status(owner), cancelledItemIds };
  }
  ...
  return { start, pause, resume, stop, cancelBooks, status, waitForIdle };
}
