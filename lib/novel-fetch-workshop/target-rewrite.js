...
function configStoreForTask(configStore, task) {
  const snapshot = object(task?.aiSlotMethodsSnapshot);
  if (!Object.keys(snapshot).length || !configStore || typeof configStore.getConfig !== 'function') return configStore;
  return {
    ...configStore,
    getConfig() {
      const config = object(configStore.getConfig());
      return {
        ...config,
        rewrite: {
          ...object(config.rewrite),
          ai_slot_methods: { ...snapshot }
        }
      };
    }
  };
...
function createTargetAwareAiGenerator({ generateAiVersion = defaultGenerateAiVersion } = {}) {
  ...
  async function runIndexes({ indexes, configStore, tasks, username, task, ai, explicit, shouldStop }) {
    ...
    const frozenConfigStore = configStoreForTask(configStore, task);
...
module.exports = { generatedVersions, statusFromResults, cancellationRequested, configStoreForTask, createTargetAwareAiGenerator };