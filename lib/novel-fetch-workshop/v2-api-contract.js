...
function registerNovelFetchV2Routes(router, { queue, scheduler, taskOps } = {}) {
  ...
  if (taskOps) {
    ...
    router.post('/tasks/batch-versions', async (req, res, next) => {
      try {
        const selectedVersions = normalizeSelectedVersions(req.body?.selected_versions);
        if (!selectedVersions.length) return res.status(400).json({ error: '请至少选择一个文案版本' });
        const result = await taskOps.setSelectedVersions(
          req.username,
          req.body?.ids || [],
          selectedVersions,
          normalizeAiSlotMethods(req.body?.ai_slot_methods)
        );
