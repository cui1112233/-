    ...
    if (typeof taskOps.prepareRetryPayloads === 'function') {
      router.post('/tasks/batch-retry', async (req, res) => {
        try {
          ...
          return res.json({ retried: payloads.length, failed: 0, retry_stages: retryStages, retry_stage_counts, tasks, ...(typeof throughput?.snapshot === 'function' ? { throughput: throughput.snapshot() } : {}) });
        } catch (error) {
