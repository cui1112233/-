...
function normalizeProcessPayload(body = {}) {
  ...
  source.text_model_id = normalizeTextModelId(source.text_model_id || source.textModelId);
  delete source.textModelId;
...
function queueDisplayItem(item = {}) {
  ...
  const ids = Array.isArray(payload.task_ids) ? payload.task_ids : (Array.isArray(payload.taskIds) ? payload.taskIds : []);
  return {
    id: String(item.id || ''),
    status: String(item.state || 'queued'),
    attempts: Math.max(0, Number(item.attempts) || 0),
    max_attempts: Math.max(1, Number(item.maxAttempts) || 3),
    error: String(item.error || ''),
    task_ids: ids.map(value => String(value || '').trim()).filter(Boolean),
    text_model_id: normalizeTextModelId(payload.text_model_id || payload.textModelId)
  };
}
...
function attachBatch(owner, payload = {}, batches) {
  ...
  const batch = batches.create(owner, {
    ...
    taskOrder: Array.isArray(source.task_ids) ? source.task_ids : [],
    sourceBatchId: source.source_batch_id || ''
  });
  return { ...source, batch_id: batch.id, batch_created_at: batch.createdAt };
}
...
function buildRealtimeStatus(queueState = {}, schedules = []) {
  ...
  return {
    ...
    items: items.map(queueDisplayItem),
    queue: queueState,
...
function queueItemToProcessJob(item = {}) {
  ...
  return {
    ...
    attempts: Math.max(0, Number(item.attempts) || 0),
    steps: [],
    progress: (Array.isArray(item.progress) ? item.progress : []).map(entry => ({
...
function registerNovelFetchV2Routes(router, { queue, scheduler, taskOps, batches, throughput } = {}) {
  ...
  const queueStatus = owner => ({ ...queue.status(owner), ...(typeof throughput?.snapshot === 'function' ? { throughput: throughput.snapshot() } : {}) });
  router.get('/process/queue/status', (req, res) => res.json(queueStatus(req.username)));
  router.get('/realtime/status', (req, res) => res.json(buildRealtimeStatus(queueStatus(req.username), scheduler.list(req.username))));
  ...
  router.post('/process/start', (req, res) => {
    try {
      ...
      const state = queue.start(req.username, [payload], { priority: 'interactive' });
      const items = Array.isArray(state.items) ? state.items : [];
      const itemId = Array.isArray(state.itemIds) ? state.itemIds[0] : '';
      const item = itemId ? items.find(entry => entry?.id === itemId) : items[items.length - 1];
  ...
  if (taskOps) {
    router.get('/tasks', async (req, res, next) => {
      try {
        const batch = batches && typeof batches.current === 'function' ? batches.current(req.username) : null;
        const currentBatchIds = Array.isArray(batch?.taskIds) ? batch.taskIds : [];
        return res.json({ tasks: await taskOps.list(req.username, req.query || {}, { currentBatchIds }) });
      }
    ...
    if (typeof taskOps.prepareRetryPayloads === 'function') {
      router.post('/tasks/batch-retry', async (req, res) => {
        try {
          ...
          const queued = payloads.length ? queue.start(req.username, payloads) : { accepted: 0, deduplicated: 0 };
          const tasks = typeof taskOps.list === 'function' ? await taskOps.list(req.username, req.body?.query || {}) : [];
          const retryStages = payloads.map(payload => payload.retry_stage).filter(Boolean);
          const retry_stage_counts = retryStages.reduce((counts, stage) => {
            counts[stage] = (counts[stage] || 0) + 1;
            return counts;
          }, {});
          return res.json({ retried: Number(queued.accepted) || 0, deduplicated: Number(queued.deduplicated) || 0, failed: 0, retry_stages: retryStages, retry_stage_counts, tasks, ...(typeof throughput?.snapshot === 'function' ? { throughput: throughput.snapshot() } : {}) });
        } catch (error) {
    ...
    router.post('/tasks/batch-delete-permanent', async (req, res, next) => {
      try {
        const deleted = await taskOps.permanentDelete(req.username, req.body?.ids || []);
        const queueResult = typeof queue.cancelBooks === 'function'
          ? queue.cancelBooks(req.username, deleted?.tombstoned || req.body?.ids || [])
          : { cancelledItemIds: [] };
        return res.json({
          ...deleted,
          cancelled_queue_item_ids: queueResult.cancelledItemIds || [],
          stop_requested_book_ids: queueResult.stopRequestedBookIds || []
        });
      }
      catch (error) { return next(error); }
    });
