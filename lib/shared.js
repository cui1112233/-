...
const DEFAULT_CONFIG = {
  ...
  // 内置头像：{ emoji, background }；未设置时为 null（前端回退为用户名首字）
  avatar: null,
  nickname: null,
  note: null
};
...
// ============================================================
// 用户账号表（从环境变量 / gitignored 种子文件读取，源码不含明文密码）
// ============================================================
function parseSeedAccounts(json, label) {
  let parsed;
  try {
    parsed = JSON.parse(json);
  } catch (_) {
    throw new Error(`${label} 必须是 JSON 对象（username -> password）`);
  }
  ...
  for (const [username, password] of Object.entries(parsed)) {
    if (typeof password !== 'string' || password.length === 0) {
      throw new Error(`${label} 中账号 ${username} 的密码必须是非空字符串`);
    }
...
function loadSeedAccounts() {
  const envValue = process.env.QIANTIE_SEED_ACCOUNTS;
  if (envValue && envValue.trim()) {
    return parseSeedAccounts(envValue, 'QIANTIE_SEED_ACCOUNTS');
  }
  const seedFile = path.join(SYSTEM_DIR, 'seed-accounts.json');
  if (fs.existsSync(seedFile)) {
    return parseSeedAccounts(fs.readFileSync(seedFile, 'utf8'), `种子账号文件 ${seedFile}`);
  }
  return {};
}

const USERS = loadSeedAccounts();
...
function configOwnerFor(account, accountStore) {
  if (!account || !accountStore) return account?.username;
...
function configUsernameFor(account, accountStore) {
  return configOwnerFor(account, accountStore);
}
...
module.exports = {
  ...
  getAccountStore, createAuthRuntime, parseSeedAccounts, loadSeedAccounts,
  ...
  // 配置
  readConfig, writeConfig, publicConfig, ensureReadyConfig, configOwnerFor, configUsernameFor,
  // 上游
