...
function readConfig(username) {
  ...
  try {
    ...
    return { ...merged, models: normalizeModelDirectory(savedConfig.models, merged) };
  } catch (error) {
    return { ...DEFAULT_CONFIG };
  }
}

const MODEL_KINDS = new Set(['text', 'video', 'image']);

function normalizeModelDirectory(value, legacy = DEFAULT_CONFIG) {
  const input = Array.isArray(value) ? value : [];
  const models = input.map((item, index) => {
    if (!item || typeof item !== 'object') return null;
    const kind = MODEL_KINDS.has(item.kind) ? item.kind : null;
    const id = typeof item.id === 'string' && /^[a-zA-Z0-9_-]{1,80}$/.test(item.id.trim())
      ? item.id.trim()
      : `model-${index + 1}`;
    const name = String(item.name || item.displayName || item.model || '').trim();
    if (!kind || !name) return null;
    return {
      id,
      kind,
      name: name.slice(0, 80),
      provider: String(item.provider || 'custom').trim().slice(0, 80),
      format: String(item.format || 'openai_compatible').trim().slice(0, 40),
      endpoint: String(item.endpoint || item.baseUrl || '').trim().slice(0, 500),
      model: String(item.model || name).trim().slice(0, 160),
      apiKey: typeof item.apiKey === 'string' ? item.apiKey.trim() : '',
      enabled: item.enabled !== false,
      maxDuration: Number.isFinite(Number(item.maxDuration)) ? Math.max(1, Math.min(900, Number(item.maxDuration))) : null
    };
  }).filter(Boolean);
  if (Array.isArray(value)) return models;

  const legacyModels = [];
  if (legacy?.apiKey && legacy?.model) legacyModels.push({
    id: 'legacy-text', kind: 'text', name: legacy.model, provider: legacy.provider || 'custom',
    format: 'openai_compatible', endpoint: legacy.baseUrl || '', model: legacy.model,
    apiKey: legacy.apiKey, enabled: true, maxDuration: null
  });
  if (legacy?.image?.apiKey && legacy?.image?.model) legacyModels.push({
    id: 'legacy-image', kind: 'image', name: legacy.image.displayName || legacy.image.model,
    provider: legacy.image.provider || 'custom', format: legacy.image.mode || 'openai_compatible',
    endpoint: legacy.image.baseUrl || '', model: legacy.image.model, apiKey: legacy.image.apiKey,
    enabled: true, maxDuration: null
  });
  if (legacy?.video?.apiKey) legacyModels.push({
    id: 'legacy-video', kind: 'video', name: legacy.video.displayName || '视频生成',
    provider: legacy.video.provider || 'custom', format: 'provider', endpoint: '', model: '',
    apiKey: legacy.video.apiKey, enabled: true, maxDuration: 15
  });
  return legacyModels;
}

function publicModelDirectory(value, legacy = DEFAULT_CONFIG) {
  return normalizeModelDirectory(value, legacy).map(({ apiKey, ...model }) => ({
    ...model,
...
function publicConfig(config) {
  ...
  const { apiKey: videoAPIKey, ...safeVideo } = video;
  return {
    ...safeConfig,
    models: publicModelDirectory(config.models, config),
    hasApiKey: Boolean(apiKey),
    image: { ...safeImage, hasApiKey: Boolean(imageAPIKey) },
    video: { ...safeVideo, hasApiKey: Boolean(videoAPIKey) }
  };
}
...
module.exports = {
  ...
  // 配置
  readConfig, writeConfig, publicConfig, normalizeImageConfig, normalizeVideoConfig, normalizeModelDirectory, publicModelDirectory, ensureReadyConfig,
  // 上游
