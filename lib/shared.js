...
function getVideoApiKey(config, provider) {
  ...
  if (provider === 'h3') return typeof video.h3ApiKey === 'string' ? video.h3ApiKey.trim() : '';
  if (provider === 'yd') {
    return typeof video.ydApiKey === 'string' && video.ydApiKey.trim()
      ? video.ydApiKey.trim()
...
function publicConfig(config) {
  ...
  const { apiKey: videoAPIKey, ydApiKey, h3ApiKey, ...safeVideo } = video;
  return {
    ...safeConfig,
    ...
    video: {
      ...safeVideo,
      hasApiKey: Boolean(videoAPIKey),
      ydHasApiKey: Boolean(videoAPIKey),
      h3HasApiKey: Boolean(h3ApiKey)
    }
  };
}
