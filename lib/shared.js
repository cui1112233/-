...
const { normalizeModelCatalog, publicModel, resolveCatalogModel } = require('./model-catalog');
...
function readConfig(username) {
  ...
  try {
    ...
    return {
      ...DEFAULT_CONFIG,
      ...savedConfig,
      productionRetentionDays: normalizeProductionRetentionDays(savedConfig.productionRetentionDays),
      image: normalizeImageConfig(savedConfig.image, DEFAULT_CONFIG.image),
      video: normalizeVideoConfig(savedConfig.video, DEFAULT_CONFIG.video),
      modelCatalog: normalizeModelCatalog(savedConfig.modelCatalog, savedConfig),
      modelCatalogVersion: 1,
...
function writeConfig(username, config) {
  ...
  const next = {
    ...DEFAULT_CONFIG,
    ...config,
    productionRetentionDays: normalizeProductionRetentionDays(config?.productionRetentionDays),
    image: normalizeImageConfig(config?.image, DEFAULT_CONFIG.image),
    video: normalizeVideoConfig(config?.video, DEFAULT_CONFIG.video),
    modelCatalog: normalizeModelCatalog(config?.modelCatalog, config),
    modelCatalogVersion: 1,
    pet: config?.pet || DEFAULT_CONFIG.pet,
    notifications: { ...DEFAULT_CONFIG.notifications, ...config?.notifications }
  };
  fs.writeFileSync(getUserConfigPath(username), JSON.stringify(next, null, 2), 'utf8');
}
...
function getVideoApiKey(config, provider) {
  ...
  const catalogModelId = provider === 'h3' ? 'minimax-h3-video' : provider === 'yd' ? 'yd2-mini-video' : '';
  if (catalogModelId) {
    const configuredModel = (catalog || []).find(model => model.id === catalogModelId);
    if (configuredModel) {
      const availableModel = resolveCatalogModel(catalog, catalogModelId, 'video');
      return availableModel?.credential || '';
    }
  }
  if (Number.isInteger(config?.modelCatalogVersion) && config.modelCatalogVersion >= 1) return '';
...
function publicConfig(config) {
  const { apiKey, image: rawImage, video: rawVideo, modelCatalog: rawModelCatalog, ...safeConfig } = config;
  ...
  return {
    ...safeConfig,
    modelCatalog: normalizeModelCatalog(rawModelCatalog, config).map(publicModel),
...
module.exports = {
  ...
  // 配置
  readConfig, writeConfig, publicConfig, normalizeImageConfig, normalizeVideoConfig, normalizeProductionRetentionDays, getVideoApiKey, ensureReadyConfig,
  // 上游
