...
function normalizeVideoConfig(value, fallback = DEFAULT_CONFIG.video) {
  ...
  const apiKey = typeof value?.apiKey === 'string' && value.apiKey.trim()
    ? value.apiKey.trim()
    : previous.apiKey || '';
  const modelKey = ['minimax-h3-video', 'yd2-mini-video', 'local-doubao-executor-video'].includes(value?.modelKey)
    ? value.modelKey
    : (previous.modelKey || DEFAULT_CONFIG.video.modelKey);
  return {
    provider: 'yd_video',
    displayName: '中转亚迪',
    modelKey,
...
module.exports = {
  ...
  // 配置
  readConfig, writeConfig, publicConfig, normalizeImageConfig, normalizeVideoConfig, ensureReadyConfig,
  // 上游
