...
const H3_DEFAULT_WORKFLOW = 'minimax_h3_image_audio_to_video_v2';
const H3_NO_IMAGE_WORKFLOW = 'minimax_h3_lightx2v_no_pic';
const H3_REFERENCE_WORKFLOW = 'minimax_h3_lightx2v_v5_15s';

function normalizeH3WorkflowId(value) {
  const workflowId = String(value || '').trim();
  if (!workflowId) return H3_DEFAULT_WORKFLOW;
  if (!/^[A-Za-z0-9_-]{1,120}$/.test(workflowId)) throw new Error('H3 工作流 ID 不合法');
  return workflowId;
}
