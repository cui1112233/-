...
const { resolveAgentModel } = require('./agent-catalog-selection');
...
function createTeamAgentResponder({ accountStore, memberStore, usageStore, configReader = readConfig, upstreamRequest = requestUpstream } = {}) {
  const responder = async function teamAgentResponder({ username, messages, textModelId, signal, onResolvedModel }) {
    const { access, selection } = resolveAgentModel({ accountStore, memberStore, usageStore, username, configReader }, textModelId);
    ...
    const upstream = await upstreamRequest(config, requestPayload, collectResponse, { timeoutMs: 30_000, signal });
    ...
    usageStore?.record({
      ...
      metadata: { statusCode: upstream.statusCode, usageEstimated: resolvedUsage.estimated, catalogModelId: selection.catalogId, pricingUnconfigured: selection.source === 'catalog' }
    });
    ...
    if (typeof onResolvedModel === 'function') onResolvedModel(selection);
    return content;
  };
  responder.supportsCatalogSelection = true;
  return responder;
}
