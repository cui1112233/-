...
function smartUnifiedVisualFieldText(value, key, maxLength = 600) {
  const text = typeof value === 'string' ? value.trim().replace(/\s+/g, ' ') : '';
  if (!text) throw new Error(`统一视觉字段 ${key} 不能为空`);
  if (text.length > maxLength) throw new Error(`统一视觉字段 ${key} 过长`);

  // The original H3 style.system requires trailer_style to describe a
  // *full-film* acquisition baseline, including Lens Family, main focal
  // lengths, T-stop and FPS/Shutter/EI. A bare “35mm/50mm/85mm” therefore
  // cannot be treated as a shot instruction in that one field. All other
  // smart-unified fields retain the stricter no-focal-length rule.
  const shotLevelPatterns = [
    /\b\d{1,2}:\d{2}(?:\.\d+)?\b/,
    /(?:分镜|镜头)\s*(?:[一二三四五六七八九十]+|\d+)/,
    /(?:第\s*\d+\s*(?:镜|秒)|第[一二三四五六七八九十]+镜)/
  ];
  if (key !== 'trailer_style') {
    shotLevelPatterns.unshift(/\b\d{1,3}\s*mm\b/i);
  } else {
    // Keep full-film optics legal while refusing vocabulary that makes the
    // field a concrete shot list or a single-shot composition instruction.
    shotLevelPatterns.push(
      /\b(?:shot|scene)\s*\d+\b/i,
      /(?:特写|近景|中景|全景|远景|俯拍|仰拍|过肩|跟拍|推镜|拉镜|摇镜|移镜|变焦|转场)/,
      /\b\d{1,3}\s*mm\s*(?:特写|近景|中景|全景|远景|俯拍|仰拍|过肩|跟拍|推进|拉远|摇镜|移镜|变焦)/i
    );
  }
...
function isH3StyleSystemResult(value) {
  return value && typeof value === 'object' && !Array.isArray(value)
    && ['final_genre', 'trailer_style', 'story_era'].every(key => typeof value[key] === 'string' && value[key].trim());
}

function normalizeH3StyleSystemResult(value) {
  const fields = {};
  for (const key of ['final_genre', 'genre', 'trailer_style', 'story_era', 'negative_prompt', 'picture_limit_prompt', 'quality_constraint_prompt']) {
    // H3's trailer_style is a complete full-film cinematography specification,
    // not a legacy one-line visual-prefix field. Keep the detailed model result
    // traceable while retaining the shot-level leakage checks above.
    fields[key] = smartUnifiedVisualFieldText(value[key], key, 2400);
  }
  const genre = fields.final_genre || fields.genre;
  return { fields, prompt: `${genre}；${fields.trailer_style}；${fields.story_era}。` };
}
...
function normalizeSmartUnifiedVisualStyle(value) {
	if (isH3StyleSystemResult(value)) return normalizeH3StyleSystemResult(value);
  const fields = extractSmartUnifiedVisualFields(value);
  return {
    fields,
