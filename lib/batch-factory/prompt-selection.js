...
function compatibleProtocol(item, category) {
  const format = item?.protocolLock?.format;
  return item?.protocolLock?.category === category
    && (format === 'batch-factory-preset' || format === 'batch-factory-user-prompt');
}
...
function publishedByCategory(category) {
  ...
  return presetStore.listAll('batch-factory')
    ...
    .filter(item => compatibleProtocol(item, category))
    .map(item => ({
      id: item.id,
      name: item.name,
...
function isSelectablePrompt(id, category) {
  const normalized = String(id || '').trim();
  if (!normalized) return false;
  if (publishedByCategory(category).some(item => item.id === normalized)) return true;
  const fallback = category === 'script' ? SCRIPT_PROMPTS : ASSET_PROMPTS;
  return Boolean(fallback[normalized]);
}
