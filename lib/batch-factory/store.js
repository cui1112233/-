...
function normalizeText(value, max = 50000) {
  return typeof value === 'string' ? value.trim().slice(0, max) : '';
}

function normalizeIso(value) {
  const input = typeof value === 'string' ? value.trim() : '';
  if (!input || Number.isNaN(Date.parse(input))) return '';
  return new Date(input).toISOString();
}

function normalizePresetVersions(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
  return Object.fromEntries(
    Object.entries(value)
      .map(([id, version]) => [String(id || '').trim(), normalizePositiveInteger(version)])
      .filter(([id, version]) => /^[a-zA-Z0-9][a-zA-Z0-9._-]{0,63}$/.test(id) && version)
      .slice(0, 200)
  );
}
...
function normalizeSettings(value = {}) {
  ...
  return {
    ...
    videoModelName: normalizeText(value.videoModelName, 160),
    maxVideoDuration,
    fixedSingleVideo,
    exactDuration,
    aspectRatio: value.aspectRatio === '16:9' ? '16:9' : '9:16',
    prefixMode: value.prefixMode === 'manual' ? 'manual' : 'auto',
    customPrefix: normalizeText(value.customPrefix),
    prefixEnabled: normalizeBoolean(value.prefixEnabled, true),
    style: normalizeText(value.style, 1000),
    synopsis: normalizeText(value.synopsis),
    scriptPromptPresetId: normalizeText(value.scriptPromptPresetId, 64) || 'standard-short-drama',
    assetPromptPresetId: normalizeText(value.assetPromptPresetId, 64) || 'standard-asset-extraction',
    injectCharacterPrompt: normalizeBoolean(value.injectCharacterPrompt, true),
    injectScenePrompt: normalizeBoolean(value.injectScenePrompt, true),
    injectPropPrompt: normalizeBoolean(value.injectPropPrompt, true),
    quality: normalizeText(value.quality),
    qualityEnabled: normalizeBoolean(value.qualityEnabled, true),
    restriction: normalizeText(value.restriction),
    restrictionEnabled: normalizeBoolean(value.restrictionEnabled, true),
    negative: normalizeText(value.negative),
    negativeEnabled: normalizeBoolean(value.negativeEnabled, true),
    subtitlePolicy: value.subtitlePolicy === 'allow' ? 'allow' : 'forbid-auto-dialogue-subtitle',
    systemConfigRevision: normalizeText(value.systemConfigRevision, 64),
    systemConfigLabel: normalizeText(value.systemConfigLabel, 120),
    systemConfigSyncedAt: normalizeIso(value.systemConfigSyncedAt),
    systemPresetVersions: normalizePresetVersions(value.systemPresetVersions)
  };
}
...
module.exports = {
  createBatchFactoryStore,
  normalizeSettings,
  normalizeSourceItem,
  appendActivity,
  normalizePresetVersions
};