...
function normalizeSettings(value = {}) {
  ...
  const publishInput = value.publishSettings;
  const publishSettings = publishInput && typeof publishInput === 'object' && !Array.isArray(publishInput)
    ? (() => {
      const organizationId = String(publishInput.organizationId ?? publishInput.organization_id ?? '').trim().slice(0, 160);
      const profileId = String(publishInput.profileId ?? publishInput.profile_id ?? '').trim().slice(0, 160);
      const unpackSpeedValue = Number(publishInput.unpackSpeed);
      const pitchValue = Number(publishInput.pitch);
      return {
        uploadType: publishInput.uploadType === 'individual' ? 'individual' : 'merged',
        ...(organizationId ? { organizationId } : {}),
        ...(profileId ? { profileId } : {}),
        durationMode: publishInput.durationMode === 'audio' ? 'audio' : 'speed',
        mergeSpeed: ['1.0x', '1.3x', '1.5x', '1.7x', '2.0x'].includes(String(publishInput.mergeSpeed)) ? String(publishInput.mergeSpeed) : '1.5x',
        scrollCount: Number.isInteger(Number(publishInput.scrollCount)) ? Math.min(99, Math.max(1, Number(publishInput.scrollCount))) : 3,
        generateCount: Number.isInteger(Number(publishInput.generateCount)) ? Math.min(999, Math.max(1, Number(publishInput.generateCount))) : 10,
        reuse: publishInput.reuse !== false,
        flip: publishInput.flip !== false,
        unpackSpeed: Number.isFinite(unpackSpeedValue) ? String(Math.min(2, Math.max(0.5, unpackSpeedValue))) : '1.7',
        pitch: Number.isFinite(pitchValue) ? String(Math.round(Math.min(50, Math.max(-50, pitchValue)))) : '0',
        aiHeader: publishInput.aiHeader === 'none' ? 'none' : 'final',
        uploadTxt: publishInput.uploadTxt !== false
      };
    })()
    : undefined;
  return {
    ...
    characterPromptEnabled: value.characterPromptEnabled !== false,
    scenePromptEnabled: value.scenePromptEnabled !== false,
    propPromptEnabled: value.propPromptEnabled !== false,
    qualityEnabled: value.qualityEnabled !== false,
    restrictionEnabled: value.restrictionEnabled !== false,
    subtitlePolicy: typeof value.subtitlePolicy === 'string' ? value.subtitlePolicy.trim().slice(0, 120) : '禁止自动对白字幕',
    negativeEnabled: value.negativeEnabled !== false,
...
function createBatchFactoryStore() {
  ...
  function updateBatchSettings(username, batchId, settings) {
    return updateBatch(username, batchId, batch => {
      batch.settings = normalizeSettings(settings);
      batch.items = batch.items.map(item => item.settingOverrides ? item : item);
      batch.updatedAt = now();
      return batch;
    });
  }
