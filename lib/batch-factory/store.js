...
const { normalizeProductionLineCount } = require('./production-text');
...
function normalizeSettings(value = {}) {
  ...
  const modelMax = Number(value.videoModelMaxDuration || value.maxVideoDuration);
  const videoModelMaxDuration = Number.isInteger(modelMax) && modelMax >= 1 && modelMax <= 60 ? modelMax : 10;
  const maxVideoDuration = Number.isInteger(requestedMaxDuration) && requestedMaxDuration >= 1 && requestedMaxDuration <= videoModelMaxDuration
    ? requestedMaxDuration
    : videoModelMaxDuration;
  const fixedSingleVideo = value.fixedSingleVideo === true;
  const requestedExact = Number(value.exactDuration);
  ...
  return {
    ...
    fixedSingleVideo,
    exactDuration,
    productionLineCount: normalizeProductionLineCount(value.productionLineCount),
    productionMode: value.productionMode === 'viral' ? 'viral' : 'original',
    hookReviewMode: value.hookReviewMode === 'manual' ? 'manual' : 'auto',
...
function normalizeSourceItem(item = {}, index = 0, { sourceType } = {}) {
  ...
  const productionTextOverride = String(item.productionTextOverride || '').trim();
  const title = String(item.title ?? item.bookTitle ?? `开篇 ${index + 1}`).trim().slice(0, 120);

  if (!sourceText && !txtText) throw new Error(`第 ${index + 1} 篇用于制作的小说内容为空`);
  if (sourceText.length > 120000) throw new Error(`第 ${index + 1} 篇用于制作的内容过长`);
  if (productionTextOverride.length > 120000) throw new Error(`第 ${index + 1} 篇自定义制作内容过长`);
  ...
  return {
    ...
    title,
    platform,
    sourceText: sourceText || txtText,
    productionTextOverride,
    txtText: txtText || sourceText,
...
function createBatchFactoryStore() {
  ...
  function createNovelFetchIntake(username, payload = {}) {
    ...
    if (!inputItems.length) throw new Error('至少需要一个可用的小说获取任务');
    ...
    for (const item of items) {
      if (seenBooks.has(item.bookId)) throw new Error(`书ID ${item.bookId} 在本次转入中重复`);
      if (seenTasks.has(item.sourceTaskId)) throw new Error(`小说获取任务 ${item.sourceTaskId} 在本次转入中重复`);
    ...
    const intake = {
      ...
      items,
      totalAvailable: Number.isInteger(Number(payload.totalAvailable)) ? Number(payload.totalAvailable) : items.length,
      accepted: items.length,
      skipped: Array.isArray(payload.skipped) ? payload.skipped.slice(0, 200) : [],
      createdAt: timestamp,
      consumedAt: '',
      batchId: ''
    };
  ...
  function createBatch(username, payload = {}) {
    const mode = payload.mode === 'viral' || payload.settings?.productionMode === 'viral' ? 'viral' : 'original';
    const settings = { ...normalizeSettings({ ...(payload.settings || {}), productionMode: mode }), productionMode: mode };
    const inputItems = Array.isArray(payload.items) ? payload.items : [];
    ...
    const batch = {
      ...
      items: normalizedItems.map(item => ({
        ...
        activityLog: [{
          at: timestamp,
          type: 'created',
          message: '已加入批量工厂，完整 TXT 已保留；等待按制作行数开始导演。',
          status: 'pending',
          videoId: ''
        }],
        status: 'pending',
