...
function normalizeDirectorOutput(rawValue, settings = {}) {
  ...
  const storyboard = sourceVideos.map((video, videoIndex) => {
    ...
    const id = String(video.id ?? videoIndex + 1).trim();
    if (!id) throw new Error(`storyboard[${videoIndex}] 缺少 id`);
    if (videoIds.has(id)) throw new Error(`storyboard 存在重复 VIDEO id：${id}`);
    videoIds.add(id);
    ...
    if (!visualPrompt) throw new Error(`storyboard[${videoIndex}] 缺少 visualPrompt`);
    const promptRevision = Number.isInteger(Number(video.promptRevision)) && Number(video.promptRevision) > 0 ? Number(video.promptRevision) : 1;
    const generatedRevision = Number.isInteger(Number(video.generatedRevision)) && Number(video.generatedRevision) >= 0 ? Number(video.generatedRevision) : 0;
    ...
    return {
      ...
      visualPrompt,
      video_desc: visualPrompt,
      dialogue_sound: cleanText(video.dialogue_sound || video.dialogueSound || video.sound),
      settingsOverride: video.settingsOverride && typeof video.settingsOverride === 'object' && !Array.isArray(video.settingsOverride) ? { ...video.settingsOverride } : {},
      promptRevision,
      generatedRevision
    };
  });
