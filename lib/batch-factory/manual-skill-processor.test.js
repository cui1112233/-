...
test('manual skill processor marks empty source content as a client error', async () => {
  const processor = createManualSkillProcessor({ respond: async () => 'never' });
  await assert.rejects(
    processor.preview({ username: 'alice', items: [{ title: 'A', sourceText: '' }], skillIds: [] }),
    error => error.code === 'INVALID' && /内容为空/.test(error.message)
  );
});
