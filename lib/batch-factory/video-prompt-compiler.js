...
function compileVideoPrompt({ directorResult, video, settings = {}, autoPrefix = '' }) {
  ...
  const prefixEnabled = settings.constraintPrefixEnabled !== false;
  const qualityEnabled = settings.constraintQualityEnabled !== false;
  const restrictionEnabled = settings.constraintRestrictionEnabled !== false;
  const negativeEnabled = settings.constraintNegativeEnabled !== false;
  ...
  const parts = [
    `【视频参数】\n视频时长：${durationSec}秒\n画幅：${aspectRatio}`,
    prefixEnabled ? section('整体画面前缀', prefix) : '',
    ...
    qualityEnabled ? section('画质要求', quality) : '',
    restrictionEnabled ? section('画面限制', restriction) : '',
    section('文字与字幕限制', subtitleRestriction),
    negativeEnabled ? section('负面提示词', negative) : ''
  ].filter(Boolean);
