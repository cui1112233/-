...
function createUsageStore({ systemDir } = {}) {
  ...
  function summarize(entries) {
    ...
    for (const entry of entries) {
      inputTokens += normalizeToken(entry.inputTokens);
      outputTokens += normalizeToken(entry.outputTokens);
      totalTokens += normalizeToken(entry.totalTokens);
      if (entry.usageKnown) knownUsageCalls += 1;
      if (entry.status === 'success') successfulCalls += 1;
      const feature = entry.feature || 'unknown';
      const model = entry.model || 'unknown';
      byFeature[feature] = (byFeature[feature] || 0) + normalizeToken(entry.totalTokens);
      byModel[model] = (byModel[model] || 0) + normalizeToken(entry.totalTokens);
    }
    return {
...
module.exports = { createUsageStore, normalizeUsage, monthBounds, dayBounds };
