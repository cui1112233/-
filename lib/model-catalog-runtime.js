...
function accountIsOwner({ username, accountStore, account } = {}) {
  if (account?.isOwner === true) return true;
  return accountStore?.getInternalAccount?.(username)?.isOwner === true;
}

function managerFor({ username, kind, memberStore, accountStore, account, requireScope = false }) {
  if (accountIsOwner({ username, accountStore, account })) return username;
...
function visibleCatalogModels({ username, kind, memberStore, accountStore, account, configReader = readConfig }) {
  const normalizedKind = normalizeKind(kind);
  if (!normalizedKind) return [];
  const ownerUsername = managerFor({ username, kind: normalizedKind, memberStore, accountStore, account, requireScope: true });
...
function resolveRuntimeModel({ username, kind, modelId, memberStore, accountStore, account, configReader = readConfig } = {}) {
  const normalizedKind = normalizeKind(kind);
...
function novelFetchModelError(message, code) {
  const error = new Error(message);
  error.status = 422;
  error.code = code;
  return error;
...
function resolveNovelFetchTextModel({ username, textModelId, memberStore, configReader = readConfig } = {}) {
  const modelId = String(textModelId || '').trim();
  if (!modelId) {
    throw novelFetchModelError('请先在小说获取配置中选择已启用的文本模型', 'NOVEL_FETCH_TEXT_MODEL_REQUIRED');
  }
  try {
    return resolveRuntimeModel({ username, kind: 'text', modelId, memberStore, configReader });
  } catch (_) {
    throw novelFetchModelError('小说获取选择的文本模型不可用、未配置或尚未启用', 'NOVEL_FETCH_TEXT_MODEL_UNAVAILABLE');
  }
}
