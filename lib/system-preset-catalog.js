...
const LEGACY_SEED_BODIES = require('./legacy-seed-bodies');
...
const SCRIPT_PRESETS = [
  ...
  ['script-format-qban', 'Q版模式', 'Q版视频镜头输出格式', 'Q版模式.md', { format: 'q版' }],
...
const NOVEL_FETCH_PRESETS = [
  {
    ...
    legacySeedBodies: [LEGACY_SEED_BODIES['novel-fetch-induce']],
  ...
  },
  {
    ...
    legacySeedBodies: [LEGACY_SEED_BODIES['novel-fetch-hook']],
...
function seedSystemPresets(store, actor) {
  for (const preset of SYSTEM_PRESETS) {
    const knownLegacyBodies = preset.legacySeedBodies || [];
    const published = store.listAll(preset.module)
      .find(item => item.id === preset.id && item.status === 'published');
    ...
    // 已是当前种子正文：无需升级
    if (published.body === preset.body) continue;

    // 仅当已发布正文与某代已知种子正文完全一致时才自动升级；
    // 用户手工修改过的预设绝不覆盖。
    if (knownLegacyBodies.includes(published.body)) {
      const draft = store.createDraft(actor, {
        id: preset.id,
        module: preset.module,
        name: preset.name,
