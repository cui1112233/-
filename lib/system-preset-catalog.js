...
function sanitizeResolvedPromptBody(value) {
  return String(value || '')
    .replace(/^\s*---\s*$/gm, '')
    .replace(/^\s*【基础设定】生成视频不带字幕\s*\|\s*9:16\s*$/gm, '')
    .replace(/总时长固定为\s*\{duration\}/g, '当前选择的是 10s / 15s 拆分模式，单个完整分镜按本次所选模式与实际内容确定总时长')
    .replace(/\{结束时间\}/g, '该分镜标题声明的实际结束时间')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
}
...
function resolveSystemPresetBody(store, id) {
  ...
  if (!preset) return sanitizeResolvedPromptBody(defaultBody(id));
  const addOns = store?.listAll(preset.module)
    ...
    .map(item => item.body) || [];
  return [preset.body, ...addOns]
    .map(sanitizeResolvedPromptBody)
    .filter(Boolean)
    .join('\n\n');
}
