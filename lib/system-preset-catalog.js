...
function seedSystemPresets(store, actor) {
  for (const preset of SYSTEM_PRESETS) {
    ...
    if (preset.id === 'script-constraint-prefix-smart-unified' && upgradedBody === SMART_UNIFIED_LEGACY_BODY) {
      upgradedBody = String(preset.body || '').trim();
    }
    const nextName = (renameQuickDirector || renameBatchStoryboardMeta) ? preset.name : current.name;
    const nextDescription = (renameQuickDirector || renameBatchStoryboardMeta) ? preset.description : current.description;
