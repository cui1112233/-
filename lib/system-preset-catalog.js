...
const SCRIPT_PRESETS = [
  ...
  ['script-duration-10s', '10秒时长', '10 秒分镜单元时长规则', '剧本时长10秒.md', { format: 'duration', duration: '10s' }],
  ['script-duration-15s', '15秒时长', '15 秒分镜单元时长规则', '剧本时长15秒.md', { format: 'duration', duration: '15s' }],
...
const SYSTEM_PRESET_SLOTS = Object.freeze([
  ...
  ['script.duration.10s', 'script', '10秒时长', 'primary'],
  ['script.duration.15s', 'script', '15秒时长', 'primary'],
...
const SLOT_BY_PRESET_ID = Object.freeze({
  ...
  'script-duration-10s': 'script.duration.10s',
  'script-duration-15s': 'script.duration.15s',
...
function seedSystemPresets(store, actor) {
  for (const preset of SYSTEM_PRESETS) {
    ...
    if (upgradedBody !== String(current.body || '').trim()) {
      const draft = store.createDraft(actor, {
        id: current.id,
