...
function isPreH3SmartUnifiedStyleSystemBody(body) {
  return crypto.createHash('sha256').update(String(body || '').trim()).digest('hex')
    === SMART_UNIFIED_PRE_H3_STYLE_SYSTEM_SHA256;
}
...
function seedSystemPresets(store, actor) {
  for (const preset of SYSTEM_PRESETS) {
    ...
    if (preset.id === 'batch-video-h3-director' && (upgradedBody === H3_DIRECTOR_LEGACY_BODY || upgradedBody === H3_DIRECTOR_LEGACY_BODY_V0)) {
      upgradedBody = String(preset.body || '').trim();
    }
    const replaceLegacyH3AssetBody = preset.id === 'batch-assets-h3' && isLegacyH3TwoCallAssetBody(upgradedBody);
    if (replaceLegacyH3AssetBody) {
      upgradedBody = String(preset.body || '').trim();
    }
    const replaceLegacySmartUnifiedBody = preset.id === 'script-constraint-prefix-smart-unified'
      && (upgradedBody === SMART_UNIFIED_LEGACY_BODY || isPreH3SmartUnifiedStyleSystemBody(upgradedBody));
