...
function createAccountStore({ systemDir, lockTimeoutMs, lockRetryMs }) {
  ...
  function createAccount({ username, password, active = true } = {}) {
    ...
    return withStoreLock(() => {
      ...
      const now = new Date().toISOString();
      const account = { username, passwordHash: hashPassword(password), isOwner: false, active, createdAt: now, updatedAt: now };
  ...
  function approveApplication(actorUsername, id) {
    return withStoreLock(() => {
      ...
      if (findAccount(state.accounts, application.username)) throw storeError('Account already exists', 'CONFLICT');
      const now = new Date().toISOString();
      const account = { username: application.username, passwordHash: application.passwordHash, isOwner: false, active: true, createdAt: now, updatedAt: now };
      state.accounts.push(account);
