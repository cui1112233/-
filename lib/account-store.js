...
function createAccountStore({ systemDir, lockTimeoutMs, lockRetryMs }) {
  ...
  function validateAccountAuditHistory(audit, accounts) {
    const accountsByUsername = new Map(accounts.map(account => [account.username, account]));
    const activeByCreatedAccount = new Map();
    const statusChangedBeforeCreation = new Set();
    for (const entry of audit) {
      if (entry.action === 'account.created') {
        if (activeByCreatedAccount.has(entry.target) || statusChangedBeforeCreation.has(entry.target)) {
          throw new Error('Invalid audit store');
        }
        activeByCreatedAccount.set(entry.target, entry.after.active);
      } else if (entry.action === 'account.status_changed') {
        if (!activeByCreatedAccount.has(entry.target)) {
          statusChangedBeforeCreation.add(entry.target);
          continue;
        }
        if (entry.before.active !== activeByCreatedAccount.get(entry.target)) {
          throw new Error('Invalid audit store');
        }
        activeByCreatedAccount.set(entry.target, entry.after.active);
      }
    }
    for (const [username, active] of activeByCreatedAccount) {
  ...
  function validateAudit(audit, accounts, applications, grants) {
    ...
    validateAccountAuditHistory(audit, accounts);
    return audit;
  }
  ...
  function appendAuditToState(state, actor, action, target, before, after, at = new Date().toISOString()) {
    const entry = {
  ...
  function createAccountRecord(state, { username, password, passwordHash, active = true } = {}) {
    ...
    let now = Date.now();
    const existingTimestamps = new Set(state.accounts.map(account => account.createdAt));
    while (existingTimestamps.has(new Date(now).toISOString())) now += 1;
  ...
  function createAccount({ username, password, active = true } = {}) {
    ...
    return withStoreLock(() => {
      ...
      appendAuditToState(state, PRIMARY_USER, 'account.created', account.username, null, accountSummary(account), account.createdAt);
      writeTransaction([
