...
function isSafeJsonValue(value) {
  ...
  return Object.entries(value).every(([key, item]) => key !== 'passwordHash' && isSafeJsonValue(item));
}
...
function createAccountStore({ systemDir, lockTimeoutMs, lockRetryMs }) {
  ...
  function validateGrant(grant, usernames) {
    if (!grant || typeof grant !== 'object' || !isValidId(grant.id) || !isValidTime(grant.at)
  ...
  function validateAudit(audit, accounts, grants) {
    ...
    for (const entry of audit) {
      if (!entry || typeof entry !== 'object' || !isValidId(entry.id) || !isValidTime(entry.at)
        ...
        || !isSafeJsonValue(entry.before) || !isSafeJsonValue(entry.after)) {
  ...
  return {
    ...
    grant,
    listGrants,
    listAudit: () => withStoreLock(() => readStateUnsafe().audit.map(entry => sanitizeAuditValue(clone(entry)))),
    files: { ...files }
  };
}
