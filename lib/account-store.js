...
const ACCOUNT_FIELDS = new Set(['username', 'passwordHash', 'isOwner', 'active', 'createdAt', 'updatedAt', 'source']);
...
function createAccountStore({ systemDir, lockTimeoutMs, lockRetryMs, legacySeedUsernames } = {}) {
  ...
  function ensureSeedAccounts(seedUsers = USERS) {
    return withStoreLock(() => {
      ...
      const accounts = Object.entries(seedUsers).map(([username, password]) => {
        ...
        return { username, passwordHash: hashPassword(password), isOwner: username === PRIMARY_USER, active: true, createdAt: now, updatedAt: now, source: 'seed' };
      });
  ...
  function createAccountRecord(state, { username, password, passwordHash, active = true, source } = {}) {
  ...
  function createAccount({ username, password, active = true } = {}) {
    ...
    return withStoreLock(() => {
      const state = readStateUnsafe();
      const account = createAccountRecord(state, { username, password, active, source: 'direct' });
