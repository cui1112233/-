...
const SENSITIVE_AUDIT_KEYS = new Set(['passwordhash', 'password', 'apikey', 'token', 'secret']);
...
function isSensitiveAuditKey(key) {
  return SENSITIVE_AUDIT_KEYS.has(String(key).toLowerCase());
}
...
function isSafeJsonValue(value) {
  ...
  return Object.entries(value).every(([key, item]) => !isSensitiveAuditKey(key) && isSafeJsonValue(item));
}
