...
function accountSource(account) {
  if (account.source !== undefined) return account.source;
  return Object.hasOwn(USERS, account.username) ? 'seed' : null;
}
...
function createAccountStore({ systemDir, lockTimeoutMs, lockRetryMs }) {
  ...
  function validateAccountAuditHistory(audit, accounts, applications) {
    ...
    for (const account of accounts) {
      ...
      if (source === 'approved' || (source === null && application)) {
        if (creations.length !== 0) throw new Error('Invalid audit store');
        if (!application) throw new Error('Invalid audit store');
        sourceByUsername.set(account.username, {
          entry: approvalAuditsByApplicationId.get(application.id),
          active: true
        });
      } else {
        if (source === 'direct' && application) throw new Error('Invalid audit store');
        if (creations.length !== 1) throw new Error('Invalid audit store');
        sourceByUsername.set(account.username, {
          entry: creations[0],
  ...
  function validateAudit(audit, accounts, applications, grants) {
    ...
    validateAccountAuditHistory(audit, accounts, applications);
    return audit;
  }
