...
const ACCOUNT_SOURCES = new Set(['seed', 'direct', 'approved']);
const ACCOUNT_FIELDS = new Set(['username', 'passwordHash', 'isOwner', 'active', 'createdAt', 'updatedAt', 'source', 'role', 'boundTo', 'apiGranted']);
const ROLES = new Set(['dev', 'manager', 'member']);
const SEED_MANIFEST_VERSION = 1;
const SEED_MANIFEST_FIELDS = new Set(['version', 'usernames']);
...
function createSeedManifest(usernames, label = 'seed manifest') {
  if (!Array.isArray(usernames) || usernames.length === 0) throw new Error(`Invalid ${label}`);
  const seen = new Set();
  for (const username of usernames) {
    if (!isValidUsername(username) || seen.has(username)) throw new Error(`Invalid ${label}`);
    seen.add(username);
  }
  if (!seen.has(PRIMARY_USER)) throw new Error(`Invalid ${label}`);
  return { version: SEED_MANIFEST_VERSION, usernames: [...usernames] };
}
...
// 角色判定：isOwner 强制 dev；缺省按 boundTo 存在与否归属（默认 member）。
function roleOf(account) {
  if (!account) return null;
  if (account.isOwner === true) return 'dev';
  if (ROLES.has(account.role)) return account.role;
  return 'member';
}
...
function publicAccount(account) {
  ...
  return {
    ...
    role: roleOf(account),
    boundTo: account.boundTo || null,
    apiGranted: account.apiGranted === true,
...
function isRoleSummary(value) {
  return hasOnlyFields(value, new Set(['role'])) && ROLES.has(value.role);
}

function isApiAccessSummary(value) {
  return hasOnlyFields(value, new Set(['apiGranted'])) && typeof value.apiGranted === 'boolean';
}
...
function createAccountStore({ systemDir, lockTimeoutMs, lockRetryMs, legacySeedUsernames } = {}) {
  if (typeof systemDir !== 'string' || systemDir.length === 0) {
    throw new Error('systemDir is required');
  }
  const legacySeedManifest = legacySeedUsernames === undefined
    ? null
    : createSeedManifest(legacySeedUsernames, 'legacy seed usernames');
  ...
  const files = {
    ...
    seedManifest: path.join(systemDir, 'seed-manifest.json'),
    transaction: path.join(systemDir, 'system-transaction.json'),
    lock: path.join(systemDir, 'system-store.lock')
  };
  const transactionPaths = [files.accounts, files.applications, files.grants, files.audit, files.seedManifest];
  const lockOptions = {
  ...
  function validateAccounts(accounts) {
    ...
    for (const account of accounts) {
      if (!hasOnlyFields(account, ACCOUNT_FIELDS) || !isValidUsername(account.username)
        ...
        || (account.source !== undefined && !ACCOUNT_SOURCES.has(account.source))
        || (account.role !== undefined && !ROLES.has(account.role))
        || (account.boundTo !== undefined && account.boundTo !== null && !isValidUsername(account.boundTo))
        || (account.apiGranted !== undefined && typeof account.apiGranted !== 'boolean')) {
    ...
    for (const account of accounts) {
      // 绑定管理必须存在、处于活跃状态且角色为 manager
      if (account.boundTo && !usernames.has(account.boundTo)) throw new Error('Invalid account store');
      const boundAccount = account.boundTo ? accounts.find(candidate => candidate.username === account.boundTo) : null;
      if (boundAccount && (!boundAccount.active || roleOf(boundAccount) !== 'manager')) {
        throw new Error('Invalid account store');
      }
      // 非 member 不得携带绑定关系
      if (account.boundTo && roleOf(account) !== 'member') throw new Error('Invalid account store');
    }
  ...
  function validateAuditEntry(entry, accounts, applications, grants) {
    ...
    if (entry.action === 'account.role_changed') {
      if (!isRoleSummary(entry.before) || !isRoleSummary(entry.after)
        || entry.before.role === entry.after.role
        || !usernames.has(entry.actor) || !usernames.has(entry.target)) throw new Error('Invalid audit store');
      return;
    }
    if (entry.action === 'member.api_access') {
      if (!isApiAccessSummary(entry.before) || !isApiAccessSummary(entry.after)
        || entry.before.apiGranted === entry.after.apiGranted
        || !usernames.has(entry.actor) || !usernames.has(entry.target)) throw new Error('Invalid audit store');
      return;
    }
  ...
  function validateAccountAuditHistory(audit, accounts, applications, seedManifest) {
    const accountsByUsername = new Map(accounts.map(account => [account.username, account]));
    const applicationsById = new Map(applications.map(application => [application.id, application]));
    const approvedApplicationByUsername = new Map();
    const creationAuditsByUsername = new Map();
    const approvalAuditsByApplicationId = new Map();
    for (const entry of audit) {
      if (entry.action === 'account.created') {
        const entries = creationAuditsByUsername.get(entry.target) || [];
        entries.push(entry);
        creationAuditsByUsername.set(entry.target, entries);
      }
      if (entry.action === 'application.approved') {
        if (approvalAuditsByApplicationId.has(entry.after.id)) throw new Error('Invalid audit store');
        approvalAuditsByApplicationId.set(entry.after.id, entry);
      }
    }
    for (const application of applications) {
      if (application.status !== 'approved') continue;
      if (approvedApplicationByUsername.has(application.username)) throw new Error('Invalid audit store');
      approvedApplicationByUsername.set(application.username, application);
      if (!approvalAuditsByApplicationId.has(application.id)) throw new Error('Invalid audit store');
    }

    const sourceByUsername = new Map();
    const seedUsernames = new Set(seedManifest.manifest.usernames);
    for (const account of accounts) {
      const creations = creationAuditsByUsername.get(account.username) || [];
      const application = approvedApplicationByUsername.get(account.username);
      if (seedUsernames.has(account.username)) {
        if (creations.length !== 0 || application) throw new Error('Invalid audit store');
        continue;
      }
      if (application) {
        if (creations.length !== 0) throw new Error('Invalid audit store');
        sourceByUsername.set(account.username, {
          entry: approvalAuditsByApplicationId.get(application.id),
          active: true
        });
      } else {
        if (creations.length !== 1) {
          if (!seedManifest.persisted && !seedManifest.migrating && creations.length === 0) {
            throw new Error('Missing seed manifest for legacy custom accounts; provide legacySeedUsernames');
          }
          throw new Error('Invalid audit store');
        }
        sourceByUsername.set(account.username, {
          entry: creations[0],
          active: creations[0].after.active
        });
      }
    }
    ...
    for (const entry of audit) {
      if (entry.action === 'account.created') {
        const source = sourceByUsername.get(entry.target);
        if (!source || source.entry.id !== entry.id) throw new Error('Invalid audit store');
        activeByAccount.set(entry.target, source.active);
      } else if (entry.action === 'application.approved') {
        const application = applicationsById.get(entry.after.id);
        const source = application && sourceByUsername.get(application.username);
        if (!source || source.entry.id !== entry.id) throw new Error('Invalid audit store');
        activeByAccount.set(application.username, source.active);
      } else if (entry.action === 'account.status_changed' && sourceByUsername.has(entry.target)) {
        if (!activeByAccount.has(entry.target) || entry.before.active !== activeByAccount.get(entry.target)) {
          throw new Error('Invalid audit store');
        }
        activeByAccount.set(entry.target, entry.after.active);
      }
    }
    for (const [username, source] of sourceByUsername) {
      if (!activeByAccount.has(username)) throw new Error('Invalid audit store');
      const active = activeByAccount.get(username);
      if (accountsByUsername.get(username)?.active !== active) throw new Error('Invalid audit store');
    }
  }

  function validateAudit(audit, accounts, applications, grants, seedManifest) {
    ...
    validateAccountAuditHistory(audit, accounts, applications, seedManifest);
    return audit;
  }
  ...
  function readAccountsUnsafe() {
    ...
    const accounts = validateAccounts(result.value);
    return {
      found: true,
  ...
  function validateSeedManifest(manifest, accounts) {
    if (!hasOnlyFields(manifest, SEED_MANIFEST_FIELDS) || manifest.version !== SEED_MANIFEST_VERSION) {
      throw new Error('Invalid seed manifest');
    }
    const normalized = createSeedManifest(manifest.usernames);
    const accountNames = new Set(accounts.map(account => account.username));
    if (normalized.usernames.some(username => !accountNames.has(username))) throw new Error('Invalid seed manifest');
    return normalized;
  }

  function readSeedManifestUnsafe() {
    const result = readJsonOrMissing(files.seedManifest);
    return result.found ? result.value : null;
  }

  function resolveSeedManifest(accounts, manifest) {
    if (manifest !== null) {
      return { manifest: validateSeedManifest(manifest, accounts), persisted: true, migrating: false };
    }
    if (legacySeedManifest) {
      return { manifest: validateSeedManifest(legacySeedManifest, accounts), persisted: false, migrating: true };
    }
    const accountNames = new Set(accounts.map(account => account.username));
    const defaultSeedUsernames = Object.keys(USERS);
    if (defaultSeedUsernames.length === 0) {
      throw new Error('No seed accounts configured and no seed manifest present; set QIANTIE_SEED_ACCOUNTS or provide data/system/seed-accounts.json');
    }
    if (defaultSeedUsernames.some(username => !accountNames.has(username))) {
      throw new Error('Missing seed manifest for legacy custom accounts; provide legacySeedUsernames');
    }
    return {
      manifest: createSeedManifest(defaultSeedUsernames),
      persisted: false,
      migrating: true
    };
  }
  ...
  function readStateUnsafe() {
    ...
    const manifest = readSeedManifestUnsafe();
    if (!accountsState.found) {
      if (applications.length !== 0 || grants.length !== 0 || audit.length !== 0 || manifest !== null) throw new Error('Invalid account store');
      return { accounts: [], applications, grants, audit, seedManifest: null };
    }
    const seedManifest = resolveSeedManifest(accountsState.accounts, manifest);
    validateApplications(applications, accountsState.accounts);
    validateGrants(grants, accountsState.accounts);
    validateAudit(audit, accountsState.accounts, applications, grants, seedManifest);
    return { accounts: accountsState.accounts, applications, grants, audit, seedManifest };
  }

  function validateJournalWrites(journal) {
    if (!journal || !Array.isArray(journal.writes) || journal.writes.length < 1 || journal.writes.length > 5) {
    ...
    for (const write of journal.writes) {
      ...
      const key = Object.entries(files).find(([, value]) => path.resolve(value) === resolved)?.[0];
      if (!key || key === 'transaction' || key === 'lock'
        || !transactionPaths.some(allowed => path.resolve(allowed) === resolved) || paths.has(resolved)
        || (key === 'seedManifest' ? !isPlainObject(write.value) : !Array.isArray(write.value))) {
        throw new Error('Invalid JSON transaction journal');
      }
      paths.add(resolved);
    }
    const writesSeedManifest = paths.has(path.resolve(files.seedManifest));
    const writesAccounts = paths.has(path.resolve(files.accounts));
    const isSeedInitialization = !auditWrite && journal.writes.length === 2 && writesAccounts && writesSeedManifest;
    const isManifestMigration = !auditWrite && journal.writes.length === 1 && writesSeedManifest;
    if ((!auditWrite && !isSeedInitialization && !isManifestMigration)
      || (auditWrite && (!Array.isArray(auditWrite.value) || writesSeedManifest))) {
      throw new Error('Invalid JSON transaction journal');
    }
    const accountsState = readAccountsUnsafe();
    const current = {
      ...
      seedManifest: readSeedManifestUnsafe()
    };
    for (const write of journal.writes) {
    ...
    validateAudit(
      ...
      current.grants,
      resolveSeedManifest(current.accounts, current.seedManifest)
    );
  }
  ...
  function ensureSeedAccounts(seedUsers = USERS) {
    return withStoreLock(() => {
      ...
      if (Object.keys(seedUsers).length === 0) return false; // 未配置种子账号：跳过，不写入任何文件
      const seedManifest = createSeedManifest(Object.keys(seedUsers));
      ...
      const accounts = Object.entries(seedUsers).map(([username, password]) => {
        ...
        const isOwner = username === PRIMARY_USER;
        return {
          username, passwordHash: hashPassword(password), isOwner, active: true,
          role: isOwner ? 'dev' : 'member', boundTo: null, apiGranted: false,
          createdAt: now, updatedAt: now, source: 'seed'
        };
      });
      validateAccounts(accounts);
      validateSeedManifest(seedManifest, accounts);
      writeTransaction([
        { filePath: files.accounts, value: accounts },
        { filePath: files.seedManifest, value: seedManifest }
      ]);
      return true;
    });
  }

  function createAccountRecord(state, { username, password, passwordHash, active = true, source, role, boundTo = null, apiGranted = false } = {}) {
    ...
    if (!ACCOUNT_SOURCES.has(source) || source === 'seed') throw storeError('Invalid account source');
    if (findAccount(state.accounts, username)) throw storeError('Account already exists', 'CONFLICT');
    const resolvedRole = role === undefined || !ROLES.has(role) ? 'member' : role;
    if (resolvedRole !== 'member' && boundTo) throw storeError('Only members can be bound to a manager');
    if (boundTo && !isValidUsername(boundTo)) throw storeError('Invalid bound manager');
    if (boundTo) {
      const boundAccount = findAccount(state.accounts, boundTo);
      if (!boundAccount || !boundAccount.active || roleOf(boundAccount) !== 'manager') {
        throw storeError('Bound manager must be an active manager account');
      }
    }
  ...
  function createAccount({ username, password, active = true, role, boundTo, apiGranted } = {}) {
    if (!isValidUsername(username)) throw storeError('Invalid username');
    if (!isValidNewPassword(password)) throw storeError('Password must be at least 8 characters');
    if (typeof active !== 'boolean') throw storeError('Active status must be boolean');
    return withStoreLock(() => {
      const state = readStateUnsafe();
      const account = createAccountRecord(state, { username, password, active, source: 'direct', role, boundTo, apiGranted });
  ...
  // 开发设置账号等级（manager / member）。PRIMARY_USER 不可改动。
  function setRole(actorUsername, username, role) {
    if (!isValidUsername(username)) throw storeError('Invalid username');
    if (!ROLES.has(role) || role === 'dev') throw storeError('Invalid role');
    return withStoreLock(() => {
      const state = readStateUnsafe();
      ...
      if (account.username === PRIMARY_USER || account.isOwner) throw storeError('Owner role is immutable');
      const before = roleOf(account);
      if (before === role) return publicAccount(account);
      if (before === 'manager' && role === 'member') {
        const dependents = state.accounts.filter(candidate => candidate.boundTo === account.username);
        if (dependents.length !== 0) {
          throw storeError('Cannot demote manager with bound members; unbind members first', 'CONFLICT');
        }
      }
      account.role = role;
      if (role !== 'member') account.boundTo = null;
      account.updatedAt = new Date().toISOString();
      appendAuditToState(state, actor.username, 'account.role_changed', account.username, { role: before }, { role });
      writeTransaction([
        { filePath: files.accounts, value: state.accounts },
        { filePath: files.audit, value: state.audit }
      ]);
  ...
  // 授权/禁止组员调用模型：actor 必须是开发，或该组员绑定的管理。
  function setApiGranted(actorUsername, username, granted) {
    if (!isValidUsername(username)) throw storeError('Invalid username');
    if (typeof granted !== 'boolean') throw storeError('Invalid grant value');
    return withStoreLock(() => {
      const state = readStateUnsafe();
      const actor = findAccount(state.accounts, actorUsername);
      if (!actor || !actor.active) throw storeError('Actor must be an active account', 'FORBIDDEN');
      const isOwner = actor.username === PRIMARY_USER && actor.isOwner;
      if (!isOwner && roleOf(actor) !== 'manager') throw storeError('Forbidden', 'FORBIDDEN');
      const account = findAccount(state.accounts, username);
      if (!account) throw storeError('Account not found', 'NOT_FOUND');
      if (roleOf(account) !== 'member') throw storeError('Only members can be granted API access');
      if (!isOwner && account.boundTo !== actor.username) throw storeError('Not your member', 'FORBIDDEN');
      const before = account.apiGranted === true;
      if (before === granted) return publicAccount(account);
      account.apiGranted = granted;
      account.updatedAt = new Date().toISOString();
      appendAuditToState(state, actor.username, 'member.api_access', account.username, { apiGranted: before }, { apiGranted: granted });
      writeTransaction([
        { filePath: files.accounts, value: state.accounts },
        { filePath: files.audit, value: state.audit }
      ]);
  ...
  // 团队视图：开发返回全部账号，管理返回自己名下的组员。
  function listTeam(actorUsername) {
    return withStoreLock(() => {
      const state = readStateUnsafe();
      const actor = findAccount(state.accounts, actorUsername);
      if (!actor || !actor.active) throw storeError('Forbidden', 'FORBIDDEN');
      const isOwner = actor.username === PRIMARY_USER && actor.isOwner;
      if (!isOwner && roleOf(actor) !== 'manager') {
        return { team: [], viewer: publicAccount(actor) };
      }
      const team = isOwner
        ? state.accounts.map(publicAccount)
        : state.accounts.filter(candidate => candidate.boundTo === actor.username).map(publicAccount);
      return { team, viewer: publicAccount(actor) };
    });
  }
  ...
  withStoreLock(() => {
    const state = readStateUnsafe();
    if (state.seedManifest?.migrating) {
      writeTransaction([{ filePath: files.seedManifest, value: state.seedManifest.manifest }]);
    }
  });
