...
function h3WorkflowPath(workflow) {
  return `/api/v1/comfyui/comfyui_workflow/${normalizeH3WorkflowId(workflow)}`;
}

function buildH3Request({ workflowId, prompt, duration, resolution, referenceImages } = {}) {
  const requestedImages = usableReferenceImages(referenceImages);
  const workflow = workflowId
    ? normalizeH3WorkflowId(workflowId)
    : (requestedImages.length ? H3_DEFAULT_WORKFLOW : H3_NO_IMAGE_WORKFLOW);
  const images = workflow === H3_NO_IMAGE_WORKFLOW ? [] : requestedImages;
