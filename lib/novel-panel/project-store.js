  ...
  function migrateLegacyIfNeeded(username) {
    ...
    return withUserMutationLock(user, lockedUser => {
      ...
      for (const candidate of candidates) {
        const grouped = candidatesByContent.get(candidate.contentIdentity) || [];
        grouped.push(candidate);
        candidatesByContent.set(candidate.contentIdentity, grouped);
        if (!seen.has(candidate.sourceKey)) continue;
        const mappedId = importedSources[candidate.sourceKey];
        const mappedIds = importedIdsByContent.get(candidate.contentIdentity) || new Set();
        mappedIds.add(mappedId);
        importedIdsByContent.set(candidate.contentIdentity, mappedIds);
      }

      for (const [contentIdentity, grouped] of candidatesByContent) {
        const mappedIds = importedIdsByContent.get(contentIdentity);
        if (!mappedIds || !mappedIds.size) continue;
        const originalIds = [...new Set(grouped.map(candidate => candidate.project.id))].sort();
        const canonicalId = originalIds.find(id => mappedIds.has(id))
          || originalIds.find(id => loadProjectUnlocked(lockedUser, id))
          || [...mappedIds].sort()[0];
        for (const candidate of grouped) {
          if (seen.has(candidate.sourceKey) && importedSources[candidate.sourceKey] !== canonicalId) {
            importedSources[candidate.sourceKey] = canonicalId;
            reconciled += 1;
          }
        }
        importedIdsByContent.set(contentIdentity, canonicalId);
      }
      ...
      if (imported > 0 || reconciled > 0 || marker.source_fingerprint !== sourceFingerprint) {
        const nextMarker = {
      ...
      return { imported, reconciled, sourceFingerprint };
    });
  }
