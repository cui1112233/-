...
function parseTosObjectUrl(value, config) {
  let parsed;
  try {
    parsed = new URL(text(value));
  } catch (_) {
    throw new Error('TOS 图片地址无效');
  }
  const endpointHost = normalizeTosSdkEndpoint(config?.endpoint || '');
  const bucketHost = String(config?.bucket || '') + '.' + endpointHost;
  if (parsed.protocol !== 'https:' || ![endpointHost, bucketHost].includes(parsed.host)) {
    throw new Error('不支持读取该图片地址');
  }
  const key = decodeURIComponent(parsed.pathname.replace(/^\/+/, ''));
  if (!key.startsWith('reference-assets/')) throw new Error('TOS 对象地址无效');
  return key;
}
...
function createTosReferenceAssetStore({ env = process.env, client } = {}) {
  ...
  return { config: { endpoint: config.endpoint, region: config.region, bucket: config.bucket }, keyFor, put, putFile, getSignedUrl, parseObjectUrl: value => parseTosObjectUrl(value, config) };
}

module.exports = { DEFAULT_REQUEST_TIMEOUT_MS, configFromEnv, createTosReferenceAssetStore, extensionFor, normalizeTosSdkEndpoint, parseTosObjectUrl, safeSegment };
