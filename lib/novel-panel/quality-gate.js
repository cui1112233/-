...
function collectCoveredLines(result, total, stats) {
  ...
  outlineShots(result).forEach(shot => {
    const shotCovered = new Set();
    const shotStats = { explicitMappingCount: 0, invalidSourceIndexCount: 0, invalidSourceReferenceCount: 0, ambiguousSourceBasisCount: 0 };
    addSourceIndexCoverage(shot && shot.source_index, shotCovered, total, shotStats);
    collectLineReferenceCoverage(shot && (shot.source_lines || shot.sourceLines || shot.source_line || shot.line_number), shotCovered, total, shotStats);
    collectLineReferenceCoverage(shot && (shot.start_line && shot.end_line ? [shot.start_line + '-' + shot.end_line] : []), shotCovered, total, shotStats);
    list(shot && shot.timeline_segments).forEach(segment => {
      addSourceIndexCoverage(segment && segment.source_index, shotCovered, total, shotStats);
      collectLineReferenceCoverage(segment && (segment.source_lines || segment.sourceLines || segment.source_line || segment.line_number), shotCovered, total, shotStats);
    });
    shotCovered.forEach(line => covered.add(line));
    stats.explicitMappingCount += shotStats.explicitMappingCount;
    stats.invalidSourceIndexCount += shotStats.invalidSourceIndexCount;
    if (!shotCovered.size) stats.invalidSourceReferenceCount += shotStats.invalidSourceReferenceCount;
  });
  return covered;
}
...
function validateSingleShotSourceReference(shot, context) {
  const report = createReport();
  const sourceInfo = sourceLineInfo(context);
  const sourceLineCount = sourceInfo.sourceLineCount;
  const result = { outline_shots: [shot] };

  report.metrics.shotCount = 1;
  report.metrics.sourceLineCount = sourceLineCount;
  if (sourceInfo.invalidMetadata) {
    addIssue(report, 'blockers', 'invalid_source_reference', '原文行数元数据无效或与实际原文行数不一致，不能应用 AI 结果。');
    return report;
  }
  if (!shot || typeof shot !== 'object') {
    addIssue(report, 'blockers', 'invalid_source_reference', '分镜不是有效对象，无法确认其原文依据。');
    return report;
  }
  if (!sourceLineCount) {
    addIssue(report, 'blockers', 'invalid_source_reference', '当前没有可验证的原文行，不能应用单条分镜。');
    return report;
  }

  const coverageStats = {
    explicitMappingCount: 0,
    invalidSourceIndexCount: 0,
    invalidSourceReferenceCount: 0,
    ambiguousSourceBasisCount: 0
  };
  const covered = collectCoveredLines(result, sourceLineCount, coverageStats);
  collectCoveredLinesBySourceBasis(result, context, coverageStats).forEach(line => covered.add(line));
  report.metrics.invalidSourceIndexCount = coverageStats.invalidSourceIndexCount;
  report.metrics.invalidSourceReferenceCount = coverageStats.invalidSourceReferenceCount;
  report.metrics.ambiguousSourceBasisCount = coverageStats.ambiguousSourceBasisCount;
  report.metrics.coveredSourceLineCount = covered.size;
  if (coverageStats.invalidSourceReferenceCount) {
    addIssue(report, 'blockers', 'invalid_source_reference', 'AI结果包含无效、越界或过深的原文行引用。', {
      count: coverageStats.invalidSourceReferenceCount
    });
  ...
  if (coverageStats.ambiguousSourceBasisCount) {
    addIssue(report, 'blockers', 'ambiguous_source_basis', 'AI结果的 source_basis 对应多条相同原文，不能作为唯一依据。', {
      count: coverageStats.ambiguousSourceBasisCount
    });
  }
  if (!coverageStats.explicitMappingCount || !covered.size) {
    addIssue(report, 'blockers', 'missing_source_reference', '分镜缺少可精确映射到原文的行引用或 source_basis。');
  }
  return report;
}
