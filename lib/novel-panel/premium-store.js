...
function createNovelPanelPremiumStore({ usersDir } = {}) {
  ...
  function nextReferenceAssetId(username, assetType, assetId, variant = 'main') {
    const safeId = safeAssetId(assetId);
    const safeVariant = safeAssetVariant(variant);
    const isOccupied = candidate => Array.from(ASSET_VARIANTS).some(candidateVariant => assetFilePath(username, assetType, candidate, candidateVariant));
    if (!isOccupied(safeId)) return safeId;
    const suffixBase = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    const prefix = safeId.slice(0, Math.max(1, 120 - suffixBase.length - 1));
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const candidate = safeAssetId(`${prefix}-${suffixBase}${attempt ? `-${attempt}` : ''}`);
      if (!isOccupied(candidate)) return candidate;
    }
    throw new Error('参考图片资产ID生成失败，请稍后重试');
  }
  ...
  function deleteReferenceAsset(username, assetType, assetId, variant) {
    const user = assertValidUsername(username);
    const safeType = safeAssetType(assetType);
    const safeId = safeAssetId(assetId);
    const safeVariant = safeAssetVariant(variant);
    const directory = referenceAssetsDir(user, safeType);
    ...
    for (const entry of fs.readdirSync(directory)) {
      ...
      try {
        ...
        // Keep the entity state unchanged if the file could not be removed.
      }
    }
    if (safeVariant === 'main') {
      const thumbStem = `${safeId}_thumb`;
      for (const entry of fs.readdirSync(directory)) {
        if (!entry.startsWith(`${thumbStem}.`)) continue;
        try {
          fs.unlinkSync(path.join(directory, entry));
        } catch (_) {
          // Best effort cleanup of the derived thumbnail.
        }
      }
    }
    return deleted;
