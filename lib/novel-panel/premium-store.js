    ...
    if (fs.existsSync(directory)) {
      const matches = fs.readdirSync(directory)
        .filter(entry => {
          ...
          const isExactVariant = entry.startsWith(`${stem}.`);
          const isSourceRevision = safeVariant === 'source' && entry.startsWith(`${safeId}_source_`);
          return isImage && (isExactVariant || isSourceRevision);
        })
