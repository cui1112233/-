...
const { createTosReferenceAssetStore } = require('./tos-reference-assets');
...
function safeAssetVariant(value) {
  ...
  if (!ASSET_VARIANTS.has(normalized) && !/^(?:source|candidate)_[0-9]{10,}_[a-f0-9]{6,64}$/.test(normalized)) throw new Error('无效参考资产图片版本');
  return normalized;
}
...
function createNovelPanelPremiumStore({ usersDir, tosStore } = {}) {
  if (typeof usersDir !== 'string' || !usersDir.trim()) throw new Error('usersDir is required');
  const resolvedUsersDir = path.resolve(usersDir);
  let resolvedTosStore = tosStore;
  if (resolvedTosStore === undefined) {
    try {
      resolvedTosStore = createTosReferenceAssetStore();
    } catch (error) {
      // Keep local storage available when an optional TOS configuration is incomplete.
      console.warn(`[TOS 参考图] 配置未启用：${error.message}`);
      resolvedTosStore = null;
    }
  }
  ...
  function assetFilePath(username, assetType, assetId, variant) {
    ...
    const candidates = fs.readdirSync(directory)
      .filter(entry => {
        ...
        return entry.startsWith(`${stem}.`) || (safeVariant === 'source' && entry.startsWith(`${safeId}_source_`));
      })
      .map(entry => ({ path: path.join(directory, entry), mtimeMs: fs.statSync(path.join(directory, entry)).mtimeMs }))
  ...
  async function syncReferenceAssetToTos(username, assetType, assetId, variant) {
    if (!resolvedTosStore) return null;
    const user = assertValidUsername(username);
    const safeType = safeAssetType(assetType);
    const safeId = safeAssetId(assetId);
    const safeVariant = safeAssetVariant(variant);
    const filePath = assetFilePath(user, safeType, safeId, safeVariant);
    if (!filePath) return null;
    const uploaded = await resolvedTosStore.putFile({
      username: user,
      assetType: safeType,
      assetId: safeId,
      variant: safeVariant,
    ...
    return { ...uploaded, url: resolvedTosStore.getSignedUrl(uploaded.key) };
  }

  function queueReferenceAssetTosSync(username, assetType, assetId, variant) {
    if (!resolvedTosStore) return false;
    void syncReferenceAssetToTos(username, assetType, assetId, variant).catch(error => {
      console.warn(`[TOS 参考图] 异步同步失败：${error.message}`);
    });
    return true;
  }

  async function syncReferenceAssetUrlToTos(username, value) {
    if (!resolvedTosStore) return null;
    let parsed;
    try { parsed = new URL(String(value || ''), 'http://qiantie.internal'); } catch { return null; }
    const match = /^\/api(?:\/novel-panel)?\/reference-assets\/file\/(character|scene|prop)\/([A-Za-z0-9_-]{1,120})\/([A-Za-z0-9_-]{1,180})$/.exec(parsed.pathname);
    if (!match) return null;
    const result = await syncReferenceAssetToTos(username, match[1], match[2], match[3]);
    return result?.url || null;
  }

  function resolveLegacyTosAssetUrl(username, value) {
    if (!resolvedTosStore) throw new Error('TOS 参考图服务未配置');
    const user = assertValidUsername(username);
    const key = resolvedTosStore.parseObjectUrl(value);
    if (!key.startsWith('reference-assets/' + encodeURIComponent(user).replace(/%/g, '~') + '/')) {
      throw new Error('旧图片不属于当前账号');
    }
    return resolvedTosStore.getSignedUrl(key, 300);
  }
  ...
  function writeReferenceAssetRevision(username, assetType, assetId, baseVariant, payload, mime = 'image/png') {
    const base = String(baseVariant || '').trim().toLowerCase();
    if (!['source', 'candidate'].includes(base)) throw new Error('无效参考资产修订类型');
    const revision = `${base}_${Date.now()}_${crypto.randomBytes(6).toString('hex')}`;
    return { revision, filePath: writeReferenceAssetBytes(username, assetType, assetId, revision, payload, mime) };
  }
  ...
  function referenceAssetResponseUrl(assetType, assetId, variant) {
    return referenceAssetPublicUrl(assetType, assetId, variant, false);
  }
  ...
  return {
    ...
    referenceAssetResponseUrl,
    referenceAssetsDir,
    resolveLegacyTosAssetUrl,
    queueReferenceAssetTosSync,
    syncReferenceAssetToTos,
    syncReferenceAssetUrlToTos,
    safeAssetId,
    safeAssetType,
    safeAssetVariant,
    writeImageSettings,
    writeReferenceAssetBytes,
    writeReferenceAssetRevision
  };
}
