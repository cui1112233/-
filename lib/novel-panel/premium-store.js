...
function createNovelPanelPremiumStore({ usersDir } = {}) {
  ...
  function assetFilePath(username, assetType, assetId, variant) {
    ...
    if (fs.existsSync(directory)) {
      const matches = fs.readdirSync(directory)
        .filter(entry => entry.startsWith(`${stem}.`) && ['.png', '.jpg', '.jpeg', '.webp'].includes(path.extname(entry).toLowerCase()))
        .map(entry => ({ entry, mtimeMs: fs.statSync(path.join(directory, entry)).mtimeMs }))
        .sort((left, right) => right.mtimeMs - left.mtimeMs);
      if (matches[0]) return path.join(directory, matches[0].entry);
    }
    if (safeVariant === 'source' && fs.existsSync(directory)) {
      const prefix = `${safeAssetId(assetId)}_source_`;
      const revisions = fs.readdirSync(directory)
        .filter(entry => entry.startsWith(prefix) && ['.png', '.jpg', '.jpeg', '.webp'].includes(path.extname(entry).toLowerCase()))
        .map(entry => ({ entry, mtimeMs: fs.statSync(path.join(directory, entry)).mtimeMs }))
        .sort((left, right) => right.mtimeMs - left.mtimeMs);
      if (revisions[0]) return path.join(directory, revisions[0].entry);
    }
    return null;
  }
  ...
  function writeReferenceAssetRevision(username, assetType, assetId, baseVariant, payload, mime = 'image/png') {
    const normalizedBase = String(baseVariant || 'source').trim().toLowerCase() === 'candidate' ? 'candidate' : 'source';
    const variant = `${normalizedBase}_${Date.now()}_${crypto.randomBytes(6).toString('hex')}`;
    const filePath = writeReferenceAssetBytes(username, assetType, assetId, variant, payload, mime);
