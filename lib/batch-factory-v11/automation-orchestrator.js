...
function createBatchFactoryAutomationController({ adapter, statePath = defaultStatePath(), pollMs = 2500, now = Date.now, logger = console, dispatcherConcurrency = 2 } = {}) {
  ...
  function deferOrFailBook(job, book, stage, label, error) {
    ...
    const delay = AUTOMATION_RETRY_DELAYS_MS[retryCount];
    if (!retryableAutomationError(error) || !Number.isFinite(delay)) {
      setBook(job, book, { status: 'failed', stage, message: `${label}失败`, error: detail, retryRequested: false, retryAt: '' });
      return;
    }
  ...
  async function runJob(job) {
    ...
    try {
      ...
      const candidates = books.filter(book => bookCanRun(job, book, now())).sort((left, right) => bookPriority(job, left) - bookPriority(job, right));
      const batchSlice = candidates.slice(0, automationConcurrency(job.concurrency, activeBookLimit));
  ...
  async function resume({ owner, batchId } = {}) {
    ...
    if (['paused', 'needs_attention', 'completed'].includes(job.state)) {
      for (const value of Object.values(job.books || {})) {
        const retryable = value.status === 'failed' || value.status === 'blocked' || (value.status === 'waiting' && value.retryAt);
        if (job.state === 'completed' && value.status !== 'ready') {
  ...
  async function removeBook({ owner, batchId, bookId } = {}) {
    const job = jobs.get(jobKey(owner, batchId));
    if (!job || !job.books?.[bookId]) return publicJob(job);
