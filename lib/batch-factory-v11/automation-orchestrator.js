...
function createBatchFactoryAutomationController({ adapter, statePath = defaultStatePath(), pollMs = 2500, now = Date.now, logger = console } = {}) {
  ...
  function loadPersisted() {
    try {
      ...
      for (const [key, value] of Object.entries(object(parsed?.jobs))) {
        ...
        job.concurrency = clampInteger(job.concurrency, 1, 4, 2);
  ...
  async function advanceBook(job, batch, book, productionStatus, mergeStatus) {
    ...
    if (merged.succeeded && job.runMode === 'full_submit') {
      ...
      await persist();
      try {
        const uploaded = await adapter.publishBook({ owner: job.owner, isOwner: job.isOwner, batchId: job.batchId, bookId: book.id, batch, book });
  ...
  async function runJob(job) {
    ...
    try {
      ...
      const batchSlice = candidates.slice(0, job.concurrency);
  ...
  async function start({ owner, isOwner = false, batchId, scheduledAt = '', concurrency = 2, autoPublish = false, runMode = '', preset = null, configSnapshot = null } = {}) {
    ...
    const job = {
      ...
      scheduledAt: when,
      concurrency: clampInteger(concurrency, 1, 4, 2),
      createdAt: timestamp,
