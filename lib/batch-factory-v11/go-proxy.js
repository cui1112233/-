...
async function proxyV11Request(req, res, { goBaseUrl, bridgeSecret, fetchImpl = globalThis.fetch, now = Date.now, transformJSONResponse } = {}) {
  ...
  if (req.v11TextRuntimeModel) {
    headers['X-Qiantie-V11-Text-Endpoint'] = req.v11TextRuntimeModel.baseUrl || '';
    headers['X-Qiantie-V11-Text-Model'] = req.v11TextRuntimeModel.modelId || req.v11TextRuntimeModel.id || '';
    headers['X-Qiantie-V11-Text-Key'] = req.v11TextRuntimeModel.credential || '';
  }
  ...
  if (typeof transformJSONResponse === 'function' && String(responseType || '').includes('application/json')) {
    try {
      const transformed = transformJSONResponse(JSON.parse(payload.toString('utf8')));
      payload = Buffer.from(JSON.stringify(transformed), 'utf8');
    } catch (_) {
      // The trusted proxy must preserve an upstream response if it is not valid JSON.
    }
  }
