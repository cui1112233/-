...
async function uploadAiHeadVideo({ directClient, cookie, workerAction, owner, filename, bytes }) {
  const requestBody = Buffer.from(JSON.stringify({ files: [{ name: filename, md5: target.md5Buffer(bytes) }], kind: 'ai_head_video', source: 'zbooklist_upload_ai_head' }));
  const presign = workerAction
    ? await workerAction(owner, 'asset_presign', { contentType: 'application/json', bodyBase64: requestBody.toString('base64') })
    : await directClient.action({
      cookie,
      method: 'POST',
...
async function readBackTargetBook({ directClient, cookie, workerAction, owner, bookId, fields, advanced }) {
  try {
    const response = workerAction
      ? await workerAction(owner, 'book_list', { bookId, page: 1, pageSize: 10 })
      : await directClient.action({
        cookie,
        method: 'GET',
        path: `${new URL(target.buildTargetBookListUrl(bookId)).pathname}${new URL(target.buildTargetBookListUrl(bookId)).search}`
      });
...
function createBatchFactory121Publisher({ loadBatch, loadMergeStatus, loadProductionStatus = async () => ({ jobs: [] }), fetchMedia, getSession, directClient, workerAction, onProgress = async () => {}, now = () => new Date() } = {}) {
  ...
  if (!directClient?.uploadPresigned) throw new Error('V11 121 publisher requires AI-head upload support');
  if (typeof workerAction !== 'function' && (typeof getSession !== 'function' || !directClient?.verify || !directClient?.action)) throw new Error('V11 121 publisher requires the shared direct 121 session');
  ...
  async function submit(owner, { batchId, bookId } = {}, submission = {}) {
    ...
    try {
      ...
      const session = typeof workerAction === 'function' ? null : await getSession(owner);
      if (!workerAction) await directClient.verify({ cookie: session?.cookie });
      await report('ai_head', '正在上传 AI 前贴视频');
      const aiHeadVideo = await uploadAiHeadVideo({ directClient, cookie: session?.cookie, workerAction, owner, filename: toVideoFilename(sourceBookId), bytes });
      ...
      const response = workerAction
        ? await workerAction(owner, 'upload', { contentType: `multipart/form-data; boundary=${multipart.boundary}`, bodyBase64: multipart.body.toString('base64') })
        : await directClient.action({
