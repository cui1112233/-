...
test('storage state saves and reloads with private file permissions', () => {
  const store = createSessionStore({ rootDir: tempRoot() });
...
test('corrupt storage state is quarantined instead of reused', () => {
  ...
  const store = createSessionStore({ rootDir: root, clock });
