...
export function IssueLogPage() {
  ...
  return <section className="utility-page issue-log-page">
    ...
    {entries.length ? <List className="issue-log-list" loading={loading} dataSource={entries} renderItem={entry => <List.Item>
      <List.Item.Meta
        ...
        description={<div className="issue-log-detail"><span>{formatTime(entry.at)}</span>{entry.path ? <span>{entry.path}</span> : null}{entry.method ? <span>{entry.method}{entry.status ? ` ${entry.status}` : ''}</span> : null}</div>}
      />
    </List.Item>} /> : <Empty description={loading ? '正在读取问题日志' : '暂时没有问题日志'} />}
  </section>;
}
