...
function ConstraintEditor({ title, value, placeholder, onChange, disabled = false }) {
  ...
  return (
    <Popover
      ...
      placement="left"
      content={
        <div className="bf-settings-popover">
          <strong>{title}</strong>
          <Input.TextArea
            rows={5}
            value={value || ''}
...
function VersionCard({ catalog, local, onSelect, onSync }) {
  ...
  return (
    <div className="bf-settings-version-card">
      <div className="bf-settings-version-head">
        <div>
          <strong>配置版本</strong>
          <small>冻结当前批次使用的后台批量工厂 Prompt 版本，后台后续发布不会静默改变本批次。</small>
        </div>
      ...
      <Select
        ...
        options={versions.map((snapshot) => ({
          ...
          label: `${snapshot.label}${snapshot.publishedAt ? ` · ${formatSnapshotDate(snapshot.publishedAt)}` : ''}`
        }))}
      />
      <div className="bf-settings-version-meta">
        <span>当前：{local.systemConfigLabel || '未冻结'}</span>
        {local.systemConfigSyncedAt ? <span>同步：{formatSnapshotDate(local.systemConfigSyncedAt)}</span> : null}
      </div>
      <Button
        icon={<RefreshCw size={14} />}
        disabled={!latest || upToDate}
        onClick={onSync}
      >
        同步批量后台配置
      </Button>
...
export function UnifiedProductionSettingsModal({ open, settings, onClose, onSave, saving }) {
  ...
  useEffect(() => {
    ...
    getBatchFactoryPromptCatalog()
      .then((result) => {
        ...
        setCatalog(result);
        setLocal((current) => {
          if (current.systemConfigRevision || !result.latestConfig) return current;
          return { ...current, ...snapshotFields(result.latestConfig) };
        });
      })
  ...
  return (
    <Modal title="生产统一设置" open={open} onCancel={onClose} width={720} footer={null} destroyOnClose>
      <div className="bf-settings-modal">
        ...
        <section className="bf-settings-section">
          ...
          <div className="bf-settings-two-col">
            <label>视频模型
              <div className="bf-settings-readonly">{local.videoModelName || '未绑定模型'} · 最大 {local.maxVideoDuration || '—'}s</div>
            </label>
            <label>视频画幅
              <Select
                value={local.aspectRatio || '9:16'}
                onChange={(value) => set('aspectRatio', value)}
                options={[{ value: '9:16', label: '9:16（竖屏）' }, { value: '16:9', label: '16:9（横屏）' }]}
              />
            </label>
            <label>剧本 Prompt
              <Select
                value={local.scriptPromptPresetId || 'standard-short-drama'}
                onChange={(value) => set('scriptPromptPresetId', value)}
                options={(catalog?.scriptPrompts || []).map((item) => ({ value: item.id, label: `${item.name} · v${item.version}` }))}
              />
            </label>
            <label>人物 / 场景 Prompt
              <Select
                value={local.assetPromptPresetId || 'standard-asset-extraction'}
                onChange={(value) => set('assetPromptPresetId', value)}
                options={(catalog?.assetPrompts || []).map((item) => ({ value: item.id, label: `${item.name} · v${item.version}` }))}
              />
            </label>
          </div>
          <div className="bf-settings-row">
            <div className="bf-settings-row-copy">
              <strong>固定单 VIDEO</strong>
              <small>开启后整段输入最终只输出 1 个 VIDEO；VIDEO 内仍可包含多个 shot，时长严格等于模型当前上限。</small>
            </div>
          ...
          <div className="bf-settings-two-col">
            <label>前缀模式
              <Select
                value={local.prefixMode || 'auto'}
                onChange={(value) => set('prefixMode', value)}
                options={[{ value: 'auto', label: 'AI 自动判断 + 可追加统一前缀' }, { value: 'manual', label: '只使用统一手动前缀' }]}
              />
            </label>
            <label>字幕策略
              <Select
                value={local.subtitlePolicy || 'forbid-auto-dialogue-subtitle'}
                onChange={(value) => set('subtitlePolicy', value)}
                options={[
                  { value: 'forbid-auto-dialogue-subtitle', label: '禁止自动对白字幕' },
                  { value: 'allow', label: '允许视频模型生成字幕' }
                ]}
              />
            </label>
          </div>
        ...
        <section className="bf-settings-section">
          ...
          <ConstraintRow
            label="画面前缀词"
            description={local.prefixMode === 'manual' ? '统一手动前缀会直接加入每个 VIDEO。' : 'AI 会先判断题材前缀，再追加这里的统一前缀。'}
            enabled={local.prefixEnabled !== false}
            onEnabledChange={(checked) => set('prefixEnabled', checked)}
            value={local.customPrefix || ''}
            onValueChange={(value) => set('customPrefix', value)}
            placeholder="例如：电影级动漫短剧，强情绪表演，动态镜头……"
          />
          <ConstraintRow
            label="人物 Prompt"
            description="把当前 VIDEO 引用的人物设定动态注入视频模型。"
            enabled={local.injectCharacterPrompt !== false}
            onEnabledChange={(checked) => set('injectCharacterPrompt', checked)}
          />
          <ConstraintRow
            label="场景 Prompt"
            description="把当前 VIDEO 引用的场景设定动态注入视频模型。"
            enabled={local.injectScenePrompt !== false}
            onEnabledChange={(checked) => set('injectScenePrompt', checked)}
          />
          <ConstraintRow
            label="道具 Prompt"
            description="把当前 VIDEO 引用的道具设定动态注入视频模型。"
            enabled={local.injectPropPrompt !== false}
            onEnabledChange={(checked) => set('injectPropPrompt', checked)}
          />
          <ConstraintRow
            label="画质约束"
            description="只在编译最终 VIDEO Prompt 时注入，不写死进剧情描述。"
            enabled={local.qualityEnabled !== false}
            onEnabledChange={(checked) => set('qualityEnabled', checked)}
            value={local.quality || ''}
            onValueChange={(value) => set('quality', value)}
            placeholder="8K超清、电影级光影、高细节……"
          />
          <ConstraintRow
            label="画面限制"
            description="限制水印、变形、无关文字等画面问题。"
            enabled={local.restrictionEnabled !== false}
            onEnabledChange={(checked) => set('restrictionEnabled', checked)}
            value={local.restriction || ''}
            onValueChange={(value) => set('restriction', value)}
            placeholder="禁止无关文字、禁止畸形手部、禁止画面水印……"
          />
          <ConstraintRow
            label="负面提示词"
            description="作为最终视频请求的负面约束动态注入。"
            enabled={local.negativeEnabled !== false}
            onEnabledChange={(checked) => set('negativeEnabled', checked)}
            value={local.negative || ''}
            onValueChange={(value) => set('negative', value)}
            placeholder="低清、模糊、畸形、重复人物……"
          />
        </section>
  ...
  return (
    <div className="bf-settings-row">
      <div className="bf-settings-row-copy">
        <strong>{label}</strong>
        <small>{description}</small>
        <OverrideState overridden={overridden} onRestore={() => onRestore(field)} />
      </div>
...
function ScopedConstraintRow({ label, description, enabledField, textField, parent, local, onSet, onRestore, placeholder }) {
  ...
  const overridden = enabledOverride || textOverride;
  ...
  return (
    <div className="bf-settings-row">
      <div className="bf-settings-row-copy">
        <strong>{label}</strong>
        <small>{description}</small>
        <OverrideState overridden={overridden} onRestore={() => { onRestore(enabledField); onRestore(textField); }} />
      </div>
      <div className="bf-settings-row-actions">
        ...
        <ConstraintEditor
          title={label}
          value={textValue || ''}
          onChange={(value) => onSet(textField, value)}
          placeholder={placeholder}
...
function ScopedSettingsEditor({ parentSettings, override, onChange, inheritKeys, setInheritKeys }) {
  ...
  return (
    <>
      <div className="bf-settings-two-col">
        <label>视频画幅
          <Select
            value={hasOwn(local, 'aspectRatio') ? local.aspectRatio : 'inherit'}
            onChange={(value) => value === 'inherit' ? restore('aspectRatio') : setField('aspectRatio', value)}
            options={[
              { value: 'inherit', label: `继承 · ${parent.aspectRatio}` },
              { value: '9:16', label: '覆盖为 9:16' },
              { value: '16:9', label: '覆盖为 16:9' }
            ]}
          />
        </label>
        <label>前缀模式
          <Select
            value={hasOwn(local, 'prefixMode') ? local.prefixMode : 'inherit'}
            onChange={(value) => value === 'inherit' ? restore('prefixMode') : setField('prefixMode', value)}
            options={[
              { value: 'inherit', label: `继承 · ${parent.prefixMode === 'manual' ? '手动' : 'AI自动'}` },
              { value: 'auto', label: '覆盖为 AI 自动' },
              { value: 'manual', label: '覆盖为统一手动' }
            ]}
          />
        </label>
      </div>
      ...
      <ScopedConstraintRow
        label="画面前缀词"
        description="只覆盖这一层的前缀开关/附加词。"
        enabledField="prefixEnabled"
        textField="customPrefix"
        parent={parent}
        local={local}
        onSet={setField}
        onRestore={restore}
        placeholder="当前层专用画面前缀……"
      />
      ...
      <div className="bf-settings-two-col">
        <label>字幕策略
          <Select
            value={hasOwn(local, 'subtitlePolicy') ? local.subtitlePolicy : 'inherit'}
            onChange={(value) => value === 'inherit' ? restore('subtitlePolicy') : setField('subtitlePolicy', value)}
            options={[
              { value: 'inherit', label: `继承 · ${parent.subtitlePolicy === 'allow' ? '允许字幕' : '禁止自动对白字幕'}` },
              { value: 'forbid-auto-dialogue-subtitle', label: '覆盖为禁止自动对白字幕' },
              { value: 'allow', label: '覆盖为允许字幕' }
            ]}
          />
        </label>
      </div>
...
function LayeredSettingsModal({ title, subtitle, open, parentSettings, initialOverride, onClose, onSave, saving, extraHeader }) {
  ...
  return (
    <Modal title={title} open={open} onCancel={onClose} width={660} footer={null} destroyOnClose>
      <div className="bf-settings-modal">
        <div className="bf-settings-layer-head">
          <div>
            <strong>{subtitle}</strong>
            <small>只保存真正修改过的字段；“恢复继承”会删除 override，之后继续跟随上一级变化。</small>
          </div>
          {extraHeader}
        </div>
...
export function VideoSettingsOverrideModal({ open, video, item, batchSettings, onClose, onSave, saving }) {
  const bookSettings = useMemo(
    () => mergePromptSettings(batchSettings, item?.settingsOverride || {}),
    [batchSettings, item?.settingsOverride]
  );
