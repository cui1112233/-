...
// Backward-compatible name for the first workspace prototype. New code should
// prefer DEFAULT_WORKSPACE_LAYOUT so the persisted schema is explicit.
export const DEFAULT_LAYOUT = DEFAULT_WORKSPACE_LAYOUT;
...
export function mergeWorkspaceDefaults(input, defaults = DEFAULT_WORKSPACE_LAYOUT) {
  ...
  for (const id of REQUIRED_CARD_IDS) {
    items[id] = normalizeItem(id, sourceItems[id], defaultItems);
  }
  ...
  return {
    // V1 is a 12-column schema. Persisted data cannot silently change the grid.
    columns: GRID_COLUMNS,
    rowHeight: rowHeightCandidate > 0 && rowHeightCandidate <= 200
      ? rowHeightCandidate
      : (defaults?.rowHeight || DEFAULT_ROW_HEIGHT),
...
export function moveItem(layout, id, position = {}) {
  return moveWorkspaceItem(layout, id, position.x, position.y);
}
...
export function resizeItem(layout, id, size = {}) {
  return resizeWorkspaceItem(layout, id, size.w, size.h);
}
...
export function setItemHidden(layout, id, hidden) {
  return setWorkspaceItemHidden(layout, id, hidden);
}
