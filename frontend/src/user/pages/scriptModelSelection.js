...
export function loadScriptModelSelection(storage, username) {
  try {
    ...
    return { textModelId: String(value?.textModelId || ''), imageModelId: String(value?.imageModelId || '') };
  } catch { return { textModelId: '', imageModelId: '' }; }
}

export function saveScriptModelSelection(storage, username, selection = {}) {
  try {
    storage?.setItem(preferenceKey(username), JSON.stringify({ textModelId: String(selection.textModelId || ''), imageModelId: String(selection.imageModelId || '') }));
    return true;
  } catch { return false; }
}

export function reconcileScriptModelSelection(selection = {}, modelsByKind = {}) {
  const modelIds = kind => new Set((Array.isArray(modelsByKind[kind]) ? modelsByKind[kind] : [])
    .filter(model => model?.enabled !== false)
    .map(model => String(model?.id || '').trim()).filter(Boolean));
  const textModelId = String(selection.textModelId || '').trim();
  const imageModelId = String(selection.imageModelId || '').trim();
  const textIds = modelIds('text');
  const imageIds = modelIds('image');
