...
export function resolveShotVideoDuration({ shotText, fallbackDuration }) {
  ...
  if (!Number.isFinite(duration)) return { ok: false, error: '时长必须是有效数字' };
  if (duration > 15) return { ok: false, error: '超过 H3 单次 15 秒上限' };
  if (!Number.isInteger(duration)) return { ok: false, error: '时长必须是整数' };
  if (duration < 1) return { ok: false, error: '时长至少 1 秒' };
