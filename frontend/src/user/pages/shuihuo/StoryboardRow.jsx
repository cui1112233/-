import { ArrowUpOutlined, CloudUploadOutlined, DeleteOutlined, EditOutlined, FullscreenOutlined, PictureOutlined, PlayCircleOutlined, PlusOutlined, ReloadOutlined, ScissorOutlined, SettingOutlined, UserOutlined, VideoCameraOutlined } from '@ant-design/icons';
...
function MediaPreview({ media, audioRef }) {
...
export function StoryboardRow({ index, segment, assets, speakerOptions = [], media, sourceUnitIds = [], voiceSettings, voiceLabel, imageReady, imageUnavailableReason, videoReady, videoUnavailableReason, audioReady, audioUnavailableReason, narrationGenerating, onDataChange, onRefresh, onConfigureVoice, onChangeSpeaker, onExpandField, onBindAssets, onUploadMedia, onGenerateNarration, onTask, onDelete, onInsertAfter }) {
  ...
  return <div className="shuihuo-workbench-row" role="row">
    ...
    <div className="shuihuo-workbench-cell shuihuo-library-cell" role="cell"><button className="shuihuo-primary-media" type="button" title={primaryImage ? '主图' : '上传主图'} onClick={() => primaryImage ? undefined : onUploadMedia('image', segment)}>{primaryImage ? <MediaPreview media={primaryImage} /> : <><CloudUploadOutlined /><span>上传主图</span></>}</button><div className="shuihuo-media-grid" aria-label="图片候选">{candidateMedia.map(item => <button className="shuihuo-media-tile" type="button" key={item.id} title="设为主图" aria-label={`将候选图 ${item.id}设为主图`} onClick={() => selectPrimaryImage(item.id)}><MediaPreview media={item} /></button>)}{Array.from({ length: Math.max(0, 4 - candidateMedia.length) }).map((_, itemIndex) => <button className="shuihuo-media-tile is-empty" type="button" key={`empty-${itemIndex}`} title="上传图片或视频" onClick={() => onUploadMedia('image', segment)}><PictureOutlined /></button>)}</div></div>
    <div className="shuihuo-workbench-cell shuihuo-row-actions" role="cell"><Tooltip title={imageReady ? '创建该分镜的真实图片任务' : imageUnavailableReason || '图片生成当前不可用'}><Button type="text" size="small" icon={<PictureOutlined />} disabled={!imageReady} onClick={() => onTask('image', segment.id)}>重生图</Button></Tooltip><Tooltip title={videoTaskTitle}><Button type="text" size="small" icon={<VideoCameraOutlined />} disabled={!canCreateVideo} onClick={() => onTask('video', segment.id)}>生成视频</Button></Tooltip><Popconfirm title="删除此分镜？原文单元与已生成媒体不会被删除。" onConfirm={() => mutate(() => onDelete(segment.id), '已删除分镜')}><Button type="text" size="small" danger icon={<DeleteOutlined />} disabled={busy}>删除分镜</Button></Popconfirm><Button type="text" size="small" icon={<PlusOutlined />} disabled={busy} onClick={() => onInsertAfter(segment)}>后方新增</Button></div>
  </div>;
}
