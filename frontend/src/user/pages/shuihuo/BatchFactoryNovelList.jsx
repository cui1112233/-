    ...
    <div className="shuihuo-workbench-table batch-factory-workbench-table" role="table" aria-label="批量工厂小说生产表">
      ...
      {books.map((book, index) => {
        ...
        return <article className="shuihuo-workbench-row batch-factory-book-row" key={book.id} role="row">
          ...
          <div className="shuihuo-workbench-cell batch-factory-book-config-cell"><button type="button" className="batch-factory-book-config-card" onClick={() => setConfigBook(book)}><SettingOutlined /><span><b>单书配置</b><small>{Object.keys(book.settingsState?.patch || {}).length ? '已覆盖当前书' : '继承当前批量作品'}</small></span></button><p>模型、VIDEO 时长与 AI 推理</p></div>
          ...
          <div className="shuihuo-workbench-cell batch-factory-actions"><Tag color={state.tone}>{state.label}</Tag>{state.manual ? <Tag color="purple">已手调</Tag> : null}<span>{state.detail}</span><Button type="link" size="small" onClick={() => setViewingBook(book)}>查看资料</Button></div>
        </article>;
      })}
