...
export default function TeamPage() {
  ...
  async function load({ silent = false } = {}) {
    ...
    try {
      const [nextCenter, nextMeta, nextGovernance] = await Promise.all([getMemberCenter(), getTeamMeta(), getTeamGovernance().catch(() => ({ teams: [] }))]);
      const nextManager = nextCenter.member?.role === 'dev'
        ? (managerUsername || nextMeta.teams?.[0]?.manager?.username || '')
        : (nextCenter.member?.username || '');
      const [nextTeam, inviteResult] = await Promise.all([nextManager ? getTeamMembers(nextManager) : { members: [] }, nextManager ? getTeamInvites(nextManager) : { invites: [] }]);
  ...
  async function changeManager(value) {
    ...
    try {
      const [nextTeam, result] = await Promise.all([getTeamMembers(value), getTeamInvites(value)]);
      setTeamResult(nextTeam);
  ...
  async function saveMember(values) {
    ...
    try {
      await createTeamMember({
        ...
        apiEnabled: Boolean(activeManager?.username) && (values.apiScopes || []).length > 0,
        apiScopes: activeManager?.username ? (values.apiScopes || []) : []
      });
      ...
      <div className="ac-team-reference-bottom">
        <Panel title="成员列表" className="ac-team-members-panel" action={<span className="ac-panel-count">共 {memberCount} 位成员</span>}>
          <div className="ac-reference-table"><div className="ac-reference-table-head"><span>成员</span><span>显示名称</span><span>角色</span><span>账号状态</span><span>在线状态</span><span>API 范围</span><span>月度额度</span><span>已用 Tokens</span><span>操作</span></div>
            {members.filter(member => member.role !== 'manager').map(member => { const status = memberStatus(member); const scopes = scopesOf(member); return <div className="ac-reference-table-row" key={member.username}><MemberIdentity member={member} size={30} showUsername /><strong>{member.displayName}</strong><RoleBadge role={member.role} compact /><Tag color={status.color}>{status.text}</Tag><b className={`ac-account-status-pill ${member.presence?.online ? 'is-online' : 'is-offline'}`}><i />{member.presence?.online ? '在线' : '离线'}</b><span>{scopes.length ? scopes.map(scope => <Tag key={scope}>{SCOPE_OPTIONS.find(item => item.value === scope)?.label || scope}</Tag>) : '未授权'}</span><span>{member.monthlyTokenLimit === null ? '不限额' : `${formatTokens(member.monthlyTokenLimit)} Tokens`}</span><strong>{formatTokens(member.usage?.month?.totalTokens)}</strong><Tooltip title="管理成员"><Button type="text" icon={<MoreHorizontal size={18} />} loading={saving} onClick={() => openMemberManage(member)} /></Tooltip></div>; })}
