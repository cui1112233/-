...
test('duration does not split an already delimited card after generation', () => {
  ...
  const cards = getShotCardsWithinDuration('shortdrama', output, '10s');
  assert.equal(cards.length, 2);
  assert.match(cards[0], /总时长：12s/);
  assert.match(cards[0], /00:00-00:06/);
  assert.match(cards[0], /00:06-00:12/);
});

test('continuous timeline without explicit shot headings is not auto-split into cards', () => {
  const legacy = `00:00-00:12 | 中景｜动作连续`;
  assert.deepEqual(splitContinuousTimeline(legacy, 10), []);
});
