...
test('runtime state exposes catalog and treats last Go-ordered entry as latest without sorting ids', () => {
  const catalog = [
  ...
  assert.equal(state.latestConfigVersion.id, 'a-last-from-go');
});
...
test('batch-only config sync preserves sparse false empty string and zero and never calls child override endpoints', async () => {
  ...
  await adapter.saveDrawer({
    scope: 'batch',
    batchId: 'b1',
    patch: { configVersion: 'cfg-current', enabled: false, empty: '', zero: 0, inherit: undefined },
    revision: 9
  });
  assert.deepEqual(calls, [[
    'batch',
    'b1',
    { patch: { configVersion: 'cfg-current', enabled: false, empty: '', zero: 0 }, expectedRevision: 9 }
  ]]);
});

test('production settings drawer uses only server catalog and exposes controlled sync action', () => {
  ...
  assert.match(source, /latestConfigVersion/);
...
test('ui page passes runtime catalog to drawer and config sync saves only the batch configVersion patch', () => {
  const source = read('BatchFactoryV11UiPage.jsx');
  assert.match(source, /runtimeState\.configVersions/);
  assert.match(source, /runtimeState\.latestConfigVersion/);
