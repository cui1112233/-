...
function countTags(impact) {
  ...
  if (impact.affectedVideos !== null) tags.push(<Tag key="videos">影响 VIDEO {impact.affectedVideos}</Tag>);
...
export function ChangeImpactNotice({ result = null, loading = false }) {
  if (loading) {
    return <Alert
      ...
      showIcon
      message="正在读取变更影响"
      description="影响范围由 Batch Factory V11 Go 服务计算。"
    />;
  }
  ...
  if (result.ok !== true) {
    return <Alert
      ...
      showIcon
      message="无法读取影响"
      description={result.message || 'V11 change-impact 暂时不可用；前端不会生成本地替代结果。'}
    />;
  }
  ...
  return <Alert
    ...
    description={<Space direction="vertical" size={6}>
      ...
      {impact.invalidatesDirector === true ? <Tag color="orange">保存后 Director revision 失效</Tag> : null}
      {impact.invalidatesDirector === false ? <Tag color="green">Director revision 保持有效</Tag> : null}
