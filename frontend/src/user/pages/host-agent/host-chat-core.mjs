...
export function createHostClient(apiRequest) {
  ...
  return {
    ...
    send: ({ taskId, prompt, context, skillIds = [], signal }) => apiRequest('/api/agent/chat', { method: 'POST', body: JSON.stringify({ taskId, prompt, context, skillIds }), signal, suppressGlobalError: true })
  };
}
