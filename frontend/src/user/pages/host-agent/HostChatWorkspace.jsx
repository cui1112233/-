...
import { Alert, Button, Drawer, Input, Modal, Space, Tag, Typography, theme } from 'antd';
import { apiRequest, getToken } from '../../../shared/api/client';
import { buildPrompt, createHostClient, defaultModelLabel, getInitialTask, normalizeTask, readableFailure, replyTargetIsCurrent, selectExistingSkill, sourceContext, taskAfterReply, taskURL } from './host-chat-core.mjs';
...
export default function HostChatWorkspace({ onProduction, onClassic }) {
  ...
  async function reloadCatalog() {
    ...
    const [list, config, skillList] = await Promise.allSettled([api.catalog(), api.config(), api.listSkills()]);
  ...
  async function selectTask(id) {
    ...
    control.current.id = id; setLoading(true); setTask(null); setArtifact(null); setFailure(''); setUncertain(false); setHistory(false);
  ...
  function fresh() {
    if (control.current.sending) return;
    ++control.current.epoch; control.current.id = ''; setActive(null); setArtifact(null); setText(''); setAttachment(null); setFailure(''); setUncertain(false); setLoading(false);
  }
  useEffect(() => {
    ...
    const expired = () => { ++control.current.epoch; requestAbort.current?.abort(); control.current.id = ''; control.current.sending = false; setTask(null); setTasks([]); setCatalog([]); setSkills([]); setArtifact(null); setBusy(false); setFailure('请通过原系统主导航重新登录。'); };
  ...
  async function send() {
    ...
    try {
      ...
      submitted = true;
      const result = await api.send({ taskId: currentTask.id, prompt, context: sourceContext(window.location.pathname, hadAttachment), skillIds: selectExistingSkill(originalText, skills), signal: ac.signal });
      if (!still(captured)) return;
  ...
  return <section className={`hc-native ${artifact ? 'hc-native--art' : ''} ${!task?.messages?.length && !loading && !busy && !artifact ? 'hc-native--empty' : ''}`} style={styles}>
    ...
    <header className="hc-native__toolbar"><div><b>聊天创作</b><span> · 原系统账号与会话</span></div><Space size={4}>
      ...
      <Button type="text" onClick={() => setDetail(true)}>接入状态</Button>
    </Space></header>
      ...
      <div className="hc-native__conversation">
        ...
        <div className="hc-native__bottom">
          ...
          <div className="hc-native__composer">
            ...
            <div className="hc-native__composer-tools"><Space size={4}><Button type="text" disabled={busy} aria-label="附加原文" onClick={() => fileInput.current?.click()}>＋ 原文</Button><Button type="text" className="hc-native__model-label" onClick={() => { setModelsOpen(true); reloadCatalog(); }}>{modelLabel} ⌄</Button></Space><Button type="primary" shape="round" loading={busy} disabled={loading || uncertain || !text.trim()} onClick={send}>发送 ↑</Button></div>
            <input ref={fileInput} type="file" accept=".txt,.md,text/plain,text/markdown" hidden onChange={attach} />
          </div>
          <p className="hc-native__note">本轮沿用原 Agent 默认模型，可能产生原平台文本调用费用。图片、视频与上传仍在原生产工作区执行。</p>
        </div>
      </div>
    ...
    <Drawer title="原后台模型" open={modelsOpen} onClose={() => setModelsOpen(false)} width={440}>
      <Alert type="info" showIcon message="不用重新填写 API Key" description="下面只读取原后台授权目录。现有 Agent 请求未携带可选模型 ID，因此本次聊天仍由原后台默认配置决定；这里不提供无效的切换按钮。" />
      <Typography.Paragraph style={{ marginTop: 16 }}>{modelLabel}</Typography.Paragraph>{catalog.map(c => <div key={c.kind}><h3>{KIND[c.kind]}模型</h3>{c.error ? <p>{c.error}</p> : c.models.length ? c.models.map(m => <Tag key={m.id}>{m.name}</Tag>) : <p>原接口没有返回可见模型。</p>}</div>)}
      <Button onClick={reloadCatalog}>重新读取目录</Button>
    </Drawer>
    <Drawer title="本次接入状态" open={detail} onClose={() => setDetail(false)} width={480}><Alert type="info" message="已切换前端入口，接口状态以原后台返回为准" description="页面使用原导航、主题、登录请求和会话接口。不连接 18089，不修改原模型配置。" /><h3>已接通的接口</h3><p>原 Agent 对话、原会话历史、原后台模型目录。发送成功仅表示得到真实文本回复。</p><h3>没有接上的部分</h3><p>新版 Go 前贴多步执行、从聊天生成图片/视频、局部重绘与上传，不在本次前端入口补丁中。原生产区仍保留原有功能；不会把它们显示成聊天已经自动执行。</p><h3>模型与原文边界</h3><p>当前只使用原后台默认对话模型，目录不是切换器。单轮需求与附件限制 5800 文本单位，超过会拒绝发送，不截断原文。原接口的历史上下文窗口仍由原服务决定。</p>{onClassic && <Button onClick={() => { setDetail(false); onClassic(); }} disabled={busy}>打开原 Agent 界面</Button>}<p>已有文字回复：{replies.length} 条</p></Drawer>
  </section>;
}
