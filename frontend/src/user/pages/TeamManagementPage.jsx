...
import { BarChart3, ChevronRight, Download, KeyRound, PauseCircle, Plus, RotateCcw, Search, ShieldCheck, UserPlus, UsersRound } from 'lucide-react';
...
export default function TeamManagementPage() {
  ...
  async function load({ quiet = false } = {}) {
    ...
    try {
      const [nextCenter, teamResult, governanceResult] = await Promise.all([
        getMemberCenter(),
        getTeamMembers(),
        getTeamGovernance().catch(() => ({ teams: [] }))
      ]);
  ...
  async function openMember(member) {
    ...
    memberForm.setFieldsValue({
      ...
      apiScopes: explicitScopes(member)
    });
    try {
  ...
  async function saveMember(values) {
    ...
    try {
      ...
      if ((updated.member?.role || selected.role) === 'member' && selected.active) {
        await setTeamMemberApiScopes(selected.username, Array.isArray(values.apiScopes) ? values.apiScopes : []);
      }
  ...
  async function saveTeamLimit(username) {
    setSaving(true);
    try {
      await updateTeamGovernance(username, teamLimits[username] ?? null);
      message.success('团队总额度已更新');
      await load({ quiet: true });
    } catch (error) {
      message.error(error.message || '团队额度更新失败');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function changeStatus(active) {
    ...
    setSaving(true);
    try {
      const result = await setTeamMemberStatus(selected.username, active);
      message.success(active ? '账号已恢复' : '账号已停用，登录会话已撤销');
  ...
  async function resetPassword(values) {
    ...
    setSaving(true);
    try {
      await resetTeamMemberPassword(selected.username, values.password);
      message.success('密码已重置，该成员其他登录会话已退出');
  ...
  function exportUsage() {
    const rows = [['username', 'displayName', 'role', 'boundTo', 'active', 'apiScopes', 'monthTokens', 'monthCalls', 'estimatedCost', 'currency']];
    for (const item of team) rows.push([
      ...
      explicitScopes(item).join('|'),
      item.usage?.month?.totalTokens || 0,
      item.usage?.month?.calls || 0,
      item.usage?.month?.estimatedCost ?? '',
      item.usage?.month?.costCurrency || ''
    ]);
  ...
  return <div className="account-center-page team-management-page">
    <PageHeader title="团队管理" subtitle="管理成员账号、分级 API 权限、个人额度与团队总额度" actions={<>
      <Button icon={<Download size={16} />} onClick={exportUsage}>导出用量</Button>
      <Button type="primary" icon={<UserPlus size={16} />} onClick={() => {
        createForm.resetFields();
        createForm.setFieldsValue({ role: 'member', apiScopes: ['text'] });
        setCreateOpen(true);
      }}>添加成员</Button>
    </>} />

    <div className="ac-metrics-grid four">
      <MetricCard label="成员总数" value={members.length} suffix="人" icon={UsersRound} accent="violet" hint={`${members.filter(item => item.active).length} 个正常账号`} />
      <MetricCard label="已授权成员" value={authorized} suffix="人" icon={UserPlus} accent="blue" hint={`${members.length ? Math.round((authorized / members.length) * 100) : 0}% 有 API 权限`} />
    ...
    <section className="ac-panel ac-governance-panel">
      <div className="ac-panel-header"><div><small>TEAM QUOTA · 70 / 90 / 100</small><h2>团队总额度与预警</h2></div><Tag color="gold">自然月</Tag></div>
      <div className="ac-governance-list">
        {governance.map(item => {
          const username = item.manager?.username || item.managerUsername;
          const tone = quotaTone(item.quota);
          return <div className="ac-governance-row" key={username}>
            <MemberIdentity member={item.manager || { username, displayName: username, role: 'manager' }} size={38} />
            <div className="ac-governance-progress">
              <div><span>{formatTokens(item.usage?.totalTokens)} / {item.monthlyTokenLimit === null ? '不限额' : formatTokens(item.monthlyTokenLimit)}</span><Tag color={tone.color}>{tone.text}</Tag></div>
              <Progress percent={item.quota?.percent || 0} showInfo={false} status={item.quota?.level === 'exhausted' ? 'exception' : 'normal'} />
            </div>
            <InputNumber min={0} max={100_000_000_000} value={teamLimits[username]} onChange={value => setTeamLimits(current => ({ ...current, [username]: value }))} placeholder="不限额" addonAfter="Tokens" />
            <Button loading={saving} onClick={() => saveTeamLimit(username)}>保存</Button>
          </div>;
        })}
        {!governance.length ? <div className="ac-empty">暂无 MANAGER 团队</div> : null}
      </div>
    </section>
      ...
      <section className="ac-panel ac-team-table-panel">
        <div className="ac-team-toolbar">
          ...
          <Select value={roleFilter} onChange={setRoleFilter} options={[
            { label: '全部角色', value: 'all' }, { label: 'DEV', value: 'dev' }, { label: 'MANAGER', value: 'manager' }, { label: 'MEMBER', value: 'member' }
          ]} />
          <Select value={apiFilter} onChange={setApiFilter} options={[
            { label: '全部 API 状态', value: 'all' }, { label: 'API 已启用', value: 'on' }, { label: 'API 已暂停', value: 'off' }
          ]} />
        </div>

        <div className="ac-team-table">
          <div className="ac-team-table-head"><span>成员</span><span>角色</span><span>绑定管理</span><span>账号 / API</span><span>额度</span><span>已用 Tokens</span><span /></div>
          {filtered.map(member => {
            const manager = team.find(item => item.username === member.boundTo);
            const tone = quotaTone(member.quota);
            return <div className={`ac-team-table-row${member.username === self.username ? ' is-self' : ''}${!member.active ? ' is-disabled' : ''}`} key={member.username}>
              ...
              <span className="ac-account-api-state"><Tag color={member.active ? 'green' : 'red'}>{member.active ? '账号正常' : '已停用'}</Tag>{member.role === 'member' ? <Switch size="small" checked={member.apiEnabled} onChange={checked => toggleApi(member, checked)} disabled={!member.active || member.username === self.username} /> : <Tag color="blue">自主</Tag>}</span>
              <span><Tag color={tone.color}>{tone.text}</Tag></span>
              <div className="ac-team-used"><strong>{formatTokens(member.usage?.month?.totalTokens)}</strong>{member.quota?.percent === null ? null : <Progress percent={member.quota?.percent || 0} showInfo={false} size="small" status={member.quota?.level === 'exhausted' ? 'exception' : 'normal'} />}<small>{formatCost(member.usage?.month)}</small></div>
        ...
        <section className="ac-panel">
          ...
          <div className="ac-ranking-list">
            {ranking.map((member, index) => <div className="ac-ranking-row" key={member.username}>
              ...
              <div className="ac-ranking-value"><strong>{formatTokens(member.usage?.month?.totalTokens)}</strong><small>{formatCost(member.usage?.month)}</small></div>
            </div>)}
    ...
    <Drawer title="添加团队成员" width={500} open={createOpen} onClose={() => setCreateOpen(false)} extra={<Button type="primary" onClick={() => createForm.submit()} loading={saving}>创建</Button>}>
      <Form form={createForm} layout="vertical" onFinish={create} initialValues={{ role: 'member', apiScopes: ['text'] }}>
        ...
        {self.role === 'dev' ? <Form.Item label="角色" name="role"><Select options={[
          { label: 'DEV · 开发', value: 'dev' }, { label: 'MANAGER · 管理', value: 'manager' }, { label: 'MEMBER · 组员', value: 'member' }
        ...
        <Form.Item label="API 授权范围" name="apiScopes"><Checkbox.Group options={API_SCOPE_OPTIONS} /></Form.Item>
        <p className="ac-form-tip">MEMBER 只能使用已勾选的团队能力，管理员 API Key 不会下发到浏览器。</p>
      </Form>
    </Drawer>

    <Drawer title={selected ? `${selected.displayName} · @${selected.username}` : '成员详情'} width={560} open={Boolean(selected)} onClose={() => setSelected(null)} extra={<Button type="primary" onClick={() => memberForm.submit()} loading={saving}>保存</Button>}>
      {selected ? <>
        <div className="ac-drawer-member-head"><Avatar size={66} src={selected.avatarUrl}>{avatarFallback(selected)}</Avatar><div><h2>{selected.displayName}</h2><p>@{selected.username}</p><div><RoleBadge role={selected.role} /><Tag color={selected.active ? 'green' : 'red'}>{selected.active ? '账号正常' : '账号停用'}</Tag></div></div></div>
        <div className="ac-drawer-usage-grid">
          ...
          <div><small>估算费用</small><strong>{formatCost(selectedUsage?.month)}</strong><span>{selectedUsage?.month?.pricedCalls || 0} 次有价格</span></div>
        </div>
        <Form form={memberForm} layout="vertical" onFinish={saveMember}>
          ...
          <Form.Item label="月度 Token 额度" name="monthlyTokenLimit"><InputNumber min={0} max={10_000_000_000} style={{ width: '100%' }} placeholder="留空表示不限额" /></Form.Item>
          {selected.role === 'member' ? <Form.Item label="API 授权范围" name="apiScopes" extra={!selected.active ? '账号已停用，恢复后才能修改 API scope。' : '文本、生图、TTS 可独立授权。'}><Checkbox.Group options={API_SCOPE_OPTIONS} disabled={!selected.active} /></Form.Item> : null}
        </Form>
        <section className="ac-governance-actions">
          <Button icon={selected.active ? <PauseCircle size={16} /> : <RotateCcw size={16} />} danger={selected.active} loading={saving} onClick={() => changeStatus(!selected.active)}>{selected.active ? '停用账号' : '恢复账号'}</Button>
          <Button icon={<KeyRound size={16} />} onClick={() => { passwordForm.resetFields(); setPasswordOpen(true); }}>重置密码</Button>
        </section>
        <section className="ac-drawer-activity"><h3>最近使用</h3>{(selectedUsage?.recent || []).slice(0, 6).map(item => <div key={item.id}><span>{item.feature || '模型调用'}</span><small>{formatDate(item.at)}</small><b>{formatTokens(item.totalTokens)} T · {item.costKnown ? `${item.costCurrency} ${Number(item.estimatedCost).toFixed(4)}` : '未计价'}</b></div>)}</section>
      </> : null}
    </Drawer>

    <Modal title={selected ? `重置 ${selected.displayName} 的密码` : '重置密码'} open={passwordOpen} onCancel={() => setPasswordOpen(false)} onOk={() => passwordForm.submit()} okText="确认重置" confirmLoading={saving}>
      <Form form={passwordForm} layout="vertical" onFinish={resetPassword}>
        <Form.Item label="新密码" name="password" rules={[{ required: true, min: 8, message: '新密码至少 8 位' }]}><Input.Password autoComplete="new-password" /></Form.Item>
        <Form.Item label="确认新密码" name="confirm" dependencies={['password']} rules={[
