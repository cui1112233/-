import { Avatar, Button, Form, Input, Modal, Skeleton, message } from 'antd';
import { Camera, CheckCircle2, Crown, KeyRound, Mail, Pencil, Phone, ShieldCheck, UsersRound } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [result, status, currentAccount] = await Promise.all([
        getMemberCenter(),
        getEmailVerificationStatus().catch(() => null),
        getCurrentAccount().catch(() => null)
      ]);
      setCenter(result);
      setSession(currentAccount);
  ...
  const hasAdminAccess = member.role === 'dev' || (session?.effectivePermissions || []).some(permission => permission.capability === '*' || permission.capability === 'admin:access');
  ...
  return <div className="account-center-page profile-page">
    ...
    <div className="ac-profile-cards-grid">
      ...
      <Panel title="联合治理权限" className="ac-profile-summary-card"><div className="ac-profile-governance"><UsersRound size={26} /><div><strong>{isGovernanceEnabled ? '已开启联合治理' : '当前为成员权限'}</strong><small>{isGovernanceEnabled ? '可以进入团队与成员治理工作区' : '由所属团队管理员统一治理'}</small></div></div><div className="ac-profile-checks"><span><CheckCircle2 size={15} />{isGovernanceEnabled ? '团队治理' : '团队协作'}</span><span><CheckCircle2 size={15} />{isGovernanceEnabled ? '成员授权管理' : '个人资料管理'}</span>{member.role === 'manager' ? <span><KeyRound size={15} />管理后台 {hasAdminAccess ? '已授权' : '未授权'}</span> : <span><CheckCircle2 size={15} />安全审计</span>}</div>{member.role === 'manager' && hasAdminAccess ? <Link href="/admin/presets" reload className="ac-profile-card-link">进入管理后台 <span>›</span></Link> : isGovernanceEnabled ? <Link href="/advanced-team-admin" className="ac-profile-card-link">前往联合治理中心 <span>›</span></Link> : <Link href="/member" className="ac-profile-card-link">查看团队身份与权益 <span>›</span></Link>}</Panel>
    </div>
