...
function bindWorkFormPersistence() {
  ...
  document.querySelectorAll('.process-version, [id^="processAiMethod"]').forEach(element => element.addEventListener('change', saveWorkFormState));
}
...
function selectedProcessVersions() {
  const versions = [...document.querySelectorAll('.process-version:checked')].map(input => input.value);
  return versions.length ? versions : ['original'];
}
