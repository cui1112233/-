...
- branch: `ops/v78-public-115.190.156.223-v2`

Go checkout：

- branch: `feat/v78-novel-fetch-go-bridge`

执行部署前必须记录两个 checkout 的完整 SHA，并先审查 diff。
...
```text
feat/v78-novel-fetch-go-bridge
```
...
1. V78 Node 能正常响应。
2. Node 能访问 Go API。
3. Node 能访问 121 Browser Worker。
4. 上述检查全部成功后，才启动 Nginx 80 公网入口。
5. 最后再次验证本机公网入口。
